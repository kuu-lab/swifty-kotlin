...
- [ ] BUG-042: 本ローカル環境（macOS, `DEVELOPER_DIR=/Applications/Xcode-beta.app`）で `bash Scripts/diff_kotlinc.sh` が対象ケースを問わず恒常的に `KSWIFTK-LINK-0001: Link step failed: Unable to locate packaged runtime object files under .../debug/Runtime.build.` で失敗する。原因: `Sources/CompilerBackend/CodegenRuntimeSupport+RuntimeObjects.swift` の `discoverRuntimeObjectPaths` が native SwiftPM（llbuild）incremental ビルドの出力レイアウト `<scratch>/debug/Runtime.build/*.swift.o` のみを探索するが、本環境の `swift build`（`.runtime-build` スクラッチ・リポジトリ直下 `.build` の両方）は XCBuild バックエンドで走り `out/Products/Debug/Runtime.o`（単一オブジェクト、`Runtime.build` ディレクトリ自体を生成しない）を出力するため、`buildRuntimeObjects` 経由の再ビルド後も discovery が空のままになり例外化する。最小再現: 任意の `.kt` 1ファイルで `bash Scripts/diff_kotlinc.sh --no-parallel Scripts/diff_cases/string_builder.kt`（StringBuilder 固有ではなく全ケースで再現。同一の full run で `Scripts/diff_cases` 配下 172+ ケースが同エラーで FAIL）。発見元: 本ファイル「CompilerCore: 参照ゼロの独立シンボル」節の `tryLowerStringBuilderMemberCall` 削除タスク検証中、リファクタ PR 必須ゲートの `diff_kotlinc.sh` 実行時に発覚（2026-07-17）。当該 PR の変更は `Sources/Runtime/`・`Sources/CompilerBackend/`・`Package.swift` を一切触れておらず本バグと無関係と確認済み（`swift build` 単体・関連 Sema/Lowering ユニットテストは green）。`.github/workflows/ci.yml` の diff_kotlinc ジョブは `runs-on: ubuntu-latest`（Linux、XCBuild非経由でnative SwiftPM/llbuildビルドのため本パス不一致は起きない想定）— CI では未再現の可能性が高いが、macOS ローカル開発環境全般（テストに `DEVELOPER_DIR=/Applications/Xcode-beta.app` を要する環境）に影響する可能性があるため要調査
