...
- [x] BUG-017: `lazy()`/`lazy(mode)` の戻り値型が `kotlin.properties.Lazy`（`lazyOf` と本家は `kotlin.Lazy`）→ KSP-681 で修正。2026-07-23 バグバックログ一括対応セッションで対応: `lazy()`/`lazyOf()` 自体の関数シグネチャは既に `kotlin.Lazy`（`rootLazyInterfaceSymbol`）を返す実装になっていたが、`HeaderHelpers+SyntheticPropertyDelegateStubs.swift` の `registerSyntheticPropertyInterfaceStubs` が同時に登録していた未使用の `kotlin.properties.Lazy`（`legacyLazyInterfaceSymbol`/`legacyLazyInterfaceType`。宣言直後にしか使われず、実質デッドコード）が、`resolveTypeRef` の非修飾名フォールバック（`lookupByShortName` を `symbolID.rawValue` 昇順でソートし先頭を採用— import 可視性を見ない）に混入し、ユーザーコードが明示的に `Lazy<T>` と書いた型注釈をこちらに誤解決させていた（登録順が早いため）。実プロダクトの `kotlin.properties` に `Lazy` は存在しないため、当該デッドコードを削除して解消（`val x: Lazy<Int> = lazyOf(1)` が `KSWIFTK-TYPE-0001` で失敗する再現を修正、`.build/debug/kswiftc` で確認）。**同時に発見・修正**: `lazy { ... }` を `by` 経由でなく直接呼び出した戻り値（`kotlin.Lazy<T>` の `.value` を明示的に読む等）が高確率でクラッシュしていた別バグも修正。原因は `lazy { ... }` のイニシャライザ引数が一般のクロージャ boxing 経路（`kk_function_create_0`）で渡されるのに対し、`RuntimeLazyBox.evaluateInitializer()`（`Sources/Runtime/RuntimeTypes.swift`）がそれをコンテキスト無しの生 C 関数ポインタとして直接 `unsafeBitCast` して呼び出しており、実際にはヒープオブジェクトのハンドルを命令ポインタとしてジャンプしていたため（bad pointer dereference で SIGSEGV）。`kk_function_invoke_0`（boxed closure なら unwrap、bare pointer ならそのまま呼ぶ）経由に変更して修正。回帰テスト: `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+PropertyDelegateEdgeCases.swift` に `testCodegenCompilesDirectLazyCallValueRead`/`testCodegenCompilesExplicitlyTypedLazyOfValueRead` を追加、`Scripts/diff_cases/lazy_direct_call_and_explicit_type.kt` を追加。既存の `RuntimeDelegateTests`/`RuntimeDelegateEdgeCaseTests`/`DelegatePropertyKIRTests`（計 61 件）は green。**残課題（本 PR では未修正、BUG-141/142 として新規追跡）**: (1) 関数ローカルスコープの `by lazy {}` は getValue を経由せず生ハンドルを返す別バグが残存（member/top-level スコープは正常）。(2) `Lazy<T>` の名前解決修正後も `val x: Lazy<Int> = lazy { 99 }` は別の pre-existing なジェネリック推論バグ（`fun <T> f(block: () -> T): T` 形の宣言を expected-type 駆動で呼ぶと `Conflicting bounds for type variable` になる。`lazy` 固有ではなく一般的な現象と確認済み）で失敗し続ける。(3) `Delegates.observable`/`vetoable`/`notNull` のメンバ委譲プロパティが広く壊れている（詳細は BUG-142）
