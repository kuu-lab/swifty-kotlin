...
最終更新: 2026-07-13（dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [ ] BUG-023: PR #4819（XCTest→Swift Testing移行）でプロセス全体共有のGC排他ロック（`RuntimeTestIsolationSupport.swift` の `gcSemaphore`、`IsolatedRuntimeXCTestCase(requiredLockSet: .gcOnly)` 由来）がファイルローカルな `NSLock` に縮小され、他の66件超の `IsolatedRuntimeXCTestCase` 使用ファイルとのクロスファイル排他が失われる — 対象 `Tests/RuntimeTests/RuntimeReadWriteLockTests.swift`（**実害を PR #4841 の CI 調査で確認**: 現在は `@Suite(.serialized)` + `init()`/`defer` からの `kk_runtime_force_reset()` 呼び出しになっているが、`.serialized` はスイート内直列化のみでクロススイート排他にはならず、BUG-040 と同一メカニズムで無関係スイートを巻き込む。詳細は BUG-040 参照）
- [ ] BUG-024: PR #4824（XCTest→Swift Testing移行）で BUG-021 と同型の `tearDown`（`kk_runtime_force_reset()`）消失 — 対象 `Tests/RuntimeTests/RuntimeSetCollectionHOFTests.swift`（**実害を PR #4841 の CI 調査で確認**: 現在は `@Suite(.serialized)` + `init()` からの `kk_runtime_force_reset()` 呼び出しに修正済みだが、`.serialized` はクロススイート排他にならないため BUG-040 と同一メカニズムで依然として無関係スイートを巻き込みうる。master HEAD `8445692247` 単独の CI 実行 (run 29416122834) でも `testSetFilterAllMatchReturnsAllElements` が `Set(listElements(result))` に期待値 `[3, 4, 2]` に対し `[0]` を返す形で failed しており、同型の破壊が疑われる）
...
- [ ] BUG-039: `launch { }` 直後（中間 suspend point なし）に `.cancel()` を呼ぶケースで、`CoroutineStart.DEFAULT` の cancel-before-start セマンティクス（PR #4730 で実装: 子コルーチン本体は一度も実行されないはず）が CI 高負荷時にレースし、本体が実行され `"cancelled"` が出力されることがある。症状: `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+CoroutineCancellationEdgeCases.swift` の `testCodegenCompilesCoroutineCancellationEdgeCases`（`assertKotlinOutput` はコンパイル後の実行バイナリを別プロセスとして起動するため、レースは Swift テストランナーでなく生成された Kotlin プログラム自身のコルーチンランタイム内で発生する）が期待値 `"null\nboom\n"` に対し `"null\ncancelled\nboom\n"` を返して failed。最小再現: 同ファイルの Kotlin ソース（`Scripts/diff_cases/coroutine_cancellation_edge_cases.kt` 相当）。単体実行では未再現、CI高負荷時のみ発現し `coroutine_delay_basic.kt` の既知タイミングフレーク（KSP-499 節）と同系統だが、200msしきい値超過ではなく launch dispatch と cancel の間の非アトミックな窓でディスパッチ順序自体が入れ替わる点で異なる。疑わしい実装箇所: `Sources/Runtime/RuntimeCoroutine.swift` の launch/cancel 経路。発見元: PR #4828 の CI 失敗調査（https://github.com/kuu-lab/swifty-kotlin/actions/runs/29344930107/job/87144476138 で観測。本PR自体の変更とは無関係の既存バグ）
- [ ] BUG-040: Swift Testing 移行後の `@Suite(.serialized)` スイートが `kk_runtime_force_reset()`（`RuntimeGC.swift` の `objectPointers`/`heapObjects` 等をプロセス全体で無条件クリアする関数）を `init()`/各 `@Test` の `defer` から呼ぶが、`.serialized` trait は **そのスイート内のテスト同士のみ** を直列化し、`.serialized` を持たない（あるいは別の）他スイートとの並行実行は一切防がない。旧 XCTest 版の `RuntimeTestIsolationSupport.swift` にある `IsolatedRuntimeXCTestCase` はプロセス全体で共有される `gcSemaphore` 等で全クラス横断の排他を取っていたが、Swift Testing 移行後はこの「クロススイート排他」に相当する機構が存在しない（BUG-023/BUG-024 が個別ファイルで指摘している構造と同型で、根本原因は共通）。症状: PR #4841 の CI 再実行 (`SWIFT_TEST_WORKERS=1` で argument-list-too-long 修正後) で `RuntimeNativeByteArraySettersTests.testSignedByteArrayStoresUseLittleEndianLayout` 系のテストが `Fatal error: KSwiftK panic [KSWIFTK-RUNTIME-0001]: invalid array handle in kk_native_byteArray_getShortAt`/`getUIntAt` で signal 4 (illegal instruction) crash（`RuntimeNativeByteArraySettersTests` 自体は `.serialized` の対象外で `kk_runtime_force_reset()` も呼ばないが、同一プロセス内で並行実行されていた `RuntimeSynchronizedTests`/`RuntimeSetCollectionHOFTests` 等の reset 呼び出しに `kk_array_new` 直後のハンドルを `objectPointers` から消され、`runtimeArrayBox(from:)` が nil を返した）。再現: 決定論的な最小再現コードはない（並行実行タイミング依存）。`Tests/RuntimeTests/RuntimeSynchronizedTests.swift`（`@Suite(.serialized)`、`init()`/各 `@Test` の `defer` で `kk_runtime_force_reset()` 呼び出し、BUG-023/024 未収載の新規インスタンス）が今回確認できた具体的な加害側。Swift Testing の196テストは `SWIFT_TEST_WORKERS`（SwiftPM の XCTest 向け worker 分散にのみ影響）に関わらず単一プロセス内で同時実行されるため、workers=1/4 いずれでも起こりうる — 実際 master HEAD `8445692247` 単独の workers=4 実行 (run 29416122834) でも `RuntimeSetCollectionHOFTests.testSetFilterAllMatchReturnsAllElements` が異常値で failed しており、同型の破壊とみられる（BUG-024 参照）。修正には suite 単位の `.serialized` ではなく、`kk_runtime_force_reset()`（および同様にプロセス全体状態を書き換える関数）を呼ぶ全スイートが共有する単一のグローバルな排他機構（例: 全スイートが明示的に acquire する global actor / 共有 lock）が必要と見られる。対象になりうる全スイートの棚卸しは未実施 — `grep -rl kk_runtime_force_reset Tests/RuntimeTests/` で該当ファイルを列挙できる。発見元: PR #4841 の CI 失敗調査（https://github.com/kuu-lab/swifty-kotlin/actions/runs/29475568461/job/87566024563 で観測。本PR自体の変更とは無関係の既存バグ。argument-list-too-long 修正 (`.github/workflows/ci.yml` の `RuntimeTests` を `--num-workers 1` に変更) 後に露見したが、上記の通り worker 数非依存のため以前から潜在していたとみられる）
