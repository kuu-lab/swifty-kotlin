...
- [~] RF-STDLIB-002: `LoadSourcesPhase` に bundled Stdlib ソース読み込みを実装する（`Bundle.module` 列挙 → `sourceManager` 登録は基本配線済み。2026-07-15 監査で残リストを補正: ユーザー入力との診断パス区別は `__bundled_` 接頭辞機構で実装済みのため残から除外。残: `--no-stdlib` の実配線（BUG-041 — フラグはパースされるが未参照の死にフラグで、`injectBundledStdlib` は無条件実行）、形式的な source origin 型の導入）
