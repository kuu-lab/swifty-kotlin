...
最終更新: 2026-07-17（dead code 削除タスク1件完了。作業中に `CompilerCoreTests`（Swift Testing）のフルスイート実行が既存バグで完走不能なことを発見し BUG-040/BUG-041 を追補。それ以前: 2026-07-16 master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。CI 側に暫定緩和を適用 — 詳細は BUG-039。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [x] DEADCODE-CORE-001: [R0] `HeaderHelpers+SyntheticNativeInteropHelpers.swift:62` の private `syntheticListType(elementType:symbols:types:interner:)` を削除する。別ファイルの同名 private helper は別 USR。2026-07-16 完了: 呼び出し箇所ゼロ（同名 private/fileprivate は他5ファイルにそれぞれ独立定義され自ファイル内で使用済み、別 USR で無関係）を確認して削除、ビルド green。テストゲートは BUG-040（無関係な既存クラッシュ）により CompilerCoreTests がフル完走不能なため、該当テストを除外して残りが green であることを確認
...
- [ ] BUG-040: `Tests/CompilerCoreTests/Sema/DataFlowAndSemaRegressionTests.swift:338` の `testDeeplyNestedGenericTypeRefEmitsDepthDiagnostic()`（`ASTArena` で `List<List<...600階層...<Int>>>` の `TypeRef` を直接構築し `resolveTypeRef` に通して `KSWIFTK-SEMA-TYPE-DEPTH` 診断を期待する回帰テスト）が、診断を返す前にネイティブスタックオーバーフローで `CompilerCoreTests`（Swift Testing）プロセス全体を `signal 10`（SIGBUS）でクラッシュさせる。テスト開始直後に落ちるため同バイナリ内の以降の全テストが未実行のまま abort し、`--filter CompilerCoreTests` 単体実行はおろかフル `swift_test.sh` 実行も CompilerCoreTests 到達時点で失敗する。`DataFlowSemaPhase.maxStructuralRecursionDepth`（512、`Sources/CompilerCore/Sema/DataFlow/BodyAnalysis.swift:514`）のガードは `resolveTypeRef`（同 `:62`）冒頭で毎回判定され `resolveTypeArgRefs` 経由で深さも正しく +1 されているが、ガードが発火する 512 段目に到達するまでの `resolveTypeRef`⇄`resolveTypeArgRefs` 相互再帰そのものが、この実行環境のスレッドスタックを使い切ってしまう模様（ガードの判定ロジック自体は正しく見えるが、セーフティネットとして機能していない）。実 Kotlin ソース経由では Parser 側の `KSWIFTK-PARSE-TYPE-DEPTH`（`Sources/CompilerCore/AST/TypeRefParserCore.swift:42`、上限512）が先に働き未然に防がれることを確認済み（600階層ネストの `List<Int>` 型注釈を `.kt` にして `kswiftc` に通すと `KSWIFTK-PARSE-TYPE-DEPTH` のパースエラーで正常終了し、クラッシュしない）。再現: `DEVELOPER_DIR=/Applications/Xcode-beta.app SWIFT_TEST_PARALLEL=0 swift test --filter CompilerCoreTests.DataFlowAndSemaRegressionTests/testDeeplyNestedGenericTypeRefEmitsDepthDiagnostic`。HEAD `a28cd1cf13`（`git stash` で変更前の状態に退避）でも同様に再現するため、当該変更とは無関係な既存バグと確認済み。発見元: 本ファイル「CompilerCore: 参照ゼロの独立シンボル」節の dead code 削除タスク第1項（`HeaderHelpers+SyntheticNativeInteropHelpers.swift` の private `syntheticListType` 削除）の完了ゲートとして `CompilerCoreTests` を実行中に遭遇
- [ ] BUG-041: BUG-040 とは別に、`CompilerCoreTests`（Swift Testing）のフルスイート実行が非決定的な箇所で `signal 10`（SIGBUS）落ちする。3回試行してそれぞれ異なる箇所で発生: (1) `LexerParserEdgeCaseTests` スイート完走直後、(2) 同一区間（baseline `git stash` 時、BUG-040 のみ除外）、(3) `LexerParserEdgeCaseTests` スイート丸ごと `--skip` で除外すると、今度は無関係な `LibMetadataSerializationTests` 内 `testAnnotationRecordEquatable()` 付近に発生箇所が移動。3回とも、クラッシュ行の直前・直後に表示される個別テストは stdout/stderr インターリーブで名前が分断されているだけで、実際にはそのテスト自身は直後に `passed` している（例: 3回目は `testAnnotationRecordEquatable() passed after 0.001 seconds.` が crash 行の直後に出力される）ため、特定のテストの決定的バグではなく、フルスイートを1プロセスで長時間流したときにタイミング依存で発生する crash と判明。累積的なメモリ/スタック圧迫（大量の `ASTArena` 等の解放や Swift Testing のスレッド管理起因）を疑うが未確認。再現: `DEVELOPER_DIR=/Applications/Xcode-beta.app SWIFT_TEST_PARALLEL=0 bash Scripts/swift_test.sh --filter CompilerCoreTests --skip testDeeplyNestedGenericTypeRefEmitsDepthDiagnostic`（BUG-040 のみ除外）を複数回実行。HEAD `a28cd1cf13`（`git stash` で変更前の状態に退避）でも同種の crash が再現するため、当該変更とは無関係な既存バグと確認済み。BUG-023/024/028/039（Swift Testing 移行でプロセス全体状態のクロススイート排他が失われた系）と症状が類似するが、CompilerCoreTests は Runtime の GC 排他ロック等を使わない純フロントエンドテストのため同一原因とは断定できない。根本原因・決定的な最小再現は未特定。発見元: 本ファイル「CompilerCore: 参照ゼロの独立シンボル」節の dead code 削除タスク第1項（`HeaderHelpers+SyntheticNativeInteropHelpers.swift` の private `syntheticListType` 削除）の完了ゲートとして、BUG-040 除外後の `CompilerCoreTests` フル実行を複数回試行中に遭遇
