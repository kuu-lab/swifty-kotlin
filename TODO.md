...
- [ ] BUG-159: `StringBuilder` を `Appendable` 型の変数に代入して `append(...)` を呼ぶと、仮想ディスパッチが itable にエントリを見つけられず `KSWIFTK-RUNTIME-0001: Virtual dispatch failed: method not found in vtable/itable` でパニックする。`StringBuilder` の bundled Kotlin ソース（`Sources/CompilerCore/Stdlib/kotlin/text/StringBuilder.kt`）は `Appendable` を実装していないため、Sema 層では `Appendable` インターフェース経由の呼び出しが解決されるが、KIR/runtime の itable/vtable には `StringBuilder` の `append` 実装が `Appendable` 用スロットに登録されない。最小再現:
  ```kotlin
