...
- [x] DEADCODE-RUNTIME-020: [R0] `RuntimeCollectionHelpers.swift:676` の `runtimeInvokeCollectionLambda5(...)` を削除する。2026-07-22 完了: 宣言以外の参照ゼロを再確認して削除
- [x] DEADCODE-RUNTIME-021: [D: RUNTIME-020] `RuntimeCollectionHelpers.swift:601` の `RuntimeCollectionLambda5` typealias を削除する。2026-07-22 完了: RUNTIME-020 削除後に参照ゼロを確認して削除
- [x] DEADCODE-RUNTIME-022: [R0] `RuntimeCoroutineContext.swift:569` の `dispatchQueue(for:)` を削除する。live dispatch は `runtimeResolveDispatcher(from:)` + `RuntimeDispatcher.queue`。2026-07-22 完了: 参照ゼロを確認して削除
- [x] DEADCODE-RUNTIME-023: [R0] `RuntimeReflection.swift:26` の private `runtimeReflectionStringRaw(_:)` を削除する。2026-07-22 完了: 参照ゼロを確認して削除
- [x] DEADCODE-RUNTIME-024: [R0] `RuntimeRandom.swift:46` の `SeededRandomBox.nextInt(bound:)` を削除する。2026-07-22 完了（要修正版で実施）: 監査記載と異なり `kk_random_nextInt_until`/`kk_random_nextLong_until` から実際に呼ばれていたため単独削除はビルドを破壊する状態だった。調査の結果、これらの呼び出し元自体が KSP-466（`RuntimeABISpec+Random.swift` のコメント参照）で ABI 表面から既に外れ、`@_cdecl` も無く外部参照ゼロの死んだラッパーだったと判明。RUNTIME-025〜028 分含め呼び出し元 11 関数（`kk_random_nextInt_until`/`_range`, `kk_random_nextLong_until`/`_range`, `kk_random_nextFloat`/`_until`/`_range`, `kk_random_nextDouble`/`_until`/`_range`, `kk_random_nextBoolean`）を同時に削除して整合。`nextFullInt()`/`nextBits()` は指示通り残置
- [x] DEADCODE-RUNTIME-025: [R0] `RuntimeRandom.swift:53` の `SeededRandomBox.nextIntRange(from:until:)` を削除する。2026-07-22 完了: RUNTIME-024 と同じ理由で呼び出し元 `kk_random_nextInt_range`/`kk_random_nextLong_range` も同時削除
- [x] DEADCODE-RUNTIME-026: [R0] `RuntimeRandom.swift:65` の `SeededRandomBox.nextDouble()` を削除する。2026-07-22 完了: 呼び出し元 `kk_random_nextDouble`/`_until`/`_range`（いずれも `@_cdecl` なし・外部参照ゼロの死んだラッパー）も同時削除
- [x] DEADCODE-RUNTIME-027: [R0] `RuntimeRandom.swift:72` の `SeededRandomBox.nextFloat()` を削除する。2026-07-22 完了: 呼び出し元 `kk_random_nextFloat`/`_until`/`_range` も同時削除
- [x] DEADCODE-RUNTIME-028: [R0] `RuntimeRandom.swift:78` の `SeededRandomBox.nextBoolean()` を削除する。`SecureRandomBox` が使う `nextBits()` と test-only `nextFullInt()` は残す。2026-07-22 完了: 呼び出し元 `kk_random_nextBoolean` も同時削除
- [x] DEADCODE-RUNTIME-029: [W0/effectless] `RuntimeCoroutine.swift:914` の `RuntimeJobHandle.setParent(_:)` と製品 5 + test 2 の call site を削除する。唯一の効果は unread `parentJob` への代入で、子キャンセルは `registerChild` が担う。2026-07-22 完了: 記載通り製品5+test2箇所を削除
- [x] DEADCODE-RUNTIME-030: [W0; 前提 RUNTIME-029] `RuntimeCoroutine.swift:865` の weak `RuntimeJobHandle.parentJob` を削除する。2026-07-22 完了
- [x] DEADCODE-RUNTIME-031: [R0] `RuntimeCoroutine.swift:1239` の `RuntimeCoroutineScope.setParent(_:)` を削除する。live パスは `scope.parent = ...` の直接代入。2026-07-22 完了: 参照ゼロを確認して削除
- [x] DEADCODE-RUNTIME-032: [R0] `RuntimeNativeAPI.swift:527` の `RuntimeCValuesBox.rawAddress` を削除する。2026-07-22 完了
- [x] DEADCODE-RUNTIME-033: [R0] `RuntimePath.swift:1867` の `RuntimeFileVisitorBox.onVisitFileFailedRaw` を削除する。setter/reader/stub/test は全て 0。2026-07-22 完了
- [x] DEADCODE-RUNTIME-034: [W0] `RuntimeTypes.swift:2102` の `RuntimeInputStreamBox.markOffset` と initializer 代入を削除する。2026-07-22 完了
- [x] DEADCODE-RUNTIME-035: [W0] `RuntimeTypes.swift:2103` の `RuntimeInputStreamBox.markLimit` と initializer 代入を削除する。`mark` は明示的 no-op、`reset` は常に false。2026-07-22 完了
- [x] DEADCODE-RUNTIME-036: [W0] `RuntimeMemory.swift:13` の `RuntimeMemorySnapshot.usedBytes` を削除する。同名 local は `totalBytes` 計算に必要なため残す。2026-07-22 完了
- [x] DEADCODE-RUNTIME-037: [W0] `RuntimeMemory.swift:17` の `RuntimeMemorySnapshot.heapObjectCount` を削除し、snapshot 作成時の GC-lock/count 計算も消す。2026-07-22 完了
- [x] DEADCODE-RUNTIME-038: [W0] `RuntimeMemory.swift:18` の `RuntimeMemorySnapshot.uptimeNanos` を削除し、`runtimeCaptureMemorySnapshot(nowNanos:)` の不要引数/default も整理する。2026-07-22 完了
- [x] DEADCODE-RUNTIME-039: [R0/public property] `RuntimeABISpec+Char.swift:3` の `RuntimeABISpec.charClassificationFunctions` を削除し、空になるファイルも削除する。内包 25 spec は全て `charFunctions` 等に重複登録済みで `allFunctions`/C header は不変。2026-07-22 完了: 25 spec全件が `RuntimeABISpec+String.swift` の `stringFunctions`（`allFunctions` 登録済み）に重複していることを確認しファイルごと削除
- [x] DEADCODE-RUNTIME-040: [R0] `RuntimeTypes.swift:2323` の `RuntimeOutputStreamBox.isClosed` を削除する。常に `false` を返す未参照 property。2026-07-22 完了
- [x] DEADCODE-RUNTIME-041: [R0] `RuntimeCoroutine.swift:788` の `RuntimeTaskHandle.awaitResultThrowing(outThrown:)` を削除する。suspend-aware/live な `awaitResult` 経路は残す。2026-07-22 完了: 記載の型名 `RuntimeTaskHandle` は現行コードに存在せず、実体は `RuntimeAsyncTask`（行番号・シグネチャ・周辺コメントが完全一致のため同一対象と判断）。参照ゼロを確認して削除
- [x] DEADCODE-RUNTIME-042: [R0 overload] `RuntimeStringFormat.swift:151` の private `runtimeFormatString(_:arguments: [Int], locale:)` を削除する。`[RuntimeValue]` overload は live。2026-07-22 完了
...
- [x] KSP-CAP-002: for-in がユーザー実装 Iterator/Iterable で1回も回らず黙って終了する lowering 欠陥を修正する（= BUG-013。`hasNext()`/`next()` 手動呼び出しは動作する）。PR #4986。検証: `Counter(3)`/`CounterBag(3)` の for-in が期待通り `for=3` を出力、`swift test --filter BuildKIRRegressionTests`、`bash Scripts/swift_test.sh --filter SmokeTests`、`bash Scripts/validate_runtime_abi_links.sh` green。ブロック対象: KSP-441 以降の全 Sequence/Iterator 系
...
- [x] BUG-044: PR #4572（StringBuilder source 移行）で StringBuilder コンストラクタの新しい intercept 経路（`CallLowerer.swift` の `isStringBuilderConstructor` + `lowerStringBuilderConstructorCall`）が `kk_string_builder_new`/`kk_string_builder_new_from_string_flat` を直接呼ぶのみで、通常のコンストラクタ経路が行う `kk_type_register_super`/`kk_object_register_itable_iface` 相当の型登録をスキップする。`kk_op_is`（`Sources/Runtime/RuntimeStringArray.swift:450-`）は String 等の組込み型は専用 `stringBase` 分岐を持つが CharSequence/Appendable のような interface 型は `nominalBase` 分岐で `runtimeObjectTypeID(rawValue:)` に依存するため、型登録が無い StringBuilder ハンドルに対する `sb is CharSequence` / `sb is Appendable` は誤って `false` を返す疑い（コード追跡で確認、実行検証は BUG-042 によりこのセッションでは不可）。BUG-038（hand-crafted Runtime オブジェクトの itable 登録漏れ）と同系統の型登録漏れパターン。発見元: PR #4572 の Devin AI レビュー（comment_id 3537407331）をコード追跡で確認。2026-07-23 バグバックログ一括対応セッションで修正: クリーンな Linux 環境でまず実機再現（`sb is CharSequence`/`sb is Appendable` が両方 `false`）を確認した上で、`Sources/Runtime/RuntimeStringBuilder.swift` の `kk_string_builder_new`/`runtimeStringBuilderNew(initial:)`（2つの StringBuilder 構築経路）に `runtimeRegisterObjectType(rawValue:classID:)` によるインスタンスの type ID 登録と `runtimeRegisterTypeEdge` による `kotlin.text.StringBuilder → kotlin.CharSequence`/`kotlin.text.Appendable` の supertype edge 登録を追加。両 API とも `kk_runtime_reset_metadata()` が管理する state（`objectTypeByPointer`/`typeParents`）を使うため、テスト分離用リセットでも自動的に整合性が保たれる。回帰テスト: `CodegenBackendIntegrationTests+StringBuilderEdgeCases.swift` に `testCodegenStringBuilderIsCharSequenceAndAppendable` を追加、`Scripts/diff_cases/stringbuilder_is_char_sequence_appendable.kt` を追加。**フォローアップ（Devin AI レビュー指摘）**: `buildStringBuilder { ... }` DSL（実際に StringBuilder を返す方 — `buildString { ... }` は内部で同じ StringBuilder を組み立てた後 `.toString()` で String に変換して返すため対象外）は `RuntimeBuilderDSL.swift` の別経路 `kk_build_string_builder`（`kk_build_string_builder_with_capacity` はこれに委譲）で StringBuilder を構築しており、上記の `kk_string_builder_new`/`runtimeStringBuilderNew(initial:)` の修正だけではカバーされず同じ症状が残っていた。`runtimeRegisterStringBuilderType` を `private` から可視にし、`kk_build_string_builder` からも呼ぶよう修正。回帰テスト: `testCodegenBuildStringBuilderResultIsCharSequenceAndAppendable` を追加。**関連して発見した別バグ（未修正、BUG-152 として新規追跡）**: `CharSequence` インターフェース自体には `length`/`get`/`subSequence` がメンバとして登録されておらず `String` 型への extension としてのみ存在するため、`val cs: CharSequence = "x"; cs.length` のような「静的型が `CharSequence` である値」への基本メンバアクセスが `String` を代入した場合でも解決できない（本項目の `is` チェック修正はこの制限とは独立に完了している）。**フォローアップ検証中にさらに発見した別バグ（未修正、BUG-153 として新規追跡）**: `buildString { ... }` の戻り値のように、コンパイル時リテラルではなく実行時に構築された `String` 値は `is CharSequence` が誤って `false` を返す（`"literal" is CharSequence` は `true` だが、`"he" + "llo"`・`StringBuilder("x").toString()`・`buildString { }` 等いずれの動的生成 String も `is CharSequence` が `false` になることを実機で確認）。根本原因は `kk_op_is`（`Sources/Runtime/RuntimeStringArray.swift:450-`）の `CharSequence` チェックが `String` 専用の `stringBase` 分岐ではなく `nominalBase` 分岐（`runtimeObjectTypeID(rawValue:)` 依存）を通ることで、`String` 自身が `kotlin.String → kotlin.CharSequence` の supertype edge を一度も登録していないため（本タスクで StringBuilder には登録したのと対照的）。リテラルの `is CharSequence` が偶然 `true` になるのは、おそらく Sema/lowering 側でリテラル式に対する `is` チェックが定数畳み込みされ、この実行時パスを経由しないため。最小再現: `fun main() { val a = "he" + "llo"; println(a is CharSequence) }` は `false`（本家 kotlinc は `true`）。今回修正できない理由: 全ての `String` 生成経路（flat string 生成・`toString()`・concatenation 等）を横断して type ID 登録を行き渡らせるか、`kk_op_is` の `CharSequence` 判定を `nominalBase` 経由ではなく `stringBase` 相当のチェックにも フォールバックさせるかの設計判断が必要で、本 PR の StringBuilder 固有の修正スコープを大きく超えるため見送り
- [x] BUG-045: Golden.Sema ハーネスの合成序数正規化（`GoldenHarnessSemaComparisonNormalizer.rewriteOrdinalMatches`、対象は `__local_N`/`$N`/`__for_N` 等）が、生の raw ID（`ExprID`/`SymbolID` の値をそのまま埋め込んだ文字列）を`GoldenHarnessDump.renderSemaOutput`/`GoldenHarnessStableRenderContext.stableKey` で**文字列比較ソート**した後に初出順で 0 から振り直す実装になっている。raw ID の桁数（例: 3桁→4桁の 999→1000 境界）がローカル変数群の宣言順スパンをまたぐと、文字列ソートが数値順と食い違い、ゴールデン出力上の宣言順が入れ替わる（例: `outer`/`client1`/`client2`/`file1`/`file2` が実際の宣言順より前に来る）。コンパイラの実際の意味論（シンボル名・型・kind・呼び出し解決）は完全に等価で実害はないが、stdlib バンドル内容を追加/削除するだけで無関係な golden ファイルが壊れる — PR #4624 で `Tests/CompilerCoreTests/GoldenCases/Sema/companion_object_private_access.golden` が該当（`Sources/CompilerCore/Stdlib/kotlin/reflect/KClassBasicAPI.kt`/`KClassMemberIntrospection.kt` 新規追加が raw ExprID のベースラインをシフトさせたことが引き金と推定）。恒久修正は `rewriteOrdinalMatches`/`stableKey` 側でゼロ埋め比較または数値ソートに直すこと。本PRのマージ作業中、masterの取り込みの度に同じgolden(`companion_object_private_access.golden`)が繰り返し再発（2026-07-08、2026-07-16、2026-07-18 の少なくとも3回再生成が必要だった）。発見元: PR #4624 の CI 失敗調査タスク（`Full Swift Tests (CompilerCoreTests shard 0/3)`）。2026-07-23 バグバックログ一括対応セッションで現行 HEAD を再確認: `GoldenHarnessDump.swift` のシンボル列挙ソートは `ctx.stableKey(for:).compare(_:options:.numeric)` を使用しており数値境界をまたいでも正しく順序付けられる。加えて `GoldenHarnessStableRenderContext.stableKey(for:)` 自体が raw ID を埋め込まない実装（完全修飾名 + 決定的な overload sort から導出した `#N` サフィックス）に変わっており、そもそも本項目が指摘する「raw ID の文字列比較」という前提条件自体が現行実装には存在しない。本項目と BUG-140 は同一クラスの不具合の異なる時点での発見であり、BUG-140 の修正（`.compare` を数値比較化）で本項目も解消したと判断した
...
- [x] BUG-140: golden ハーネスの Sema symbol 一覧ソートが `GoldenHarnessDump.swift` の `requiredSymbols.sorted { ctx.stableKey(...) < ... }` で素の文字列比較を使っており、`__local_N` 等の合成スコープ名に埋め込まれた生の（ゼロ埋めなし）arena 序数が桁数境界をまたぐと表示順が崩れる（例: `"__local_10002" < "__local_9874"` が文字列比較では真になる。`'1' < '9'` のため）。この境界は bundled stdlib 側の式数が変わるだけで無関係なテストケースの golden 表示順を壊しうる。最小再現: `Tests/CompilerCoreTests/GoldenCases/Sema/companion_object_private_access.kt`（本 PR の BUG-038 対応で `RangeMembership.kt` を1関数→3関数に分割し bundled stdlib の式数が+約30 増えた副作用で、`client2`/`file1`/`file2` の local ordinal が 9999→10000 をまたぎ表示順が崩れて CI 落ち。`GoldenHarnessWorker` を直接実行し、master 版 `RangeMembership.kt` に戻すと境界をまたがず再現しないことを確認済み。3回連続実行で出力が完全一致し非決定性ではないことも確認済み）。`.compare(_:options:.numeric)` に変更して修正。他297件の Sema golden を body 参照順の単調性で静的検査し、他に同種の崩れがないことを確認済み（0件）— 発見元 `CI-4669` / PR #4669 open。2026-07-23 バグバックログ一括対応セッションで現行 HEAD を再確認: `GoldenHarnessDump.swift:134` の該当ソートは `.compare(_:options:.numeric)` を使用しており記載の修正が適用済みであることをソース確認した
