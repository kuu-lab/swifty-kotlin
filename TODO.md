...
- [x] RF-LOWER-CALL-004: `buildList` の旧runtime rewriteを削除する（前提: CALL-003、KSP-697の必要な移行・fallback整理完了）
  ...
  - 順序について: 前提の CALL-003 は未着手のまま先行した。CALL-003 の対象 `isSourceBackedStdlibBuilderDSLCall` は `isStdlibBuilderDSLCall` 内の恒偽分岐で、本項の後も `buildSet` / `buildMap` 用に残存するため buildList 経路と直交する。CALL-002/003 の並行PRは無し（`gh pr list --search "RF-LOWER-CALL"` で確認）なので、直列化の理由（同一 lookup / dispatcher / test の編集衝突）も発生しない。
  - 削除内容:
