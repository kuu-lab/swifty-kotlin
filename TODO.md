...
- [x] DEADCODE-CORE-042: [W0] 同ファイル `:776` の `onLeaveName` と initializer `:1567` の代入を削除する。2026-07-17 完了: プロジェクト全体 grep で宣言・初期化子代入(現行行番号は `:775`/`:1565`)以外の参照ゼロを確認し削除。`swift build` 成功、`loc_report.sh` メトリクス悪化なし、Golden(13 tests/6 suites)・拡張タイムアウト版 `diff_kotlinc`(639 中 0 failed)は green。完了ゲート実行中に並行 worktree 高負荷由来のテスト疑似失敗を多数観測、詳細は BUG-041
...
- [ ] BUG-041: 本ファイル Dead Code 削除タスクの必須ゲート実行中（worktree 58個同時稼働、load average 一時292超）に判明したテストゲートの負荷脆弱性2件。(1) `Tests/CompilerBackendTests/KIR/DelegatePropertyKIRTests.swift:362` の `testNotNullDelegateReadBeforeAssignmentTrapsWithHelpfulMessage` は生成したネイティブ実行ファイルの起動〜終了待ちに固定 `Date().addingTimeInterval(5)`（+1秒 terminate 猶予）のウォールクロックデッドラインを使っており、他 worktree の同時ビルド負荷下では compile→link→exec の所要時間がこれを超え `Timed out waiting for delegated property test executable to exit` で失敗する。load average 5〜9 まで下げても4回中4回再現した一方、同条件で同時に切り分けた `CodegenBackendIntegrationTests` 側の一過性失敗（延べ26件）は全て0件に収束したため、本テストのみデッドラインが恒常的に短すぎると判断（真に idle な状態は未検証）。(2) `Scripts/diff_kotlinc.sh` のデフォルト `--run-timeout 10`（秒）が同条件下では不十分で、ロジック差分が皆無にもかかわらず639ケース中636件が `candidate run timed out after 10s` で一律 FAIL する（`--run-timeout 60 --compile-timeout 180` に伸ばすと 639 中 0 failed / 3 passed→639 passed で再現。並行 worktree 多数稼働時にデフォルトタイムアウトが不十分になる既知の環境要因と同根）。恒久対応は未着手（(1) はテストのデッドラインを負荷耐性のある値に緩めるか `CommandRunner` 経由の実行に統一、(2) は CLAUDE.md のリファクタ PR 必須ゲート手順に `--run-timeout` 明記、または worktree 同時実行検知時のデフォルト値底上げ）。発見元: 本ファイル `onLeaveName` 削除タスクの完了ゲート実行（2026-07-17）。当該削除との因果関係は否定済み（grep で参照ゼロ確認、Golden/拡張タイムアウト版 diff_kotlinc は green）
