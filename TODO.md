...
- [ ] BUG-042: `LoweringCodegenRegressionTests.testKxMiniRunBlockingDelayExecutableReturnsExpectedExitCode` が現行 `master` で実行ファイルリンクに失敗し、`CompilerPipelineError.outputUnavailable` になる。`KSWIFTK-LINK-0001` の linker diagnostics には `__kk_byteArray_toKString` / `__kk_bytearray_decodeToString*` / `__kk_charset_*` / `kk_kclass_*` / 例外生成関数など多数の undefined symbol が含まれる。最小再現 Kotlin: `suspend fun delayedValue(): Int { delay(1); return 42 }` / `fun main(): Any? = runBlocking(delayedValue)`（既存ケース: `Tests/CompilerBackendTests/Lowering/LoweringCodegenRegressionTests.swift`）。再現コマンド: `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer SWIFT_TEST_PARALLEL=0 bash Scripts/swift_test.sh --filter LoweringCodegenRegressionTests`。発見元: XCTest→Swift Testing 全件移行タスク（2026-07-17、移行前の XCTest 版でも失敗することを比較確認）。
