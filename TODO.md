...
- [ ] BUG-038: `launch { ... }` 直後（間に suspend を挟まず）に `.cancel()` した場合、本体を一切実行しない「cancel-before-start」保証（PR #4730 / commit `d419b2b854` で導入）が本物の OS スレッド競合により時々破れ、キャンセルされたはずのコルーチン本体が実際に走り出し `try { ... } finally { ... }` の finally が実行されてしまう。原因: `KxMiniRuntime.launch(workItem:)`（`Sources/Runtime/RuntimeHelpers.swift:306-308`）は `DispatchQueue.global()` の実 OS スレッドへ dispatch する一方、`job.cancel()`（`Sources/Runtime/RuntimeCoroutine.swift` の `cancel(message:cause:)`、~1064-1123 付近）は呼び出し元スレッドで同期実行される。`job.dispatchWorkItem?.cancel()` + dispatch 先頭での `markStarted()`/`cancellationSnapshot()` 再チェック（同ファイル、`kk_kxmini_launch` 系関数群に同型パターンが repeat）はレースウィンドウを縮めるだけで閉じておらず、`repeat{ensureActive(); delay(1)}`（=job2、finally なし）と `try{delay(...)}finally{...}`（=job3/job4）を区別しない content-agnostic な guard のため、京大 vs スケジューラのどちらが先着するかは非決定的。本家 Kotlin の `runBlocking` は単一スレッド協調スケジューリングのため構造的にこの競合が起こらない。最小再現: `Scripts/diff_cases/coroutine_cancellation_advanced.kt` の job3/job4（`val job = launch { try { delay(Long.MAX_VALUE) } finally { println("finally ran") } }; job.cancel(); job.join()` で "finally ran" が漏れることがある。高負荷環境ほど再現しやすい）。発見元: PR #4700（arrayOf boxing 修正）の CI 失敗調査（kotlinc Diff Regression Shard 1/11: `coroutine_cancellation_advanced.kt` の stdout に想定外の "finally ran"/"cleanup done" が混入）。PR #4700 自体の変更（`CallLowerer+Operators.swift` の配列インデックスアクセス boxing）とは無関係の既存バグ
- [ ] BUG-038: `launch { }` の cancel-before-start 短絡（PR #4730 で追加、`RuntimeJobHandle.cancel`/`markStarted`/`cancellationSnapshot`, `Sources/Runtime/RuntimeCoroutine.swift`）が本物のレース条件で機能しないことがある。`job.cancel()` は `dispatchWorkItem?.cancel()` + ワーカースレッド側の起動時 `cancellationSnapshot()` 再チェックに依存するが、`KxMiniRuntime.launch(workItem:)`（`Sources/Runtime/RuntimeHelpers.swift:306-308`）は cooperative な単一スレッドではなく `DispatchQueue.global()`（実 OS スレッドプール）へ dispatch するため、`launch { }` 直後に間を置かず `cancel()` を呼んでも、ワーカースレッドが `cancellationSnapshot()` チェックより先に本体（`runSuspendEntryLoopWithContinuation`）の実行を開始し得る。このガードは本体の中身に関わらず一律（try/finally 専用の分岐は存在しない）なので、`repeat(10) { ensureActive(); delay(1) }`（観測可能な副作用なし）ではレースが起きても出力に現れず見過ごされる一方、`try { delay(...) } finally { println(...) } ` のように finally に観測可能な副作用があると、レースに負けた場合だけ本来実行されないはずの finally が漏れて実行される。実 kotlinx.coroutines の `runBlocking` は cooperative/単一スレッドのため、この種のレースは構造的に発生しない。最小再現: `Scripts/diff_cases/coroutine_cancellation_advanced.kt` の job3/job4（`val job3 = launch { try { delay(Long.MAX_VALUE) } finally { println("finally ran") } }; job3.cancel(); job3.join()` — 参照 kotlinc は "finally ran" を出力しないが、高負荷環境下では kswiftc 側で出力されることがある。CI: kotlinc Diff Regression Shard (1/11), run 29348296913, job 87159766355）。発見元: PR #4700（`arrayOf` boxing 修正、本バグとは無関係）の CI 失敗調査中
