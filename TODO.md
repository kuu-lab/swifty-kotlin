...
- [x] RF-LOWER-CALL-012: Map HOFのsource-backed保護と残存rewriteを整理する（前提: CALL-011）
  ...
  - **前提からの逸脱**: CALL-007〜011 は未着手（CALL-001のみマージ、CALL-003が#6758で open）。CALL-007の専用policyモジュールは存在しないため、TODO冒頭の「着手時に既存の同責務ファイルがあれば再利用する」に従い、現行policyである `+CallRewrite.swift` の `shouldPreserveSourceBackedAggregateCall` と `+VirtualCallRewrite.swift` の `shouldPreserveSourceBackedVirtualCall` をそのまま対象とした。CALL-007〜011のPRは open していないので編集衝突はない。
  - 対象を Map 専用名6件に限定: `mapValues` / `mapValuesTo` / `mapKeys` / `mapKeysTo` / `filterKeys` / `filterValues`。List / Set / Sequence と共有する `map` / `filter` / `filterNot` / `forEach` / `flatMap` / `any` / `all` / `none` / `maxByOrNull` / `minByOrNull` は CALL-008/010/014 の担当なので、preserve列挙・rewrite・lookupのいずれも触っていない。
  - 解決先の実測（製品経路）:
    - source 注入（`--emit kir`）: 6件すべて `call mapValues symbol=mapValues` 等として保持され、`kk_map_{mapValues,mapKeys,filterKeys,filterValues,mapKeysTo,mapValuesTo}` は0件。解決先は `__bundled_kotlin/collections/MapHOF.kt` の非synthetic・externalLinkName無し・source-backed宣言で、`kotlin.collections.Map` 配下に synthetic overload は0件。
    - artifact（`--stdlib-library ...kklib`）: `nm` でも上記6名は0件。実行結果も正しい（重複key・挿入順・`mapKeysTo`/`mapValuesTo`・例外伝播）。
    - `--no-stdlib`: **stub は登録され、Map HOF は解決する**（当初 `mapOf("a" to 1)` で試して SEMA-0024 になったのは中置 `to` が未解決でレシーバがエラー型だったため。`mutableMapOf` / `emptyMap` / `HashMap` は `--no-stdlib` でも解決する）。ただしこの経路は変更前後どちらもリンク不能: KIR 上 `call mapValues symbol=_`（nil symbol）となり、変更前は preserve の `guard let symbol` が外れて rewrite が発火し `_kk_map_mapValues` が undefined、変更後は `_mapValues` が undefined になるだけで、到達可能な挙動は変わらない。`.runtime-build` のリンク対象 Runtime オブジェクトに6名の定義は1件も無い（`_kk_map_size` は定義あり、という対照で確認）。
    - これは CALL-012 の6名に限らない既存条件で、`HeaderHelpers+SyntheticMapStubs.swift` が参照する `kk_map_*` 22名のうち実在するのは `kk_map_size` / `kk_map_is_empty` の2名のみ（KSP-430 で runtime bridge が削除された結果）。`--no-stdlib` の Map HOF 表面全体の整合は CALL-015 / KSP-430 残作業の担当で、本PRでは扱わない。
  - rewriteが死んでいた二重の理由: (1) preserve が source-backed 宣言で先に短絡する。(2) `StdlibSurfaceSpec.mapHOFMembers` は KSP-430 で空配列になっており、`Sources/Runtime` には上記6名の `@_cdecl` が**存在しない**（定義は `Tests/RuntimeTests/RuntimeCollectionHOF430MapShims.swift` のテスト用シムのみ）。仮に発火しても未定義シンボルでリンクできない。加えて `+CallRewriteHandlers.swift` の `isCollectionHOFMemberName` と `+CallRewriteHOFCore.swift` の外側ゲートは `filterKeys` / `filterValues` を列挙していないため、その2件は内側の分岐自体に到達しなかった。
  - 削除したもの: `+CallRewriteHandlers.swift`（`isCollectionHOFMemberName` / `mapHOFRuntimeName` / `mapHOFReturnsMap`）、`+CallRewriteHOFCore.swift`（外側ゲート・map receiver分岐・結果タグ付け）、`+VirtualCallRewrite.swift`（`rewriteMapHOF` のguardと結果タグ付け）の Map 専用分岐と、両 preserve 列挙の Map 専用6名。孤立した lookup 6件（`kkMap{MapValues,MapKeys,FilterKeys,FilterValues}Name` と `map{Values,Keys}ToName`）も除去。
  - 分類の保持: `mapValues`/`mapKeys`/`filterKeys`/`filterValues` の結果を Map として扱う分類は rewrite ではなく `+PreScan.swift`（389-390 / 460-461行）が担っており、preserve 経路は `loweredBody.append(instruction)` でタグ付けしないため、これが唯一の担い手。よって同4名の lookup は PreScan 用に保持した。`mapValuesTo`/`mapKeysTo` は PreScan も参照していないので lookup ごと削除。
  - 意図的に残したもの: `RuntimeABISpec+Collection.swift:535-591` の6エントリと `Tests/RuntimeTests` のシム。`HeaderHelpers+SyntheticMapStubs.swift` が `externalLinkName` で同名を参照しており利用者ゼロではないため、TODO冒頭のABI削除条件（利用者ゼロ確認＋Spec/parity/リンク検証）を満たさない。`CallLowerer+MemberCallEmission.swift:788-789` の `kk_map_mapKeysTo`/`kk_map_mapValuesTo` も CallLowerer 側の別責務なので対象外。ABI export の増減なし。
  - テスト: `Tests/CompilerCoreTests/Lowering/MapHOFLoweringRoutingTests.swift`（5テスト: 6件がsource callとして生存 / 解決先が `MapHOF.kt` の source-backed 宣言 / synthetic overload 0件 / PreScan由来の Map 分類が `kk_map_size`・`__kk_map_get` として残る / ユーザー定義同名関数の非rewrite）。`Scripts/diff_cases/map_hof_key_collision.kt` を追加し、既存 `map_hof.kt` / `map_entries_hof.kt` に無かった非単射 `mapKeys` の重複key・挿入順・`mapKeysTo` の衝突・4種のlambda例外を kotlinc 突き合わせで固定。
  - ゲート（実施範囲）: `swift build` green。`MapHOFLoweringRoutingTests` は削除**前**のHEADで4/5 PASS（残1件は期待値を `kk_map_get` と書いた自テストの誤りで、実際は `__kk_map_get`）— rewrite が削除前から既に死んでいたことの直接証拠。修正・削除後は `MapHOFLoweringRoutingTests` / `BuilderDSLLoweringRoutingTests` / `CollectionLiteralLoweringTests` / `CollectionClassificationTests` / `CollectionRewriteStateTests` の5スイート89テストが全PASS。同一バッテリで source KIR・artifact の `nm`・実行 stdout がいずれも変更前とバイト一致（移動のみのPRの不変性条件）。`diff_kotlinc.sh` は map 系7ケース（`map_basic` / `map_hof` / `map_entries_hof` / `map_mapkeysto` / `map_mapvaluesto` / `stdlib_kotlin_collections_Map_map` / 新規 `map_hof_key_collision`）PASS。全 Swift テスト・全 Golden・全 `diff_kotlinc.sh`・`loc_report.sh` は未実行（2026-09-13 のユーザー指示により、変更範囲に限定した検証に留めた）。
  - 新規 diff ケースの初回 FAIL は `mapKeysTo` の destination 型を `MutableMap<Int, String>` と書いた自ケースの型エラー（`Map<String,Int>` + `String` 変換なので `MutableMap<in String, in Int>` が必要）。kotlinc も同じく拒否しており、コンパイラのバグではない。修正後PASS。
  - **CALL-011 申し送りの対応（2026-09-14、origin/master マージ後に追加）**: CALL-011 が本項へ振った2点を両方処理した。
    - (1) `+CallRewriteHOFCore.swift` の map receiver 条件で `flatMap` / `maxByOrNull` / `minByOrNull` が同一 `if` 内に二重列挙されていた重複を解消（単一の `||` 式内の重複なので意味は不変）。
    - (2) Map群の `maxByOrNull` / `minByOrNull` を、本項の6名と**同一の構造**で到達不能と確認して削除した。(a) `mapHOFRuntimeName` が `kk_map_{max,min}ByOrNull` へ写すが、その唯一の呼び出し元 `rewriteCollectionHOFCall` を囲む `isCollectionHOFMemberName` は両名を列挙しない。(b) `+CallRewriteHOFCore.swift` の map receiver 分岐も、外側ゲート（`rewriteCoreHigherOrderCollectionCall` 先頭の `if`）が両名を含まないため到達しない。(c) `StdlibSurfaceSpec` に行が無く `collectionHOFRuntimeName` は nil。(d) リンク対象 Runtime オブジェクトに `kk_map_maxByOrNull` / `kk_map_minByOrNull` の定義なし。(e) 解決先は `MapHOF.kt:618,640`。削除範囲は両 preserve 列挙・`mapHOFRuntimeName` の2 case・HOFCore の条件、および読み手を失った lookup 4件（`maxByOrNullName` / `minByOrNullName` / `kkMapMaxByOrNullName` / `kkMapMinByOrNullName`）。Lowering 配下に残存参照 0 件で、Sema 側（`CallTypeChecker+CollectionMemberFallback.swift` 等）の同名文字列は別責務のため未変更。
    - 回帰テストは CALL-011 が既に書いていた `ListSortExtremaLoweringRoutingTests.mapExtremaKeepTheirSourceCallAndNotTheMapRuntimeRewrite`（Map の byOrNull が source call として残り `kk_map_*` が出ないこと）が削除前後の両方を固定するので、新規テストは追加せずコメントのみ現状に合わせた。
    - 検証: `swift build` green。`maxByOrNull` / `minByOrNull` を Map / List / Set / Sequence の4レシーバ＋空入力＋同順位で並べたバッテリの lowering 後 KIR が変更前後で**バイト一致**。Map HOF 6名バッテリは callee 列が完全一致（生の差分はすべて `kk_lambda_NNNNN` と `SymbolID.rawValue` / 関数 ID の振り直しで、master 側の stdlib 宣言増減に由来）、artifact 経路の `nm` に Map HOF export 0件・stdout 一致。Core Lowering 8スイートPASS。
  - **副産物として発見したバグ（本PRの範囲外・別タスク切り出し済み）**: `setOf(3,1,2).maxByOrNull { it }` は**リンクできない**。Sema が Set レシーバの `maxByOrNull` を解決できず、`CallLowerer+UnresolvedMemberCalls.swift:183,185,293,295` の未解決メンバ fallback が `kk_list_maxByOrNull` / `kk_list_minByOrNull` を名前で emit するが、両者は spec-only で `Sources/Runtime` に `@_cdecl` が無いため undefined symbol になる。`List` / `Map` レシーバは影響なし。Set レシーバでも `sumOf` / `maxOfOrNull` / `minOfOrNull` は健全で、当たるのはこの2名のみ。emit 元が CallLowerer（本項は Lowering パス）なので同PRでは直さず別タスクへ。**「Runtime に定義が無い ⇒ rewrite は dead」は名前ゲート付き rewrite でのみ成立し、未解決メンバ fallback のように名前で無条件に emit する経路では「live かつ壊れている」**という反例。
