...
- [ ] BUG-046: `swiftpm-testing-helper` プロセス自体が signal code 10 (SIGBUS) で異常終了するクラッシュパターン。`CompilerCoreTests.LibraryMetadataSignatureParsingTests.testDeeplyNestedNullableSignatureDoesNotCrashAndEmitsWarning()`（`Q<Q<...I...>>` 1000段ネストの signature をわざと読ませ、クラッシュせず `KSWIFTK-LIB-0003` 警告1件になることを検証するテスト）実行直後に、`--parallel --num-workers 14`（デフォルト）/ `--num-workers 2` / `SWIFT_TEST_PARALLEL=0`（単独・直列実行）の3パターン全てで再現。同一クラッシュ（signal 10）は別の dead code 削除タスク対応中にも独立に観測されており、その際は `vm_stat` で空きメモリが約326MBまで逼迫（swap 未設定の64GB機）していたことを確認済みで、テスト対象や変更ファイルに依らない共有マシンのメモリ逼迫が真因と推定される。最小再現: `SWIFT_TEST_PARALLEL=0 bash Scripts/swift_test.sh --filter CompilerCoreTests.LibraryMetadataSignatureParsingTests`。発見元: `HeaderHelpers+SyntheticMetaprogAnnotationHelpers.swift` の無関係な dead code 削除タスク対応中の `CompilerCoreTests` 検証。変更ファイルとは無関係な subsystem（signature parsing vs. JVM annotation 登録）で、変更に直接関係する Annotation 系276件（`--filter "CompilerCoreTests.*Annotation"`）は同一環境下で全て green だったことを確認済み。対処: ワーカー数を絞っても解消しないため、高負荷時は変更に直接関係するテストサブセットへの絞り込みで代替し、フルスイート完走に固執しない
