...
- [~] CLEANUP-STUB-098: Function 型の fiction ほかを削除する — PR 作成中
  - 対象: `Function1.andThen` / `Function1.compose` / `Function2.curried`（`HeaderHelpers+SyntheticFunctionTypeStubs.swift` + `RuntimeFunctionTypes.swift`）を削除
  - 対象: `compareToOrNull`（`kotlin.comparisons` 未実装拡張、`HeaderHelpers+SyntheticComparableHelpers.swift`）を削除
  - 対象: `kotlin.jvm.isArrayOf`（`HeaderHelpers+SyntheticArrayStubs.swift`）を削除
  - 追随削除: `RuntimeABISpec+ABIParity.swift` から `kk_function_andThen` / `kk_function_compose` / `kk_function_curried` / `kk_array_isArrayOf` を削除。`ABIMismatchTests+RuntimeExportParity.swift` の `allowedSpecOnlyRuntimeABINames` から `kk_array_isArrayOf` を削除
  - テスト削除: `JvmArrayIsArrayOfSyntheticStubTests.swift` を削除
