...
- [ ] DEBT-RT-007: `Long.until(Long)` で作った `LongRange` を `in`（contains）チェックすると、`Sources/Runtime/RuntimeBoxing.swift:120` の `kk_unbox_long` が non-LongBox オブジェクトに対して呼ばれ、DEBUG ビルドで `KSwiftK warning [KSWIFTK-RUNTIME-0001]: kk_unbox_long called on non-LongBox object (0x...)` を stdout に出す（Boolean 自体の値は正しいが余計な行が混入する）。再現: `println(15L in 10L until 20L)`。`println(15L in 10L..20L)`（rangeTo）では発生しない — `kk_op_rangeTo` の `RuntimeRangeBox` 経路と `until` の経路が異なることが疑われる。`Scripts/diff_cases/random_overload_edge_cases.kt` の PR #4621 kotlinc-diff 対応中に発見。同ファイルは `10L until 20L` を同値の `10L..19L` へ書き換えて回避済み（Sources/CompilerCore の `until` lowering は `Sources/CompilerCore/Sema/DataFlow/HeaderHelpers+SyntheticRangeUntilStubs.swift` 付近を起点に調査）
...
> 2026-07-06 棚卸し（2026-07-09 に 005 を 15→18 へ更新: PR #4621 対応で `random_nextfloat_ranged_synthetic.kt` / `secure_random.kt` / `uuid_put_uuid.kt` を追加）: [`docs/diff-skip-inventory.md`](docs/diff-skip-inventory.md)。現時点の `DEBT-DIFF-*` タグ付き skip は 70 件（001:22 / 002:8 / 003:14 / 004:5 / 005:18 / 006:3）。各タスクの完了条件は、該当ケースを通常 `diff_kotlinc.sh` に戻すか、JVM kotlinc を oracle にできない理由と代替 runner / unit test owner を同文書へ移すこと。
