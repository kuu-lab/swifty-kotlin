...
最終更新: 2026-07-17（PR #4844 のマージコンフリクト解消時に master の BUG-039/040（`SWIFT_TEST_PARALLEL=0` 暫定緩和が exec() 引数長制限に抵触した件、PR #4854 で `full-swift-tests-runtime` 独立ジョブ化により恒久対応済み）を統合。無関係な coroutine diff flake の記録を BUG-041 として採番し直し。それ以前 2026-07-16: master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
