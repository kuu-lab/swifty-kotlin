...
最終更新: 2026-07-17（XCTest→Swift Testing 全件移行の検証中、Xcode 27 beta / SwiftPM の `Runtime-t.build` 出力を packaged Runtime object 探索が認識できない BUG-042 を追補。それ以前: master CI 失敗調査で BUG-039 の暫定緩和(PR #4846, `SWIFT_TEST_PARALLEL=0`)が Linux exec() 引数長制限に抵触し `Full Swift Tests (RuntimeTests)` を落とし続けていたことを確認、BUG-040 を追補し CI 側を修正。それ以前 2026-07-16: master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系などを扱う）
...
- [ ] BUG-042: Xcode 27 beta / Swift 6.4 の SwiftPM build が packaged Runtime object を `.runtime-build/.../Runtime-t.build/Objects-normal/<arch>/*.o` として生成する一方、`Sources/CompilerBackend/CodegenRuntimeSupport+RuntimeObjects.swift` は `Runtime.build` ディレクトリと `*.swift.o` だけを探索するため、実行ファイル生成が `Unable to locate packaged runtime object files` で失敗する。再現: `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer SWIFT_TEST_PARALLEL=0 bash Scripts/swift_test.sh --filter LinkPhaseIntegrationTests`（元の XCTest 版でも 16件中10件・23 failure）、同 `--filter DelegatePropertyKIRTests`（18件中2件 failure）。最小再現ケース: `Tests/CompilerBackendTests/Codegen/LinkPhaseIntegrationTests.swift` および `Tests/CompilerBackendTests/KIR/DelegatePropertyKIRTests.swift`。発見元: XCTest→Swift Testing 全件移行タスク（2026-07-17）。
