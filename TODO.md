...
- [x] KSP-CAP-018: object 式によるクラス継承を通す（= BUG-215）。ブロック対象だった KSP-441（object 式でパイプラインを表現する方針）は解消。2026-08-18 + 2026-09-13 に本項目の2症状（空ボディ含む）まで実装・検証済みで、2026-09-14 に実機再検証（`anon` / `7` を実際に出力すること、`diff_kotlinc.sh` PASS、バックエンド回帰16件 PASS を確認）の上で [x] 化。残る2件（object 式プロパティ delegation 未実装、名前付き `object : Base(x)` 宣言）は当初から本項目の完了条件外のため、BUG-259 / BUG-260 として独立起票（下記バグバックログ参照）。
  ...
  - **未解消（本項目の完了条件外。バグバックログへ独立起票）**: object 式のプロパティ delegation 未実装は BUG-259、名前付き（非リテラル）`object : Base(x) { ... }` 宣言のスーパークラス実引数破棄と virtual dispatch バグは BUG-260 を参照。
