...
- [ ] BUG-259: `object` のインスタンスプロパティが external ブリッジ（`get()`/`set()` が `private external fun` を呼ぶだけでバッキングフィールドを持たない）で実装されている場合、事前ビルド済み stdlib ライブラリ（`.kklib`）経由でコンパイル・リンクすると `kk_global_root_slot_<FQN>` シンボル未定義でリンク失敗する。最小再現: `@file:OptIn(kotlin.experimental.ExperimentalNativeApi::class)` の下で `import kotlin.native.Platform` し `fun main() { println(Platform.canAccessUnaligned) }` を既定オプション（`.build/debug/kswiftc repro.kt -o out`、キャッシュ済み stdlib artifact 使用）でコンパイルすると `Undefined symbols for architecture arm64: "_kk_global_root_slot_kotlin_native_Platform_canAccessUnaligned"` でリンク失敗する。`--stdlib-only --emit library` で自前ビルドした `.kklib` を `KSWIFTK_STDLIB_LIBRARY` で指定しても同じ症状（キャッシュの汚染ではなく機能自体の欠落と確認済み）。`--stdlib-from-source`（単一コンパイル単位でのソース注入）ではリンクは通るが、実行時に未初期化グローバルスロットを読むらしく SIGBUS（exit 138）でクラッシュする。原因: `Sources/CompilerBackend/NativeEmitter.swift` の `shouldEmitImportedGlobalReference(for:)` は `.importedLibrary` フラグを持つ任意のプロパティシンボルに対して `kk_global_root_slot_<FQN>` というグローバル変数参照を要求するが、getter のみ（バッキングフィールドなし）のプロパティは stdlib 側のコンパイル時にこのグローバル変数の定義を一切生成しない（`stableGlobalSlotName(for:)` 参照）ため、消費側の参照が unresolved になる。`kotlin.native.Platform` の全プロパティ（`isMemoryLeakCheckerActive`・`isDebugBinary` 等）と `kotlin.native.runtime.GC.MainThreadFinalizerProcessor` の全プロパティが同様に影響を受ける。発見元: KSP-1263（`GC.MainThreadFinalizerProcessor` の4プロパティ実装）の end-to-end 実行検証中に、無関係の既存機能 `Platform.canAccessUnaligned` でも同一症状が再現することを確認して発覚（KSP-1263 で新規導入した変更には起因しない）。今回修正しない理由: `NativeEmitter.swift`・kklib メタデータ（getter-only かどうかを表す bit の追加）・ライブラリ import 側の消費経路（getter-only import を global-root-slot 参照からゲッター呼び出しへ切り替える）にまたがる backend 横断の修正が必要で、KSP-1263 のスコープ（sema 層でのプロパティ型サーフェス追加）を大きく超える。ユーザー承認済みで別 PR に切り出す。
- [ ] BUG-260: `Outer.Inner`（`Inner` が `Outer` 直下にネストした `object`）を、代入のレシーバ（`Outer.Inner.prop = value`）またはスタンドアロンの値（`val x = Outer.Inner`）として使用すると、Sema の型チェックは通る（診断なし）が KIR lowering で `KSWIFTK-KIR-0003: KIR verifier: ... call to 'Inner' does not resolve to a module function, an external link name, or a runtime ABI function` エラーになる。最小再現: `@file:OptIn(kotlin.native.runtime.NativeRuntimeApi::class)` の下で `import kotlin.native.runtime.GC` し `fun main() { GC.MainThreadFinalizerProcessor.batchSize = 5uL }` をコンパイルすると上記エラーになる。同じ `import`/`OptIn` で `println(GC.MainThreadFinalizerProcessor.available)`（チェーンされた getter 読み取りのみ）はコンパイル・リンクまで成功する（実行は BUG-259 によりブロックされる）ため、失敗は「代入レシーバ・スタンドアロン参照としてのネストオブジェクト値materialization」に限定される。原因: KSP-1263 で `CallTypeChecker+MemberCallInferenceRegularNoCandidateFallbacks.swift` に追加した、`object` receiver 上のネストオーナー（`class`/`enumClass`/`object`/`annotationClass`）解決フォールバックは、既存の class-name-receiver 用ネストオーナー解決（`CallTypeChecker+MemberCallInferenceRegularResolution.swift`）と同じ Sema バインディング（`bindIdentifier`/`bindExprType` のみ）を行うが、後者は常に `TimeSource.Monotonic.markNow()` のように同一 memberCall 連鎖内でのみ使われてきた（`Outer.Inner` 単体の値化やその代入レシーバとしての使用は本 PR 以前に一度も Sema を通ったことがなく、lowering 側の対応も存在しない）。発見元: KSP-1263 で `GC.MainThreadFinalizerProcessor` を値としてアクセス可能にする Sema 修正を追加した際、getter のチェーン読み取りは動作する一方、`var` プロパティへの代入がこのエラーで失敗することが判明した（本 PR が新規に可能にした Sema 経路が原因のため、BUG-259 とは独立した別バグとして記録）。今回修正しない理由: KIR lowering / NativeEmitter 側でネストオブジェクトの値 materialization（代入レシーバ・スタンドアロン参照を含む）を補う変更が必要で、sema 層の型サーフェス追加という KSP-1263 のスコープを超える。ユーザー承認済みで別 PR に切り出す。
