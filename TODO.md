...
最終更新: 2026-07-17（Dead Code 削除タスク完了に伴い `CallLowerer+CollectionStdlibMemberCalls.swift` を削除、関連タスクの重複記載を解消。完了ゲート実行中に beta Xcode ツールチェーン固有の CompilerBackendTests 全滅要因を特定し BUG-042 として追補(master 側と ID 衝突したため BUG-041→BUG-042 に繰り下げ)。それ以前 2026-07-17: PR #4578 の CI 失敗調査で `launch{}` cancel-before-start レース(`kk_kxmini_launch`, `Sources/Runtime/RuntimeCoroutine.swift`)を確認、BUG-041 を追補。それ以前 2026-07-17: master CI 失敗調査で BUG-039 の暫定緩和(PR #4846, `SWIFT_TEST_PARALLEL=0`)が Linux exec() 引数長制限に抵触し `Full Swift Tests (RuntimeTests)` を落とし続けていたことを確認、BUG-040 を追補し CI 側を修正。それ以前 2026-07-16: master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [ ] CLEANUP-STUB-101: 監査で確定したデッドコードを一括削除する（`kk_math_round_mode` 系15（Sema 登録ゼロ・到達不能）/ `kotlin.experimental` Int 版登録一式（汎用特例が先に解決）/ `kk_check_not_null(_lazy)`/`kk_require_not_null(_lazy)` 4 / CLEANUP-STUB-084 取り残しの `registerSyntheticJvmAnnotationClass`・`registerSyntheticBooleanAnnotationPropertyAndConstructor` 2関数 / `kk_sequence_of_single` の生死判定。※`kk_system_measureTime*` 3 は KSP-617 と重複するため先行した方で実施。`CallLowerer+CollectionStdlibMemberCalls.swift` は Dead Code 削除タスク側で既にファイルごと削除済み（本タスクの対象外））
...
- [ ] BUG-042: ローカル環境（`DEVELOPER_DIR=/Applications/Xcode-beta.app`、`SDKROOT` が `MacOSX27.0.sdk` の beta ツールチェーン）で `bash Scripts/swift_test.sh`（フルスイート）を実行すると、`CompilerBackendTests` に属する実行可能ケース（Codegen/Link 系）が例外なく全滅する（今回の実測: 935/935 件失敗、他の4スイート — CompilerCoreTests/RuntimeTests/LSPServerTests/KSwiftKCLITests — は0件失敗）。エラーは共通して `KSWIFTK-LINK-0001: Link step failed: Unable to locate packaged runtime object files under .../.runtime-build/runtime-nocov-v2-<hash>/<triple>/debug/Runtime.build.`。根本原因: `Sources/CompilerBackend/CodegenRuntimeSupport+RuntimeObjects.swift` の `discoverRuntimeObjectPaths`/`collectObjectPaths` は `swift build --target Runtime --scratch-path .runtime-build/...` の出力を「`debug/Runtime.build/` 直下 or 再帰探索で見つかる `Runtime.build` という名前のディレクトリに `*.swift.o` が入っている」という**旧来 SwiftPM（llbuild）レイアウト**前提で探索するが、この beta Xcode の `swift build` は実際には `.runtime-build/.../out/Intermediates.noindex/KSwiftK.build/Debug/Runtime-t.build/Objects-normal/arm64/*.o` という**新 Swift Build（Xcode 統合ビルドシステム）レイアウト**（ディレクトリ名も `Runtime.build` ではなく `Runtime-t.build`）で出力するため、探索が空振りし `runtimeObjectsUnavailable` を投げる。最小再現: 当該 beta ツールチェーンで `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer bash Scripts/swift_test.sh --filter CompilerBackendTests` を実行し、`.runtime-build/` 配下に `Runtime.build` ではなく `Runtime-t.build` が生成されていることを確認する。CI（安定版 Xcode 想定）や旧来ビルドシステムのローカル環境では再現しない可能性が高い — 未検証。発見元: Dead Code 削除タスク(`CallLowerer+CollectionStdlibMemberCalls.swift` の 1,336 行 orphan legacy entrypoint 削除)（本タスクの完了確認のためのフルテストゲート実行中に検出。対象削除ファイルは CompilerCore/KIR のみで CompilerBackend/Runtime とは無関係であることをコード上・CompilerCoreTests 全件通過の両面から確認済みで、本バグは無関係な既存環境要因と判断）。修正方針(未実施): `swift build --build-system` 相当の明示指定でレイアウトを固定するか、`discoverRuntimeObjectPaths` を新旧両レイアウト（`Runtime.build`/`Runtime-t.build`、`*.swift.o`/`Objects-normal/**/*.o`）に対応させる。
