...
最終更新: 2026-07-17（PR #4844 のマージコンフリクト再解消時に master の BUG-041（`launch{}` cancel-before-start レース、PR #4578 の CI 診断で発見）を統合。無関係な coroutine diff flake の記録を BUG-042 として採番し直し。それ以前: master の BUG-039/040（`SWIFT_TEST_PARALLEL=0` 暫定緩和が exec() 引数長制限に抵触した件、PR #4854 で `full-swift-tests-runtime` 独立ジョブ化により恒久対応済み）を統合。それ以前 2026-07-16: master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [ ] BUG-042: `Scripts/diff_cases/coroutine_cancellation_advanced.kt`（`kotlinc Diff Regression` CI ジョブ）が非決定的に stdout mismatch で失敗することがある。症状: 4番目の `job4`（`try { delay(Long.MAX_VALUE) } finally { withContext(NonCancellable) { delay(1); println("cleanup done") } }` を cancel）由来の `"cleanup done"` 行が、kswiftc（candidate）側の出力にのみ現れ kotlinc/JVM（reference）側には現れない（`job2 cancelled: true` の直後・`done` の直前の3行 vs 2行の diff）。再現: `Scripts/diff_cases/coroutine_cancellation_advanced.kt` を `Scripts/diff_kotlinc.sh` で実行（発生ジョブ: [kotlinc Diff Regression Shard (2/11)](https://github.com/kuu-lab/swifty-kotlin/actions/runs/29438909053/job/87458007052)）。PR #4844（DEBT-KIR-004 の Base64.kt operand-order workaround 除去のみ）の CI で発見したが、当該 PR の diff とは無関係（コルーチン関連コードは一切変更していない）。master 直近4回の同ジョブはいずれも成功しており低頻度の再現と見られる。cancellation 由来のタイミング依存バグは本ファイルで過去にも複数回発生・修正されている（`docs/diff-skip-inventory.md` の DEBT-DIFF-003 参照）ため、cleanup（`NonCancellable` context 内の処理）が cancel 後の後始末レースにより実行タイミングがズレる、または JVM 側で実行順序が異なる可能性がある。BUG-041（`kk_kxmini_launch` の起動-キャンセル競合）と根の部分で類似だが、対象は `withContext(NonCancellable)` 経由のクリーンアップ実行タイミングであり別事象。**再検証（2026-07-17、同PRの再プッシュ後CI run 29507739056）: `kotlinc Diff Regression Shard (2/11)` は success で再現せず**、非決定性の傍証がさらに増えた。ローカルでの直接再現は本 PR 作業時に使用した worktree の `.build` がリンク時に `_kk_*` runtime シンボル未解決で失敗する別問題（Runtime 側の環境不整合の疑い、本バグとは無関係。後に `swift build` のデフォルト build system が `swiftbuild` に変わっていたことが原因と判明し `--build-system native` で解消）に阻まれ未実施 — 発見元: PR #4844 (claude/bitwise-operator-bug-870025) の CI failure 調査
