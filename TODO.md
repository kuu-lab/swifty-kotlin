...
最終更新: 2026-07-16（master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。CI 側に暫定緩和を適用 — 詳細は BUG-039。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [ ] BUG-039: master CI（`Full Swift Tests (RuntimeTests)` ジョブ）が2026-07-15〜16に直近4回中3回失敗。原因は BUG-021/023/024/028 系と同根で、`kk_runtime_force_reset()` 等が書き換える `Sources/Runtime/RuntimeGC.swift` のプロセス全体共有ハンドルテーブル（`runtimeStorage.objectPointers` 等）に対し、XCTest 時代の `RuntimeTestIsolationSupport.swift`（`gcSemaphore` 等のプロセス全体セマフォ、`IsolatedRuntimeXCTestCase.setUp/tearDown` 経由）が提供していたクロスファイル排他が、Swift Testing の `@Suite` では構造的に再現できない（`XCTestCase` サブクラス専用の仕組みで Swift Testing の struct suite からは使えない）。`@Suite(.serialized)` はスイート内のみを直列化し、他ファイルの並行スイートとは排他されないため、あるスイートの reset が別スイートの生存中ハンドルを消し飛ばし、`invalid range/array/string handle` パニックや `Set`/`Array` 内容破損（解放先アドレスの再利用によるものと推定）を引き起こす。対象確認: `Tests/RuntimeTests/RuntimeSynchronizedTests.swift`（PR #4823 で移行済み・`init()` と各 `@Test` の `defer` で `kk_runtime_force_reset()` を正しく呼び `.serialized` も付与済みだが、cross-suite 排他が無いため依然として攻撃源になり得ることを示す新規ケース。BUG-020〜035 では未記載）。既存の BUG-023/024/028 も現行 master 上で確認済みの同根の実攻撃源（詳細は各項目参照）。最小再現: `DEVELOPER_DIR=/Applications/Xcode-beta.app SWIFT_TEST_WORKERS=4 bash Scripts/swift_test.sh --filter '^RuntimeTests\.' -Xswiftc -swift-version -Xswiftc 6` をローカルで数回実行（`SWIFT_TEST_PARALLEL` 未設定＝デフォルト `--parallel --num-workers 4`）。発見元: 本 master CI 失敗調査タスク（2026-07-16）。暫定対応: `.github/workflows/ci.yml` の `full-swift-tests` ジョブで `RuntimeTests` マトリクスエントリのみ `SWIFT_TEST_PARALLEL: "0"` を設定し直列実行に固定（実測 196 tests / 0.235 秒、タイムアウト無関係）。**PR #4843 側でさらに判明した E2BIG 問題（BUG-041 参照）を受け、RuntimeTests は専用ジョブ + `shard_swift_tests.sh --mode static` によるチャンク実行へ置き換え済み（本ファイルの `full-swift-tests-runtime` ジョブ）。根本修正（Swift Testing 向けクロススイート排他機構の再構築、または全対象ファイルの個別是正）は未着手**。暫定緩和 PR: #4846
- [ ] BUG-040: `CodegenRuntimeSupport+RuntimeObjects.swift` の `discoverRuntimeObjectPaths` が、新しい XCBuild ベースの swiftpm ビルドレイアウト（`out/Intermediates.noindex/KSwiftK.build/Debug/Runtime-t.build/Objects-normal/<arch>/*.o`）を認識できず、`kswiftc` の LinkPhase が `KSWIFTK-LINK-0001: Unable to locate packaged runtime object files` で失敗する（XCTest 経由では `LinkPhase.swift:109` で `caught error: "outputUnavailable"`）。当該コードは `<scratch>/<triple>/debug/Runtime.build/*.swift.o` という旧レイアウト（ファイル名末尾 `.swift.o`、ディレクトリ名 `Runtime.build`）のみを探索しており、新レイアウトはディレクトリ名が `Runtime-t.build`、ファイル拡張子が単なる `.o` になっている。再現: Xcode 26 beta 系トールチェイン（`swift-tools-version` 6.2 / XCBuild 統合ビルドシステムを使う環境）で `bash Scripts/swift_test.sh --filter CodegenBackendIntegrationTests/testPrimitiveArrayZeroInit` を実行する、または `.build/debug/kswiftc <any>.kt -o out` で任意のコードをコンパイルする（`for`ループの有無を問わず、`LinkPhase` を経由する実行系テスト全般が影響を受ける）。発見元: DEBT-KIR-005（配列 for-in ループ修正）の検証中、Command Line Tools 単体ではビルド不可（`XCTest` module 解決不可）だったため `DEVELOPER_DIR=/Applications/Xcode-beta.app/...` を設定したところ本事象が判明。回避策: `discoverPackageBuildRuntimeObjectPaths`/`discoverScratchRuntimeObjectPaths` の探索パターンに新レイアウト（`Intermediates.noindex/**/Objects-normal/**/*.o`）を追加する
- [ ] BUG-041: PR #4775（XCTest→Swift Testing移行）で `@Suite` に `.serialized` が付与されておらず、デフォルト並列実行下で `RuntimeURITests` が非決定的に失敗する（`testURIParsesAndNormalizesComponents` で `stringValue(kk_uri_authority(uriRaw))` が期待値 `"example.com"` の代わりに空文字列を返す等、失敗するテスト・箇所が実行のたびに変わる）。移行前の XCTestCase 版にも `tearDown` は存在せず BUG-021 系（`kk_runtime_force_reset()` 消失）とは別系統 — BUG-028 系（`.serialized` 欠落）に近いが、URI パースが触れる共有状態（インターン文字列テーブル等）がそもそもスレッドセーフでない可能性も未切り分け。対象 `Tests/RuntimeTests/RuntimeURITests.swift`。再現: master 上で複数コミットにわたり `Full Swift Tests (RuntimeTests)` が非決定的に失敗（例: commit `39068a0c` 時点の Actions run 29405211032。同様に `RuntimeSetCollectionHOFTests`=BUG-024 も別コミットで単独失敗しており、両者は独立に発現する）。発見元: PR #4843 の CI 失敗（`Full Swift Tests (RuntimeTests)` の `--no-parallel` 化に伴う "Argument list too long" エラー）調査中、その直列化の動機を裏付けるため master の直近 CI 履歴を確認する過程で判明
