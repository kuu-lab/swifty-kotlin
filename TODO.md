...
- [ ] BUG-155: 明示的に nullable な型引数（または nullable な期待型）を持つ generic vararg 関数呼び出し（`arrayOf<T?>(...)` 等）で、非 null 実引数が1つでもあると型推論が型変数を上限（`T?`）へクランプせず共通上位型 `Any?` に推論し `KSWIFTK-TYPE-0001: Conflicting bounds for type variable #0: inferred Any? is not a subtype of T?` で失敗する。原因は制約ソルバが lower bounds（非 null 実引数由来の `String` 等）から解を組み立てる際に、明示型引数／期待型由来の upper bound（`String?`）を解のクランプに使わないこと。最小再現:
  ```kotlin
  fun main() {
      val a = arrayOf<String?>("p", null)   // error（null を除いた arrayOf<String?>("p") でも同一エラー）
      val b: Array<Int?> = arrayOf(1, null) // error（期待型経由の upper bound でも再現）
...
- [ ] BUG-156: `Array<T>.any { ... }` および `Array<T>.none { ... }`（述語ラムダ版）を Linux でコンパイル・実行すると、ラムダ本体呼び出し時に `Signal 11: Bad pointer dereference at 0x1`（不正なクロージャコンテキストポインタ）でクラッシュする。同じ `runtimeInvokeCollectionLambda1` を使う `Array<T>.all { }`/`count { }`/引数なし `any()` および `List<T>.any { }` は正常に動作するため、`kk_array_any`/`kk_array_none`（`Sources/Runtime/RuntimeCollectionHOFArray.swift`）呼び出し側のクロージャ引数の lowering/codegen 固有の miscompile と推定（runtime 関数自体は `kk_array_map`/`filter` と同一構造で健全）。最小再現:
  ```kotlin
  fun main() {
      val a = arrayOf(1, 2, 3)
      println(a.any { it > 2 })   // crash (Signal 11)
      println(a.none { it > 5 })  // crash (Signal 11)
      // a.all { it > 0 } / a.count { it > 1 } / a.any() / listOf(1,2,3).any { it > 2 } はいずれも正常
  }
  ```
  既存テスト `CompilerBackendTests.CodegenBackendIntegrationTests/testCodegenArrayAnyAllNoneCount` がこのクラッシュで失敗する。発見元: KSP-658 の完了ゲート `swift_test.sh` 実行中に検出。KSP-658 の変更を含まない clean master（同一 Linux/Swift ツールチェーン）を別 worktree でビルドしても同一クラッシュが再現するため本 PR の変更由来ではない（CI の CompilerBackendTests は self-hosted ランナーで実行されるため CI では顕在化しない可能性がある）。今回修正できない理由: Array HOF の述語ラムダ渡しの lowering/codegen 経路の調査と修正が必要で、KSP-658（contentEquals/contentToString/copyOf/copyOfRange の stdlib 移行）のスコープと安全な小修正の範囲を超える
