...
- [x] DEADCODE-CORE-038: [W0] 同ファイル `:749` の `kkPathNewName` と initializer `:1540` の代入を削除する
...
- [ ] BUG-041: `launch { }` の子コルーチン本体が、直後（介在する suspension point なし）の同期的な `cancel()` より先に実行を開始してしまうことがある、`kk_kxmini_launch`（`Sources/Runtime/RuntimeCoroutine.swift`）の起動-キャンセル競合。原因: `KxMiniRuntime.launch(workItem:)`（`Sources/Runtime/RuntimeHelpers.swift:306-308`）が cooperative な単一スレッドではなく `DispatchQueue.global()`（実 OS スレッドプール）へ即座に dispatch するため、子スレッド側 `job.markStarted()` と親スレッド側 `job.cancel()`（`dispatchWorkItem?.cancel()` + `NSLock` 経由の `cancellationSnapshot()` 再チェック）の獲得順序に確定的な happens-before 保証がない。本家 kotlinx.coroutines の `runBlocking` 直下の `launch` は単一スレッドの協調的 event loop で動くため、suspension point を跨がない `cancel()` は子の実行開始を確実に防げるが、本実装は実スレッド並行に依存しており、子スレッドが `markStarted()`→`cancellationSnapshot()` チェックを親の `cancel()` より先に通過すると本体（`runSuspendEntryLoopWithContinuation`）が走り出す。このガードは本体の中身に関わらず一律（try/finally 専用の分岐は存在しない）なので、`repeat(10) { ensureActive(); delay(1) }`（観測可能な副作用なし）ではレースが起きても出力に現れず見過ごされる一方、`try { delay(...) } finally { println(...) } ` のように finally に観測可能な副作用があると、レースに負けた場合だけ本来実行されないはずの finally が漏れて実行される。
  - 再現1: `Scripts/diff_cases/coroutine_cancellation_advanced.kt` の job3/job4（`val job3 = launch { try { delay(Long.MAX_VALUE) } finally { println("finally ran") } }; job3.cancel(); job3.join()` — 参照 kotlinc は "finally ran" を出力しないが、高負荷環境下では kswiftc 側で出力されることがある。CI: kotlinc Diff Regression Shard (1/11), run 29348296913, job 87159766355）。発見元: PR #4700（`arrayOf` boxing 修正、本バグとは無関係）の CI 失敗調査中
  - 再現2: `Scripts/diff_cases/coroutine_cancellation_edge_cases.kt` / `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+CoroutineCancellationEdgeCases.swift` の `testCodegenCompilesCoroutineCancellationEdgeCases`（PR #4830 で追加）。CI高負荷時に発現（本件は PR #4578 の `CompilerBackendTests shard 3/6`、`Actions run 29439186351/job 87460299566` で観測）。頻度は未確定 — ローカルでの繰り返し再現を試みたが、本開発機で他セッションの並行ビルドが多数走っていたことによるリソース競合とみられる無関係な `LinkPhase.swift:109` の `"outputUnavailable"` エラーに阻まれ、対象のレース自体をクリーンに再現できなかった（1回だけ `cancelJob.join()` 待ちとみられる無限待機/0%CPU sleepingも観測したが、上記のリソース競合下での観測のため本バグと確定できない）。低負荷環境での再現率は改めて要検証。発見元: PR #4578 の CI 診断中（#4830 自体の変更ではなく `kk_kxmini_launch` の既存 dispatch モデルに起因する既存バグ）
  - 番号について: 本項目は PR #4700 と PR #4578 の2セッションが互いに無関係に独立発見し、当初どちらも別々に番号を採番していた（PR #4578側は当初 BUG-039 だったが、その後 master 側で BUG-039/040 が全く別内容（RuntimeTests CI の cross-suite GC race / exec引数長制限）で確定したため BUG-041 に採番し直された）。マージ時に両者が同一の根本原因（`kk_kxmini_launch` の起動-キャンセル競合）を指していることを確認し、本エントリへ統合した
...
- [ ] BUG-045: `kk_op_add`/`kk_op_sub`/`kk_op_mul`/`kk_op_div`/`kk_op_mod`（`Sources/CompilerBackend/NativeEmitter+EmissionConstants.swift`、`buildAdd`/`buildSub`/`buildMul`等）が要素型に関係なく常に整数 LLVM 命令を emit するため、`DoubleArray`/`FloatArray`（および `Array<Double>`/`Array<Float>`）の indexed compound-assign（`arr[i] += x` 等）が整数演算の結果を返し破損する。最小再現: `val d = doubleArrayOf(1.5, 2.5); d[0] += 1.0; println(d[0])` — 期待値 `2.5` に対し `1.348269851146737E308` のような無関係な値を出力。発見元: PR #4700（`arrayOf` boxing 修正）の PR 本文で「別の pre-existing バグ、スコープ外」として言及されていたが TODO.md 未記載だったため、PR #4700 の Devin レビュー対応中に再現・追記（採番: 当初 BUG-042 だったが、master 側で PR #4572 が同番号を StringBuilder 関連の別バグに使用したため BUG-045 に採番し直し）。2026-07-19 修正: `CallLowerer+Operators.swift` が receiver の `Double`/`Float` 要素型に応じて `kk_op_dadd`〜`kk_op_dmod` / `kk_op_fadd`〜`kk_op_fmod` を選択するよう変更。`BoxingIntegrationTests.testCompoundAssignOnFloatingPointArrayUsesFloatingPointRuntimeOps` で generic/primitive 両配列を検証し、`Scripts/diff_cases/indexed_compound_floating_array.kt` の `diff_kotlinc.sh` が pass。修正 PR: #4700（マージ後に `[x]` 化）
- [ ] BUG-046: `Array<*>` を明示的に `as Array<Any?>`（unchecked cast）してから要素へ代入する（`(a as Array<Any?>)[0] = v` / `val b = a as Array<Any?>; b[0] = v`）と、コンパイルは通る（`KSWIFTK-SEMA-UNCHECKED-CAST` 警告のみ）が実行時に `KSwiftK panic [KSWIFTK-LINK-0003]: Unhandled top-level exception` で異常終了する。star projection のまま（cast を挟まない）`Array<*>` への代入 `a[0] = v` は正常動作するため、cast 経由の代入パスに固有の欠陥と思われる。最小再現: `fun assign(a: Array<*>) { val b = a as Array<Any?>; b[0] = 42 }; fun main() { val arr: Array<Int> = arrayOf(1, 2, 3); assign(arr); println(arr[0]) }`。発見元: PR #4700（`arrayOf` boxing 修正）の Devin レビュー対応中、indexed compound-assign の star-projection 検証で偶然発見（採番: 当初 BUG-043 だったが、master 側で PR #4572 が同番号を StringBuilder 関連の別バグに使用したため BUG-046 に採番し直し）。2026-07-19 修正: generic `Array` の同一 raw class 間 cast は実行時に型引数を検査せず operand をそのまま返し、generic 型 `>` の後で次の文を結合しない parser 境界修正を追加。`BoxingIntegrationTests.testGenericArrayCastWriteUsesArrayRuntimeSetter` で `kk_array_set`/boxing と cast no-op を検証し、`Scripts/diff_cases/array_star_projection_cast_write.kt` の `diff_kotlinc.sh` が pass。修正 PR: #4700（マージ後に `[x]` 化）
