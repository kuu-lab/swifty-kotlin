...
- [x] KSP-700: core collection / iterable / Comparable / List interface shells を Kotlin 化し、旧 synthetic shell 登録を residual 責務へ分離する
  ...
  - **2026-09-15 実装メモ**: 着手時に前提・対象の実態を rg で再固定したところ、issue 記載と現コードの乖離が大きいと判明した。
    ...
    3. **スコープの大半は既に他タスクで充足済み**: `Comparable.kt`（KSP-797）・`Iterable.kt`/`MutableCollection.kt`（KSP-697/KSP-943 系）・`List.kt`/`AbstractList.kt`（KSP-697 の shell のみ）・`Collection` interface の全メンバ（KSP-934、`AbstractCollection.kt` 内に実装済み）・`MutableIterable`/`ListIterator`/`MutableListIterator`（KSP-633/943/945/1064）が、本 issue 起票後の別タスクで先行完了していた。新設 `Collection.kt` は `AbstractCollection.kt` 内の既存宣言（`<out E>`、KSP-1542 の前方制約どおり）で満たされているため見送り（KSP-1541 のファイル統合スコープとの重複を避けた）。`Iterable.iterator()`/`Collection`/`MutableCollection` の残余メンバ登録（`HeaderHelpers+SyntheticCollectionTypeFallbacks.swift`）は KSP-1542（`[~]`、2026-09-13 監査で (c) 確定済み）が明示的に担当領域として引き取っている。
    4. **実施した変更**: `List<E>` に `operator fun get(index: Int): E` を source 化（`@KsSymbolName("__kk_list_get")` の external 宣言。既存ブリッジ名は変更なし）。`HeaderHelpers+SyntheticListResiduals.registerListGetOperator` は `bundledIndex` ガード付きの `--no-stdlib`/precompiled-metadata フォールバックへ縮小（削除はしない — KSP-1509/1542 が同フォールバックを維持した前例に合わせた）。呼び出し元のいない dead 関数 3 件（`registerListSetOperationMembers`＝KSP-428 で呼び出し元削除済み、`registerListToMapMember`＝KSP-429 で本体削除済みの空関数、`registerListAsSequenceMember`＝呼び出し元なし）と、実利用箇所が無い `List.contentEquals` member（`registerListContentEqualsMember`、kotlin-stdlib に存在しない API で fiction 判定）を削除。
    5. **意図的に見送った範囲（(c) 残置と判断）**: `listIterator()`/`listIterator(index)` は MutableList 側の共変オーバーライド（`MutableListIterator` 返却）とペアで、MutableList 自体が未移行（KSP-1503/705）のため片側だけ source 化すると `MutableIterable.iterator()` で確認済みの BUG-200（precompiled metadata が再型付けを表現できない）を再現する懸念があり見送った。`isEmpty()` は `kk_list_is_empty` が `CallLowerer+UnresolvedMemberCalls.swift` と `CollectionLiteralLoweringPass+LookupTables+List.swift` の2箇所で Sema メンバーシンボルと独立に name-string 定数として直接参照されており、`Collection.isEmpty` 継承へ寄せる変更はこの2経路の監査なしには安全に行えないため見送った。`Comparable` の primitive conformances（`setupPrimitiveComparableImplementations`）は Int/Double 等がコンパイラ組込プリミティブで宣言可能な supertype list を持たないため (c) 恒久残留、`registerOpenEndRangeComparableUpperBound`/`patchSyntheticClosedRangeTypeParameterUpperBound` は Range モジュール側（KSP-714）の担当。
    6. **副産物のバグ報告（2件）**: (a) `CallLowerer+UnresolvedMemberCalls.swift` の `isEmpty` 解決に、型パラメータ経由（`__kk_collection_isEmpty`、List/Set 両対応）と具象クラスシンボル経由（`.collection?` を `kk_list_is_empty` へ、List 専用で Set には無条件 `true` を返す）で不整合な分岐を発見したが、短時間では到達可能な最小再現が作れなかったため KUU-543 に起票（KSP-700 スコープ外）。(b) `AbstractList` を継承したユーザークラスが独自 `iterator()` 内の無名 `object : Iterator<E>` から `get(i)` をレシーバなしで呼ぶ動作確認中、リンクエラー（`Undefined symbols: "_get"`）を発見。`List`/`AbstractList`/stdlib と無関係な純粋ユーザー定義 interface（`interface Getter<E> { fun fetch(index: Int): E }` を継承したクラス内の無名 `object` から `fetch(0)` をレシーバなし呼び出し）でも同一症状で再現することを確認し、KSP-700 起因ではなく既存の Sema/KIR 側（無名クラスからの暗黙外側レシーバ解決）の一般的なギャップと判断、KUU-544 に起票した（ワークアラウンド: 外側インスタンスをローカル変数にキャプチャしレシーバ付きで呼べば回避可能、と確認済み）。本 PR の diff ケース（`ksp700_list_get_source_backed.kt`）はこのパターンを含まないため無関係。
...
- [~] KSP-703: Map shell / HOF を Kotlin 化し `HeaderHelpers+SyntheticMapStubs.swift` を削除する
  ...
  - 前提: KSP-700, KSP-701
  - **2026-09-15 実装メモ**: 着手時に前提を確認した。KSP-701 は完了済み（PR #5915/#5990）。KSP-700 は未着手だが、KSP-704/KSP-1542 の前例（前提未達のまま個別シェルを先行させて問題ない）に倣い着手した。着手時点で `Map`/`MutableMap`/`HashMap` は KSP-928/930/946/1055/1067/1076 により既に大部分が bundled Kotlin source 化済みで、チケット記載の「新設」は stale だった（`Map/Map.kt`/`MutableMap.kt`/`HashMap/HashMap.kt` は既存）。実際に残っていたのは以下。
    1. `HeaderHelpers+SyntheticMapStubs.swift` の `registerMapHigherOrderMembers` 内、`forEach`/`map`/`mapNotNull`/`mapValues`/`mapKeys`/`mapKeysTo`/`mapValuesTo`/`filter`/`filterNot`/`filterKeys`/`filterValues`/`count`(2種)/`any`/`all`/`none`/`plus`/`minus`/`flatMap`/`maxByOrNull`/`minByOrNull` の合成登録は、すべて `bundledIndex.contains(...)` による無条件早期 return で完全に到達不能（`MapHOF.kt`/`MapLookupAndTransform.kt` が全メンバを既に提供）と確認し削除した。直前の RF-LOWER-CALL-012（#6817）が Lowering 側で同一の結論に達していたことと整合する。同様に `registerSyntheticMapEntryStub` の `component1`/`component2`（KSP-961 で `Entry.kt` に移行済み）、`registerSyntheticMutableMapEntryStub` の `setValue`（KSP-1076 で `MutableEntry.kt` に移行済み）も削除した。
    2. `Map.isEmpty`/`Map.get`（いずれも `.function`）を KSP-704 の `Set.kt` 前例に倣い `Map/Map.kt` へ `@KsSymbolName` 付き body なし override として直接ソース化し、対応する Swift 側登録（`registerSyntheticMapStub` の `get`、`registerMapHigherOrderMembers` の `isEmpty`）を削除した。externalLinkName の値（`kk_map_is_empty`/`__kk_map_get`）はリネームせず維持したため、既存の `MapAsSourceMigrationTests` はそのまま green。
    3. `MutableMap.remove`/`MutableMap.clear` も同様に `MutableMap.kt` へ `@KsSymbolName`（`__kk_mutable_map_remove`/`__kk_mutable_map_clear`）で直接ソース化した。KSP-704 が `MutableSet` の変異系メンバで踏んだ「interface のボディなしメンバは一律 abstract 化され、オーバーライドせず継承に頼る具象クラスを壊す」制約は、`HashMap`/`AbstractMutableMap` が `remove`/`clear` を両方とも自前で override 済みのため該当せず、`swift build` green・`MutableMapInterfaceSourceMigrationTests` green で実証した。
    4. **意図的に残置/未着手**（KSP-704 に倣い部分完了として記録）:
       - `size`/`keys`/`values`/`entries`（Map の残り4プロパティ）: `.property` シンボルへの `@KsSymbolName` は `HeaderHelpers.swift` の `registerAnnotations` が未対応（`.function`/`.constructor` のみ）。Swift 側 `registerPropertyMember` に残置。
       - `MutableMap.put`: `.throwingFunction` ABI フラグが `@KsSymbolName` 経由の interface メンバでも正しく伝播するかを確認できず、壊れ方がビルド失敗ではなく実行時の例外消失という発見しにくい形になるリスク（BUG-257 系）があるため見送り、Swift 側登録を維持。
       - `MutableMap.putAll`: `BundledDeclarationIndex.isRuntimeBackedSyntheticRetainedOverlap` に本家 Kotlin 準拠の意図的な overload 衝突としてホワイトリスト登録されており単純な dead code ではないため、Swift 側登録を維持。
       - `Map.Entry`/`MutableMap.MutableEntry` の `key`/`value`・interface 宣言自体: 対応する bundled Kotlin の interface 宣言が存在しない（`Entry.kt`/`MutableEntry.kt` は拡張関数のみ）ため、Swift 側 shell 登録を維持。
       - `LinkedHashMap` を `LinkedHashSet` 同様の concrete class へ昇格する構造変更はスコープ外。**2026-09-15 訂正**: 本項は当初「本家は `typealias HashMap = LinkedHashMap`、実体は `LinkedHashMap`」と記載していたが誤り（kotlin-native と diff オラクルを混同していた）。実際には diff オラクルの `kotlinc-jvm` では `HashMap`/`LinkedHashMap` は java.util の別クラスで `LinkedHashMap extends HashMap`（実測: `HashMap() is LinkedHashMap<*, *>` は false、`LinkedHashMap() is HashMap<*, *>` は true）。kotlin-native は逆に `HashMap` が `LinkedHashMap` への typealias で同一型になるが diff オラクルには使われない。正しい follow-up 方針は typealias 化ではなく `class LinkedHashMap<K, V> : HashMap<K, V>`（JVM 参照形の一方向継承を再現、`linkedHashMapRuntimeTypeID` 新設 + Sema/Lowering 拡張を伴う）。`docs/stdlib-pipeline.md` §13-8 構造逸脱台帳を訂正済み。**2026-09-16 KUU-556 で解消**: `LinkedHashMap` を `HashMap` の concrete subclass へ昇格した。副産物として2つのバグを発見・修正: (1) `isMapLikeSymbol`/`isMutableMapSymbol`（`CompilerKnownNames.swift`）が `LinkedHashMap` を認識しておらず（typealias 時代は `MutableMap` のシンボルへ直接解決されていたため無症状だった）、Map HOF のラムダ型推論等が壊れる箇所を修正。(2) `HashMap` が初めて subtype を持つことで、`HashMap` 自身が保持する `keys`/`values`/`entries`/`size`（Map interface 由来の abstract realization、`@KsSymbolName` が `.property` 未対応のため external link を持たない）が `CallLowerer+MemberPropertyReads.swift` の KSP-928/BUG-227 系 vtable dispatch 判定に誤って合致するようになり、`HashMap`/`LinkedHashMap` 型レシーバの `keys` 等アクセスが `KSWIFTK-RUNTIME-0001`（vtable lookup failed）で panic する回帰を作り込んでいたため、HashSet の `size`/`isEmpty` 特別扱いと同じ形で除外を追加した。
       - `BundledDeclarationIndex.isRuntimeBackedSyntheticRetainedOverlap` の `MutableMap.remove`/`Map.get` エントリ（arity 1）: 本PRでこの2メンバの合成登録自体を削除したため、両エントリは「合成 vs bundled のオーバーラップ警告抑制」という本来の役割に対して到達不能（vestigial）になった。`putAll` は合成登録を維持しているため同じ分岐を共有する形で残置し、コード側にコメントを追記した（削除は他の呼び出し経路を全て洗い出すコストに見合わないと判断）。
       - `kk_map_size`/`kk_map_is_empty` の `__kk_` 降格: 3つの独立した名前解決経路（`CollectionLiteralLoweringPass+LookupTables+Map.swift` の literal 直接書き換え、`CallLowerer+UnresolvedMemberCalls.swift` の unresolved member call フォールバック ×6、interface メンバの externalLinkName）+ `RuntimeABISpec` + Runtime テスト8件超に跨るリネームで、本タスクの主目的（スタブ削除）と直交する高コスト変更のため見送り。
    5. `LinkedHashMap` の typealias を `CollectionAliases.kt` から本家準拠のファイル名 `LinkedHashMap.kt` へ分離（KSP-704 の HashSet/LinkedHashSet 分離と同じ手順、内容は無変更）。
    6. diff: 新規 `linkedhashmap_constructors.kt`（`LinkedHashMap(capacity)`/`(capacity, loadFactor)`/`(originalMap)` のコピーコンストラクタを同一関数内リテラル経由・関数引数経由の両方で検証）を追加し `diff_kotlinc.sh` PASS。コピーコンストラクタが `state.mapExprIDs` の関数ローカルな literal 追跡に依存し関数引数越しでは空マップになるのではという懸念を事前に持っていたが、実際には既存の Lowering が両方とも正しく処理していることを確認した（未解決バグではなかった）。
    7. 動作確認: `swift build` green。対象 Sema/Lowering/Codegen テスト `MapAsSourceMigrationTests`/`MapInterfaceSourceMigrationTests`/`MutableMapInterfaceSourceMigrationTests`/`MutableMapEntrySourceMigrationTests`/`MapHOFLoweringRoutingTests`/`RuntimeABIExternalLinkValidationTests`/`ABIMismatchRuntimeExportParityTests`/`CodegenBackendMapHOFTests`（計26テスト）green。`diff_kotlinc.sh` を map/mutable_map/hashmap/linkedhashmap 関連60ケース（新規 `linkedhashmap_constructors.kt` 含む）で実行し `total=59 failed=0 passed=59 skipped=1`（skip は `ksp687_map_not_null.kt` の既存 `SKIP-DIFF (DEBT-DIFF-001)` タグによるもので本変更と無関係）。Golden Sema（`GoldenSemaGoldenTests/matchesGolden`）全件・全 Swift suite・全 Golden（Lexer/Parser/Diagnostics）・全 `diff_kotlinc.sh`（約1400ケース）は本メモ執筆時点で未実行（CI に委ねる。symbol ID 割当が変わるため Sema golden に機械的差分が出る可能性はあり、出た場合は `UPDATE_GOLDEN=1` で追従予定）。
    8. `loc_report.sh`（git diff から算出）: `header_helpers_synthetic_total_lines` 31058→30466（-592、`HeaderHelpers+SyntheticMapStubs.swift` の縮小分）。`kk_literal_count` 6090→6069（-21、削除した22件の `"kk_map_*"` literal のうち1件 `"kk_map_is_empty"` は `Map.kt` の `@KsSymbolName` として維持のため再カウント）。`kk_cdecl_count`（838）・`__kk_cdecl_count`（849）は不変（Runtime 変更なしのため予定通り）。
    9. **§13-4 の二重 oracle 未対応（明記）**: `remove`/`clear` をインターフェース経由（`MutableMap`-typed receiver）で呼ぶ自己完結型の実行テスト（KSP-INF-006）は本PRでは追加していない。CI の `diff_kotlinc` ジョブは `continue-on-error` のため enforcing ではなく、`remove`/`clear` の実行時挙動を enforcing に検証しているのは `swift_test.sh` 配下のテストのみ（Sema レベルの型検査止まり）。KSP-704 も同様の状態で完了しており本PRもその前例に倣ったが、明示的に記録しておく。
...
- [~] CLEANUP-STUB-115: `HeaderHelpers+SyntheticPathStubs.swift`（本体）を削除する。CI green 確認後に `[x]` へ更新する
