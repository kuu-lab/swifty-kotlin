...
最終更新: 2026-07-16（PR #4844 での検証で BUG-039 の暫定緩和（PR #4846、`SWIFT_TEST_PARALLEL=0` 全体直列化）が `Argument list too long` を誘発し不十分と判明、XCTest/Swift Testing 分割による恒久対応へ更新。無関係な coroutine diff flake を BUG-040 として新規追加。それ以前: master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [ ] BUG-039: master CI（`Full Swift Tests (RuntimeTests)` ジョブ）が2026-07-15〜16に直近4回中3回失敗。原因は BUG-021/023/024/028 系と同根で、`kk_runtime_force_reset()` 等が書き換える `Sources/Runtime/RuntimeGC.swift` のプロセス全体共有ハンドルテーブル（`runtimeStorage.objectPointers` 等）に対し、XCTest 時代の `RuntimeTestIsolationSupport.swift`（`gcSemaphore` 等のプロセス全体セマフォ、`IsolatedRuntimeXCTestCase.setUp/tearDown` 経由）が提供していたクロスファイル排他が、Swift Testing の `@Suite` では構造的に再現できない（`XCTestCase` サブクラス専用の仕組みで Swift Testing の struct suite からは使えない）。`@Suite(.serialized)` はスイート内のみを直列化し、他ファイルの並行スイートとは排他されないため、あるスイートの reset が別スイートの生存中ハンドルを消し飛ばし、`invalid range/array/string handle` パニックや `Set`/`Array` 内容破損（解放先アドレスの再利用によるものと推定）を引き起こす。対象確認: `Tests/RuntimeTests/RuntimeSynchronizedTests.swift`（PR #4823 で移行済み・`init()` と各 `@Test` の `defer` で `kk_runtime_force_reset()` を正しく呼び `.serialized` も付与済みだが、cross-suite 排他が無いため依然として攻撃源になり得ることを示す新規ケース。BUG-020〜035 では未記載）。既存の BUG-023/024/028 も現行 master 上で確認済みの同根の実攻撃源（詳細は各項目参照）。最小再現: `DEVELOPER_DIR=/Applications/Xcode-beta.app SWIFT_TEST_WORKERS=4 bash Scripts/swift_test.sh --filter '^RuntimeTests\.' -Xswiftc -swift-version -Xswiftc 6` をローカルで数回実行（`SWIFT_TEST_PARALLEL` 未設定＝デフォルト `--parallel --num-workers 4`）。発見元: 本 master CI 失敗調査タスク（2026-07-16）。暫定対応: `.github/workflows/ci.yml` の `full-swift-tests` ジョブで `RuntimeTests` マトリクスエントリのみ `SWIFT_TEST_PARALLEL: "0"` を設定し直列実行に固定（実測 196 tests / 0.235 秒、タイムアウト無関係）。根本修正（Swift Testing 向けクロススイート排他機構の再構築、または全対象ファイルの個別是正）は未着手。暫定緩和 PR: #4846。**追記（2026-07-16、PR #4844 での検証）: この暫定緩和は不十分と判明。** master HEAD `a28cd1cf13`（PR #4846 マージ後）の CI（run 29479751122）で `Full Swift Tests (RuntimeTests)` が引き続き failure。ログを確認したところ原因は cross-suite race ではなく `error: posix_spawn error: Argument list too long (7)` — `SWIFT_TEST_PARALLEL=0` で RuntimeTests ターゲット全体（XCTest + Swift Testing 双方、数百件）を直列実行すると、SwiftPM のシリアル XCTest ランナーが `--filter` マッチした全テストIDを単一の exec 引数として渡すため Linux の `MAX_ARG_STRLEN` を超過し、どのテストも実行される前に落ちる。PR #4844 側でこれを独立に発見し、`.github/workflows/ci.yml` の `full-swift-tests` を「XCTest スイートは並列のまま `--disable-swift-testing` で実行」「Swift Testing スイートのみ `--disable-xctest --no-parallel` で直列実行」の2ステップに分割する形へ修正済み（XCTest 側は `IsolatedRuntimeXCTestCase` によるクロスファイル排他を維持したまま並列を保てるため引数長も増えず、Swift Testing 側は対象数が少なく直列化しても引数長を超過しない）。この分割修正を BUG-039 の恒久対応として本 PR に統合する
- [ ] BUG-040: `Scripts/diff_cases/coroutine_cancellation_advanced.kt`（`kotlinc Diff Regression` CI ジョブ）が非決定的に stdout mismatch で失敗することがある。症状: 4番目の `job4`（`try { delay(Long.MAX_VALUE) } finally { withContext(NonCancellable) { delay(1); println("cleanup done") } }` を cancel）由来の `"cleanup done"` 行が、kswiftc（candidate）側の出力にのみ現れ kotlinc/JVM（reference）側には現れない（`job2 cancelled: true` の直後・`done` の直前の3行 vs 2行の diff）。再現: `Scripts/diff_cases/coroutine_cancellation_advanced.kt` を `Scripts/diff_kotlinc.sh` で実行（発生ジョブ: [kotlinc Diff Regression Shard (2/11)](https://github.com/kuu-lab/swifty-kotlin/actions/runs/29438909053/job/87458007052)）。PR #4844（DEBT-KIR-004 の Base64.kt operand-order workaround 除去のみ）の CI で発見したが、当該 PR の diff とは無関係（コルーチン関連コードは一切変更していない）。master 直近4回の同ジョブはいずれも成功しており低頻度の再現と見られる。cancellation 由来のタイミング依存バグは本ファイルで過去にも複数回発生・修正されている（`docs/diff-skip-inventory.md` の DEBT-DIFF-003 参照）ため、cleanup（`NonCancellable` context 内の処理）が cancel 後の後始末レースにより実行タイミングがズレる、または JVM 側で実行順序が異なる可能性がある。ローカルでの再現は本 PR 作業時に使用した worktree の `.build` がリンク時に `_kk_*` runtime シンボル未解決で失敗する別問題（Runtime 側の環境不整合の疑い、本バグとは無関係）に阻まれ未実施 — 発見元: PR #4844 (claude/bitwise-operator-bug-870025) の CI failure 調査
