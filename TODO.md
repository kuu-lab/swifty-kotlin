...
- [x] BUG-155: ジェネリック interface `Sequence<out T>` を実装するクラス/object 式の `override fun iterator(): Iterator<T>` が `KSWIFTK-SEMA-OVERRIDE-RETURN` になる。原因: `Inheritance.swift` の `bindInheritanceEdges` が class/interface の継承句における型引数 `Seq<T>` の `T` を型パラメータとして解決できておらず、`supertypeTypeArgs` が空配列になっていた。修正: `resolveTypeRefForInheritance` 等に `localTypeParameters` を渡し、単純名が現在の型パラメータと一致する場合は `typeParam` 型を返すようにした。これにより `liftedNominalSupertypeArgs` が `Seq<T>` の型引数を正しく子クラスの `T` に lift し、戻り値型の型引数 substitution が正しく行われる。最小再現: `interface Seq<out T> { operator fun iterator(): Iterator<T> }` `class MySeq<T>(val it: Iterator<T>) : Seq<T> { override fun iterator(): Iterator<T> = it }`。検証: `bug155f.kt`（object 式で `Seq<Int>` を実装し `for-in` 実行）で `1 2 3` を出力、`SmokeTests`/`Golden`/`validate_runtime_abi_links.sh` green。発見元: KSP-441〜447 Sequence ソース化
