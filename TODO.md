...
- [x] RF-LOWER-CALL-008: Listの変換・filter系だけをsymbol基準の保持判断へ寄せる（前提: CALL-007）
  ...
  - 発見したバグ（本PRでは未修正、理由付きで PR description に記録。TODO.md への追記は他セッションとの同箇所挿入衝突を避けるため見送った）: `mutableListOf()` 直渡し destination の型推論失敗、`windowed(size) { transform }` の戻り型が期待型検査に伝播しない不具合、`Array<out Iterable<T>>.flatten()` 未実装の3件。いずれも Sema 型推論層 / stdlib surface の問題で、本PRの「KIR・IR 不変性を根拠に削除する」という検証構造と混ぜられないため分離した。
