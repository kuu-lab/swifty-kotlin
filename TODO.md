...
- [ ] BUG-145: ジェネリック interface `Sequence<out T>` を実装するクラス/object 式の `override fun iterator(): Iterator<T>` が `KSWIFTK-SEMA-OVERRIDE-RETURN` になる。戻り値型 `Iterator<T>` の型引数が基底側の型引数と同一視されず、共変関係が認識できていない。最小再現: `interface Seq<out T> { operator fun iterator(): Iterator<T> }` `class MySeq<T>(val it: Iterator<T>) : Seq<T> { override fun iterator(): Iterator<T> = it }`。`Sequences.kt` 内の `SingleSequence`/`GeneratorSequence`/`ConstrainedOnceSequence`/`asSequence` helper の `object : Sequence<T>` でも同様。発見元: KSP-441〜447 Sequence ソース化。修正にはメソッドオーバーライド時の戻り値型の型引数 substitution を正しく行う Sema 修正が必要
