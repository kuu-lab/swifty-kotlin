...
- [~] RF-LOWER-INLINE-004: 型引数代入・reified token生成を分離する（前提: INLINE-003）
  ...
  - 2026-09-15 実施: `InlineTypeSubstitution` と `buildInlineTypeSubstitution` / `collectInlineTypeSubstitution` / `substituteInlineType` / `buildTypeParamTokenValues` を `Sources/CompilerCore/Lowering/InlineTypeSubstitution.swift` へ移し、`InlineLoweringPass` は `sema` と substitution を渡すだけの呼び出し側に整理した。`TypeSystem.substituteTypeParameters` と既存の引数走査・hidden reified token 対応はそのまま委譲し、generic / nullable / function receiver / nested type argument の結果を固定する `InlineTypeSubstitutionTests` 2件を追加した。Semaの推論・制約解決は変更していない。
  - 検証: `swift build` PASS / `InlineTypeSubstitutionTests` 2件 PASS / `--filter Inline --no-parallel` は Core 106件・27 suite + Backend 14件・6 suite の計120件・33 suite PASS。新規2ファイルは `swiftlint lint --disable-sourcekit --no-cache` で違反なし（SourceKit依存の `statement_position` は環境上スキップ）。全Swiftテスト・全Golden・`Scripts/diff_kotlinc.sh` 全件は未実施。
...
- [ ] KSP-1353: kotlin.sequences.Sequence.max-family の未実装 stdlib API を実装する（18 件）
  ...
  - 現状確認（2026-09-15、監査 stale・追記のみ）: 18件中8件（`max`/`maxBy`/`maxByOrNull`/`maxOf`(#A: Comparable版)/`maxOfOrNull`(同)/`maxOrNull`/`maxWith`/`maxWithOrNull`、いずれも汎用 `Comparable` 版のみ）は `SequenceAggregateHOF.kt:302-453,923-937` に実装済み。真の残件は10件: `max`/`maxOf`/`maxOfOrNull`/`maxOrNull` の Double/Float 特化 overload（各2件・計8件）と、`maxOfWith`/`maxOfWithOrNull`（Comparator 版、計2件）。チェックボックスは残件があるため据え置き、次回着手者は上記10件のみに絞ってよい。
...
- [ ] KSP-1354: kotlin.sequences.Sequence.min-family の未実装 stdlib API を実装する（18 件）
  ...
  - 現状確認（2026-09-15、監査 stale・追記のみ）: max-family と同型。18件中8件（`min`/`minBy`/`minByOrNull`/`minOf`/`minOfOrNull`(Comparable版)/`minOrNull`/`minWith`/`minWithOrNull`）は `SequenceAggregateHOF.kt:317-440,939-953` に実装済み。真の残件は10件: `min`/`minOf`/`minOfOrNull`/`minOrNull` の Double/Float 特化 overload（計8件）と `minOfWith`/`minOfWithOrNull`（計2件）。チェックボックスは残件があるため据え置き。
