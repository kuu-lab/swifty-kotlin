...
- [ ] BUG-257: `kotlin.native.*` の source-backed `object`（`Platform` / `Debugging` 等）が持つ、Kotlin ソース側で `get() = __bridge() != 0` のように書かれた計算プロパティ（`val`/`var` 問わず）を実際に呼び出すと `EXC_BAD_ACCESS`（exit 138、SIGBUS）でクラッシュする。最小再現1（`Platform.kt` は本 PR 未変更。ただし同一ワーキングツリーでビルドした `kswiftc` バイナリでの確認であり、別 baseline バイナリでの再現は未確認 — 試みたが main checkout の baseline binary をこの worktree の CWD から実行すると `.runtime-build` キャッシュパスが worktree 側に解決され `_kk_debugging_is_thread_state_runnable` の未定義シンボルでリンク失敗し、クリーンな検証はできなかった）: `@file:OptIn(kotlin.experimental.ExperimentalNativeApi::class)` + `import kotlin.native.Platform` + `fun main() { println(Platform.canAccessUnaligned) }` を `kswiftc --stdlib-from-source` でビルドして実行するとクラッシュする。最小再現2（本 PR で追加した `Debugging.kt`）: `import kotlin.native.runtime.Debugging` + `@OptIn(kotlin.native.runtime.NativeRuntimeApi::class)` + `fun main() { println(Debugging.isThreadStateRunnable) }` も同様にクラッシュする。一方、`object` の**プロパティではない**プレーンな member 関数（例: `kotlin.system.System.currentTimeMillis()`、実装は同じく top-level `private external fun` ブリッジを呼ぶだけ）は正常に動作する（`dumpMemory(fd: Long): Boolean` はプレーンな `fun` なのでこの回避対象——ただし実機での実行は未確認）。`lldb` で確認したクラッシュ地点はプロパティ getter が生成する `kk_fn_get_...` 内の「呼び出し前に outThrown スロットを 0 クリアする」プロローグ（`str xzr, [x8]`）で、`x8`（= 呼び出し元が渡した outThrown ポインタ `x1` の再読み込み）がゴミ値（例: `0x20106e317`）になっている。根本原因を特定済み: `Sources/CompilerCore/Lowering/ABILoweringPass.swift:374-377` の `isSyntheticAccessor`（`SyntheticSymbolScheme.isLikelySyntheticPropertyAccessor` を呼ぶ）は、`Sources/CompilerCore/Sema/Models/SyntheticSymbolScheme.swift:146-175` の通り「プロパティの getter/setter アクセサとして符号化された `SymbolID`」であれば真偽値の由来（純粋な synthetic stub か、実 Kotlin ソースの `get() { ... }` 本体を持つ source-backed プロパティか）を区別せずに `true` を返す（コメント自身が「historical heuristic」と認めている）。`ABILoweringPass.swift:418-422` の `canThrow` 計算はこれを「synthetic アクセサ＝ほぼ確実に non-throwing」という前提で `isDelegatedAccessor`（delegate はこの前提の唯一の既存の例外）以外は non-throwing 扱いにするため、呼び出し元は outThrown 用のレジスタ/スタック領域を用意しない。ところが `isThreadStateRunnable`/`canAccessUnaligned` のような **source-backed** な `get() = bridge() != 0` は、`!= 0` 比較のための unboxing（`kk_unbox_int` 呼び出しが2回、ボックス化を経由する既存の boxing 経路によるものと推測）を getter 本体の内部に持つため、getter 自身は（別の独立した lowering 判断で）throwing ABI（self + outThrown の2引数）で emit される。この caller 側 `canThrow=false` と callee 側「throwing ABI で emit」の不一致がゴミ `x1` の正体。想定される修正の形（未検証・未適用）: `isDelegatedAccessor` と同様の追加条件として「元プロパティが `symbols.isSourceBackedSymbol(propertySymbol)` かつ `symbols.externalLinkName(for: propertySymbol) == nil`（＝プロパティ自体に直接の外部リンクがなく、実 Kotlin 本体を持つ）なら `canThrow` に含める」を `canThrow` の算出に足す案があるが、`ABILoweringPass` のこの分岐はプロパティアクセス全般を通る中枢経路であり、ビルド・関連 lowering/ABI テスト一式での検証なしに適用するのは危険なため未適用。発見元: KSP-1260 で `Debugging.kt` を実装し、Sema/golden/ABI 整合テスト（golden 4種・`NativeDebuggingSourceAPITests`・`RuntimeABIExternalLinkValidationTests` 含む）がいずれも green であることを確認した上で、`kswiftc --stdlib-from-source` による実行時スモークテストを行って発覚。診断ゲート（`diff_kotlinc.sh`）はこの系統の API を軒並み `// SKIP-DIFF (DEBT-DIFF-001)` として実行自体をスキップするため、`Platform` 導入時からこの種の回帰は一度も CI で実行検出されていなかった可能性が高い。なお `isThreadStateRunnable` は本 PR 適用前は「`externalLinkName` を直接持つ純粋 synthetic プロパティ」（getter 本体を持たず `isSyntheticAccessor` 分岐にすら乗らない、直接 external link 呼び出しの単純な形）だったため、この不整合を踏んでおらずクラッシュしなかった可能性が高い——その意味で `isThreadStateRunnable`/新規 `forceCheckedShutdown` の呼び出し時クラッシュは、本 PR の「synthetic 直結プロパティ → source-backed 計算プロパティ」への移行そのものが顕在化させた退行である（`Platform.kt` 側は本 PR 未変更で同じ症状が出るため、バグの根本原因自体は pre-existing）。今回修正しない理由: 原因は `ABILoweringPass` のプロパティアクセサ throws 分類という、プロパティアクセス全般に使われる中枢の ABI 決定ロジックにあり、影響範囲が `Debugging` 単体ではなく `kotlin.native.Platform` の既存プロパティ全般（`canAccessUnaligned`/`isLittleEndian`/`isDebugBinary`/`programName`/`isMemoryLeakCheckerActive` 等、いずれも未検証だが同一パターン）や、将来の他モジュール移行（M1–M17）で同種の computed property を source-back する際にも及ぶ可能性が高い。安全な修正には、上記の追加条件案を適用した上でプロパティ/ABI lowering の回帰テスト一式（`ABILoweringPass` 関連・`PropertyLoweringPass` 関連・golden 一式）を通す検証が要り、KSP-1260（`Debugging` の Sema 表面を kotlinc 2.3.10 相当へ揃える）1タスクの安全な修正範囲を超えるため、BUG-239/BUG-250 と同じ方針で追跡する。
