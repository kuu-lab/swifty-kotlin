...
- [ ] BUG-145: `String` 拡張関数の本体内で `substring(...)` を**暗黙レシーバ**（`this.` 無し）で呼ぶと実行時に巨大サイズ確保（`Fatal error: failed to allocate <huge> bytes`）でクラッシュする。`this.substring(...)` と明示すれば正常。同じ暗黙レシーバでも `get`/`uppercase`/`reversed` 等は正常に動作し、`substring` 固有。`substring` は `CallLowerer+LegacyMemberLikeCalls.swift` の name-based lowering（`kk_string_substring_flat`）を通り、暗黙レシーバ経路で `loweredReceiverID` が誤り、Int 引数がレシーバ／長さとして誤解釈されるものと推定。最小再現:
  ...
  - 修正（KSP-406、本タスクの PR）: `substring`/`subSequence`/`removeRange`/`replaceRange` の legacy name-based lowering（`CallLowerer+LegacyMemberLikeCalls.swift`）を撤去し、通常の source-backed member 解決経路に載せた。暗黙レシーバ `substring(n)` と明示レシーバ `this.substring(n)` が同一の結果（`"hello".g(2) == "llo"`）を返すことを `.build/debug/kswiftc` で実機確認。回帰テストは `Scripts/diff_cases/string_substring_slice_range.kt`（暗黙/明示両経路）で固定。マージ後 `[x]` 化する
