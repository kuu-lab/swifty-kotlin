...
> 2026-07-06 棚卸し: [`docs/diff-skip-inventory.md`](docs/diff-skip-inventory.md)。2026-07-12 時点の `DEBT-DIFF-*` タグ付き skip は 61 件（001:22 / 002:8 / 003:14 / 004:0（解消済み） / 005:14 / 006:3）。各タスクの完了条件は、該当ケースを通常 `diff_kotlinc.sh` に戻すか、JVM kotlinc を oracle にできない理由と代替 runner / unit test owner を同文書へ移すこと。
