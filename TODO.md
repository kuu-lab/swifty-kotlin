...
- [ ] DEBT-KIR-007: bundled Kotlin stdlib source（`Sources/CompilerCore/Stdlib/**/*.kt`）で定義済みのメンバ関数・プロパティの解決結果（実シンボル/`callBindings` の有無、および overload index `#N` の割り当て）が、**同一コミットにもかかわらず macOS ローカルビルドと Linux CI ビルドで異なる**現象を調査・修正する。観測例: `java.io.File.name` は macOS では `ref=java.io.File.name`（実プロパティ参照）に解決されるが、Linux CI では `call=kotlin.io.name.$get`（旧 synthetic dispatch 経由、`ref=` なし）になる。`kotlin.time.TimedValue.value`/`.duration` も同型。`kotlin.collections.emptyList`（`CollectionFactories.kt` 移行済み）は macOS では `call=kotlin.collections.emptyList` の実シンボル参照 + シンボルテーブルへの `emptyList`/`emptyList.T` エントリを伴うが、Linux CI では `CallTypeChecker.swift` 内の `stdlibCollectionFactoryNames` 早期分岐（`inferCallExpr` 内、コメント `P5-84`）を経由し `callBindings` 自体が設定されない。`kotlin.text.first`/`firstOrNull`/`replace`/`replaceFirst` は overload index（`#0`/`#1`/`#2`）の割り当てが macOS/Linux で異なる（登録順序依存と推定）。原因未特定だが `Bundle.module` のリソース列挙順序または SwiftPM の Linux 向けリソースバンドル完全性の違いを疑う。KSP-482 (#4625) で `abstract_class` / `file_is_rooted` / `file_props` / `file_tree_walk` / `flatten_stdlib` / `list_collection_hofs` / `measure_timed_value` / `override_generic_covariant_return` / `stdlib_string_ops` の9件の golden ファイルがこれで CI 上不一致になり発覚（Base64 移行そのものとは無関係）。**運用上の注意**: 現状このリポジトリの慣習として golden ファイルは Linux CI の実際の出力を正とする（`git log` 上、上記シンボルを含む既存 golden は Linux CI 実行結果と一致し、macOS ローカルの `UPDATE_GOLDEN=1` 再生成はこれらのシンボルに限り Linux CI と食い違う値を書き込みうる）。根本修正までは、bundled stdlib 移行済みシンボルを含む golden ケースを macOS でだけ検証して確定させないこと
...
> 2026-07-06 棚卸し: [`docs/diff-skip-inventory.md`](docs/diff-skip-inventory.md)。現時点の `DEBT-DIFF-*` タグ付き skip は 66 件（001:22 / 002:8 / 003:14 / 004:5 / 005:14 / 006:3）。各タスクの完了条件は、該当ケースを通常 `diff_kotlinc.sh` に戻すか、JVM kotlinc を oracle にできない理由と代替 runner / unit test owner を同文書へ移すこと。
