...
- [x] RF-LOWER-CALL-007: source-backed呼び出しの保持判断を専用policyへ抽出する（前提: CALL-002）
  ...
  - **マージ順序の制約（重要）**: 本項は series の他項より後に取り込む必要がある。着手時の `gh pr list --state all --search "RF-LOWER-CALL"` では CALL-001 しか出なかったが、これは検索インデックスの遅延による見落ちで、実際には CALL-002 (#6761) / 003 (#6758) / 004 (#6768) / 005 (#6759) / 006 (#6765) / 009 (#6762) / 010 (#6766) / 011 (#6763) / 012 (#6764) が open で、いずれも本項が移動させた同じ hunk を編集している（列挙は `gh pr list --state open --limit 40` で確認）。
    - とくに 009 / 010 / 011 / 012 は inline の `||` 連鎖から名前を削除する変更で、合計 54 名（009: 12、011: 24、012: 11、010: 7）を「外側のメンバー名ゲートに届かないので何も守っていない」と KIR byte-identical で実証して落としている。本項の `sharedAggregateNames` は移動時点の 102 名すべてを保持しているため、**それらが先にマージされた後に本項を「抽出側を採用」で解決すると削除済みの名前が丸ごと復活する**。`kk_map_maxByOrNull` / `kk_list_maxOfOrNull` など spec-only の export は `@_cdecl` が無いため、復活すればリンクエラーになる。
    - したがって正しい順序は 002・009〜012 を先に取り込み、本項をその上に rebase して、生き残った名前だけで Sets を構成すること。抽出は移動のみなので rebase 自体は機械的だが、解決時は必ず各PRの削除を取り込んだ側を基準にする。
