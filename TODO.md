...
> 2026-07-06 棚卸し: [`docs/diff-skip-inventory.md`](docs/diff-skip-inventory.md)。現時点（2026-07-09）の `DEBT-DIFF-*` タグ付き skip は **70 件**（001:23 / 002:8 / 003:14 / 004:5 / 005:15 / 006:3 / 007:1 / 008:1）。各タスクの完了条件は、該当ケースを通常 `diff_kotlinc.sh` に戻すか、JVM kotlinc を oracle にできない理由と代替 runner / unit test owner を同文書へ移すこと。内訳の変遷: 2026-07-06 棚卸し時点は 001:22 / 005:14 の 66 件。2026-07-08 の #4575（`uuid_basic.kt` 追加）で 001 が 23 に、2026-07-09 に本タスクで 005 の要約行漏れ（詳細表は元々 15 件列挙済み）を 15 へ補正し、DEBT-DIFF-002 の一部（top-level functions / custom declarations 4件）を実測して timeout 無関係と確定、新たに DEBT-DIFF-007 / 008 の2バグを発見・分離した。
...
- [ ] DEBT-DIFF-002: script-style diff cases の kotlinc JVM 起動 timeout / top-level execution parity を整理し、script runner 側で安定比較できるケースから `SKIP-DIFF` を外す。2026-07-09: top-level functions / custom declarations の4件は timeout 無関係と確定（kswiftc がトップレベル `fun` と文の混在から暗黙 `main` を合成できないことが blocker）。timeout-only suspect 3件（`script_imports.kt` 等）は未着手のまま。
...
- [ ] DEBT-DIFF-007: `firstOrNull { predicate }` の trailing-lambda 呼び出しが無引数オーバーロードに誤解決される bug を、overload resolution（Sema）で修正する。最小再現: `Scripts/diff_cases/firstornull_trailing_lambda.kt`。2026-07-09、DEBT-DIFF-002 の調査中に発見。
- [ ] DEBT-DIFF-008: `vararg items: Any` 経由で取り出した `Boolean` の `toString()` が boxing 境界で壊れる（`"true"` ではなく `"1"`）bug を、Runtime ABI / boxing boundary で修正する。最小再現: `Scripts/diff_cases/vararg_any_boolean_element.kt`。2026-07-09、DEBT-DIFF-002 の調査中に発見。
...
- [~] KSP-472: Instant/Clock/measureTime のブリッジを確定する（2026-07-08、一部配線）
  ...
  - 未達（技術的制約、direct stub のまま変更なし）: `kk_instant_now`, `kk_instant_from_epoch_millis`, `kk_clock_system_now` は companion/nested-object のファクトリメソッドで、このコンパイラの拡張関数宣言はレシーバ型としてネストした `Foo.Companion`/`Foo.Bar` を解決できない（`fun Foo.Companion.f(): Foo` は "Unresolved type 'Companion'" になる。単純ネストクラスの `fun Outer.Inner.f()` でも同様に再現する一般的な制約）ため Kotlin source の拡張として書けない。`kk_clock_now` は `Clock` がユーザー実装可能な interface であり `now()` の member dispatch が必須。`kk_measureTime`/`kk_measureTimedValue` は `CallLowerer+StdlibLoops.swift` の KIR 特殊インライン展開が `stdlibSpecialCallKind`（関数名ベース）でディスパッチしており、direct stub 名を変更すると壊れるため対象外。`kk_timedvalue_new` は `measureTimedValue` の KIR lowering からのみ呼ばれる内部コンストラクタで、Kotlin source から到達しないため対象外
  - フォローアップ候補: companion/nested-object 拡張のレシーバ型解決をコンパイラがサポートすれば `kk_instant_from_epoch_millis` 等も配線可能になる（新規タスク化を検討）
