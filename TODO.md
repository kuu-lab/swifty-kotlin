...
- [ ] BUG-038: `swift build` が XCBuild バックエンド（macOS + Xcode-beta 系ツールチェインで発生。中間ディレクトリ名が `<Target>-t.build/Objects-normal/<arch>/` 形式になる構成）でランタイムを再ビルドすると、`CodegenRuntimeSupport+RuntimeObjects.swift` の `collectObjectPaths` が `*.swift.o` サフィックスのみを走査するため、実際に生成される `RuntimeXxx.o`（`.swift.o` ではなく `.o`）を発見できず `KSWIFTK-LINK-0001: Unable to locate packaged runtime object files` でリンク失敗する（`swift build --target Runtime` 自体は exit 0 で成功し `.o` は生成されている。Linux 上のネイティブ SwiftPM ビルドシステムでは `.swift.o` 命名になるため CI では再現しない）。最小再現: 該当構成のローカル macOS で `bash Scripts/diff_kotlinc.sh --clean-runtime-cache Scripts/diff_cases/string_indent.kt`（内容非依存、`.runtime-build/` を伴うどの diff_cases ケースでも再現する想定）— PR #4598 の CI 失敗（StringIndentFunctionTests）調査中、ローカル kotlinc diff 回帰検証で発見。BUG-036 の「Xcode ツールチェイン不一致」所見と同根の疑い
- [ ] BUG-039: CI ジョブ `Full Swift Tests (RuntimeTests)`（`.github/workflows/ci.yml` の `full-swift-tests` マトリクス、`test-target: RuntimeTests`）が `swift test --parallel --num-workers 4 --filter '^RuntimeTests\.'` 実行時に `error: posix_spawn error: Argument list too long (7)` で毎回確定的に失敗する。`RuntimeTests` は `Tests/RuntimeTests/*.swift` 全体で `func test` が3171件（`ABIMismatchTests.swift` だけで97件）あるのに対し `CompilerCoreTests`/`CompilerBackendTests` と異なり `Scripts/shard_swift_tests.sh` によるシャーディングが一切されておらず単一ジョブのまま並列ワーカーに丸投げしているため、いずれかのワーカーに割り当てられたテスト ID のカンマ区切りリストが `posix_spawn` の引数長上限を超える。**PR 固有の問題ではない**: `master` の直近4回の完了済み CI 実行（2026-07-15 07:27・07:56・09:37・12:40 UTC、`gh run list --branch master --workflow CI`）も同一ジョブが同一メカニズムで失敗しており、PR #4598 のブランチ差分（`Sources/CompilerCore/Stdlib/kotlin/text/StringIndentFormat.kt`, `TODO.md` のみ）とは無関係にリポジトリ全体で再現する。再実行（`gh run rerun --failed`）しても同一コマンドラインで再現するため、フレーキーではなく確定的な回帰。想定される修正: `CompilerCoreTests`/`CompilerBackendTests` と同じく `Scripts/shard_swift_tests.sh --mode dynamic --list-filter '^RuntimeTests\.'` を使い `full-swift-tests` の `RuntimeTests` エントリもシャード化する（`RuntimeTests` は XCTest ベースなので dynamic モードの前提を満たす）。CI ワークフロー変更のため PR #4598 の作業中には未着手 — PR #4598 の CI 監視中、`master` の直近失敗履歴と突き合わせて発見
