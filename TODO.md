...
- [ ] BUG-046: `String.prependIndent()`（`Sources/CompilerCore/Stdlib/kotlin/text/StringIndentFormat.kt` の該当関数）が空行の扱いで本家 Kotlin stdlib と異なる。本家は「indent より短い空行には indent のみを出力」「indent 以上の長さの空行は変更せず維持」という特殊処理を行うが、現行実装は全行に無条件で `indent + line` を付与する。`splitIntoLines` が末尾 `\n` を持つ文字列から正しく末尾の空要素を生成するようになったことで、この既存の乖離がより顕在化しやすくなっている。`Scripts/diff_cases/string_indent.kt` の既存ケースはこの境界条件を踏んでおらずテストは通過するため、現時点でテスト失敗は起きない — PR #4567 の Devin レビュー（comment_id=3536459152）で指摘・発見。修正 PR: 未着手
