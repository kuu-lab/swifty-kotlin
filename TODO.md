...
- [x] RF-LOWER-CALL-002: 未消費の `builderLambdaKinds` 事前走査と引数配線を除去する（前提: CALL-001）
  ...
  - ゲート: `swift build` green。`Scripts/loc_report.sh` は `loc_by_directory Sources` −67 行、`kk_literal_count` −1（`"kk_lambda_\(...)"` の消滅）のみで、`HeaderHelpers+Synthetic*` 行数・KIR/Lowering TODO/FIXME 数・`interner.resolve == "..."` 数・`kk_cdecl_count` / `__kk_cdecl_count` は不変。ローカルは `CompilerCoreTests.BuilderDSLLoweringRoutingTests`（5）/ `CompilerCoreTests.CollectionLiteralLoweringTests`（68）/ `CompilerCoreTests.FrontendPhasesTests`（20）/ `CompilerBackendTests.CodegenBackendIntegrationTests` を実行。全 Swift テスト・Golden 4 系統・`diff_kotlinc.sh` 全ケースは CI に委譲。
