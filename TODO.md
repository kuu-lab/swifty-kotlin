...
- [ ] BUG-141: ユーザー定義 Iterator/Iterable クラスが `MutableList` または `IntArray` フィールドを保持し、そのフィールドに対して `size`/`get`/`iterator` 等を呼び出すと、実行時に `RuntimeValue.legacyRawValue.getter` で null/不正ポインタ読み出しにより SIGSEGV する（`RuntimeArrayBox.elements` 経路）。for-in lowering 自体は正しく生成されるが、イテレータ実体内のコレクション操作でクラッシュする。最小再現（手動 `hasNext()`/`next()` でも再現）:
  ```kotlin
