...
- [ ] BUG-134: PR #4834 で `Flow.collect`/`collectLatest` にローカル変数経由で渡した非suspendラムダ（最小再現: `val h = { v: Int -> println(v) }; flow.collect(h)`）が実行時 SIGSEGV。`isCollectionHOFCallee` に両APIを追加して通常HOFと同じ `kk_hof_adapter` を生成し、collector関数ポインタとclosure環境を `kk_flow_collect`/`kk_flow_collectLatest` へ渡すよう修正。`Scripts/diff_cases/coroutine_flow_local_collector.kt` で `kotlinc` と出力一致を確認。発見元: PR #4834 レビュー。修正PR: #4834（マージ後に `[x]` 化）
