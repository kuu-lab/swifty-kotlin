...
- [ ] BUG-045: `bash Scripts/swift_test.sh --filter CompilerCoreTests`（`--parallel`・`SWIFT_TEST_PARALLEL=0` いずれでも再現）が、実行のたびに異なる無関係な位置で `error: Process '.../swiftpm-testing-helper ... --filter CompilerCoreTests ... --testing-library swift-testing' exited with unexpected signal code 10` を出して落ち、スクリプト全体の exit code が 1 になる（BUG-040 の `posix_spawn`/Argument list too long とは別症状 — exec() 引数長ではなくテストランナー本体プロセスの異常終了）。個々の `Executed N tests, with 0 failures` チェックポイントは3回のフル実行全てで一貫して 0 failures を報告しており、クラッシュ後もテスト実行自体は継続する（`swift test` 側に crash 検出後の再走行/継続機構がある模様）。3回連続フル実行したところ、毎回無関係な異なる箇所（1回目: Lexer/Token 系、2回目: `DataFlowAndSemaRegressionTests` の深いジェネリックネスト系、3回目: SmokeDriver/Golden Lexer 系）でこの signal 10 が発生した。1回目・2回目は67 worktree 同時稼働による極端な高負荷時（load average 101/159/161、14コア）だったが、host load が正常化（7.74/17.73/44.14）した後に行った3回目でも再現したため、高負荷だけが原因とは限らない。一方 `--filter CompilerCoreTests.GoldenSemaGoldenTests/matchesGolden`（297件）や `--filter CompilerCoreTests.TypeCheckHelpersCoverageTests`（7件）のようにフィルタ範囲を絞ると3回とも即座に "All tests passed." で完走しており、対象を絞るほど再現しにくい傾向がある。発見元: `shouldUseRuntimeStdlibSpecialCall` 削除タスクの完了ゲート確認中（2026-07-17）。最小再現: `bash Scripts/swift_test.sh --filter CompilerCoreTests` を数回連続実行する（CI 側での再現は未確認、ローカル環境要因の可能性が高い）。未調査: `swiftpm-testing-helper` のクラッシュログ/core dump は未取得のため根本原因（メモリ不足由来の SIGBUS か、Swift Testing 側の並行実行バグか）は特定できていない。※本項目は当初 `BUG-041`、次いで `BUG-042` として登録したが、いずれも別セッションが並行して同番号を別内容で先に確定させたため、コンフリクト解消のたびに繰り上げ採番し直している（上記 `BUG-042` の WMO 単一 `Runtime.o` リンク不可とは別症状 — こちらはビルド済みテストバイナリの実行段階でランナープロセス自体が signal 10 で死ぬ）。（[[dev-environment-bash-version]] 参照）
