...
> 2026-07-11 棚卸し: [`docs/diff-skip-inventory.md`](docs/diff-skip-inventory.md)。現時点の `DEBT-DIFF-*` タグ付き skip は 68 件（001:20 / 002:7 / 003:14 / 004:5 / 005:19 / 006:3）。PR #4621 対応で `random_nextfloat_ranged_synthetic.kt` / `secure_random.kt` / `random_nextbytes_hang_workaround.kt`（KSP-466 の `Random.nextBytes` ハング回帰の一時退避）を追加した。各タスクの完了条件は、該当ケースを通常 `diff_kotlinc.sh` に戻すか、JVM kotlinc を oracle にできない理由と代替 runner / unit test owner を同文書へ移すこと。
