...
最終更新: 2026-07-19（PR #4842 で `--no-stdlib` の実配線と回帰テストを追加し BUG-043 を修正済みに更新）。それ以前: 2026-07-17（PR #4578 の CI 失敗調査で `launch{}` cancel-before-start レース（`kk_kxmini_launch`, `Sources/Runtime/RuntimeCoroutine.swift`）を確認、BUG-041 を追補。master 側の BUG-039/040（RuntimeTests CI の cross-suite GC race / exec引数長制限）とは無関係の別問題。同日: BUG-038 が 2 件に重複割当されていたのを検出し、後発の #4636 系を BUG-042 へ改番、両者とも修正 PR がマージ済みのため `[x]` 化。PR #4842 の `--no-stdlib` 死にフラグは BUG-041 の採番競合により BUG-043 へ改番。それ以前: master CI 失敗調査で BUG-039 の暫定緩和(PR #4846, `SWIFT_TEST_PARALLEL=0`)が Linux exec() 引数長制限に抵触し `Full Swift Tests (RuntimeTests)` を落とし続けていたことを確認、BUG-040 を追補し CI 側を修正。それ以前 2026-07-16: master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。それ以前 2026-07-15: RF0–RF9 完了項目の実態監査を反映 — SwiftLint ラチェット・CallTypeChecker 行数・ABI 検証網羅化 ×2 の計 4 件を `[x]` から `[~]` へ降格、Runtime HOF 共通化 1 件を `[ ]` から `[~]` へ更新、注記 4 件を実測値で補正、`--no-stdlib` 死にフラグを追補（現 BUG-043。各タスク行の「2026-07-15 監査」注記を参照）。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う）
...
- [~] RF-GUARD-003: SwiftLint の `file_length` / `type_body_length` を有効化し、既存違反は `.swiftlint.baseline.json` で凍結する（新規悪化のみ CI fail にするラチェット）。2026-07-15 監査で `[x]` から降格: `.swiftlint.yml` の両ルール有効化と `.swiftlint.baseline.json`（凍結違反 28 件）は実在するが、CI に swiftlint 実行が存在せず baseline は休眠状態（`f016878c75` 2026-03-12 で lint チェックが CI から削除されて以降未復旧）。残: CI での baseline 比較ラチェットの再導入
...
- [~] RF-STDLIB-002: `LoadSourcesPhase` に bundled Stdlib ソース読み込みを実装する（`Bundle.module` 列挙 → `sourceManager` 登録は基本配線済み。2026-07-15 監査で残リストを補正: ユーザー入力との診断パス区別は `__bundled_` 接頭辞機構で実装済みのため残から除外。2026-07-19 に PR #4842 で `--no-stdlib` の実配線（BUG-043）と回帰テストを追加済み。残: 形式的な source origin 型の導入）
...
- [x] RF-SEMA-001: TypeCheck の名前比較特例 104 箇所の台帳を作る（機能・対応スタブ・スタブ/ソース移行後に削除可能か、の 3 列）。台帳: [`docs/rf4-name-special-case-inventory.md`](docs/rf4-name-special-case-inventory.md)。件数は台帳記載の baseline コマンドで都度取得する（台帳自体は意図的にスナップショット数値を記録しない。2026-07-15 実測は 107 行 — 元メモの 104 / 07-06 時点の 102 とはブランチ差分で変動）
...
- [~] RF-SEMA-005: `CallTypeChecker.swift` の特例ブロックをレジストリ移行済み分から削除し 3,000 行未満にする（以降 SwiftLint baseline のバジェットで維持）。2026-07-15 監査で `[x]` から降格: PR #4594 で 4,085 → 3,267 行に削減したが 3,000 行未満は未達（残 268 行。元メモの起点 3,896 行も削減直前の実測は 4,085 行だった）
...
- [~] RF-KIR-005: runtime ABI external-link gap 検証を enforcing に昇格する（`RuntimeABISpec` 未宣言の `kk_*` 名 emit を CI fail にする）。2026-07-15 監査で `[x]` から降格: `RuntimeABIExternalLinkValidationTests` は `#expect` で fail する enforcing として CI 必須ゲートに含まれるが、[`docs/runtime-abi-external-link-validation-gaps.md`](docs/runtime-abi-external-link-validation-gaps.md) の引き継ぎ 5 単位のうち実装済みは単位 1（Sema 登録名）・2（KIR リテラル collector）のみ。残: 単位 3（動的 prefix `kk_xxx_\(...)` 展開）・4（`StdlibSurfaceSpec` runtimeLinkName 照合）・5（内部名前空間分類）と `allowedCompilerExternalLinks`（30 項目）の縮小
...
- [~] RF-RT-005: Runtime の全 `@_cdecl` が `RuntimeABISpec` に宣言されていることの CI 検証を網羅化する（`validate_runtime_abi_links.sh` 拡張、compiler emit 側の enforcing check と対）。2026-07-15 監査で `[x]` から降格: `ABIMismatchTests+RuntimeExportParity` が export⊆spec / spec⊆export / シグネチャ一致を CI で双方向検証しているが、`kk_string_` / `__kk_string_` / `__string_` / `kk_regex_` / `kk_locale_` プレフィックスの 482 件（Runtime 全 `@_cdecl` の約 17%）が除外され、同等の網羅検証が存在しない。`validate_runtime_abi_links.sh` も未拡張（compiler 側テストを起動するだけの 8 行のまま）。残: 除外プレフィックスのパリティ検証追加と allowlist（export-only 17 / spec-only ~85）の縮小
...
- [x] RF-TEST-003: `*SyntheticMemberLinkTests` 群は対応スタブの削除と同一 PR で削除するルールにする（リファクタ対象にしない）。2026-07-10 完了: `docs/ARCHITECTURE.md` §5 に synthetic member link tests の扱いを明文化。2026-07-15 再計測: List 2,362 行 + 既存分割 `+MutableAndAdvancedMembers` 2,238 行 / Sequence 2,739 行 / String 2,043 行（07-10 メモの List 2,284 / String 2,057 は実測と不一致だったため補正。List の分割はルール制定前の既存物で、以後の分割・整理は行わない）。
...
- [x] KSP-INF-001: `--no-stdlib` を実装する（`CLIParser.swift` でパースした `CompilerOptions.includeStdlib` に従って `LoadSourcesPhase.run` の bundled stdlib 注入を制御）。PR #4842 で分岐 + 動作テストを追加。完了: includeStdlib の読出実装 + `FrontendPhasesTests.testLoadSourcesRespectsIncludeStdlibOption` green + G
...
- [ ] BUG-024: PR #4824（XCTest→Swift Testing移行）で BUG-021 と同型の `tearDown`（`kk_runtime_force_reset()`）消失 — 対象 `Tests/RuntimeTests/RuntimeSetCollectionHOFTests.swift`。2026-07-16: master CI（`Full Swift Tests (RuntimeTests)`）で `testSetFilterAllMatchReturnsAllElements()` 等の Set 内容破損として実観測、ローカル `--parallel --num-workers 4` でも再現確認（BUG-039 参照）。CI 側は `SWIFT_TEST_PARALLEL=0` 固定で暫定緩和済み（PR #4846）
...
- [x] BUG-042（旧 BUG-038 — 先行の itable 系と ID が重複していたため 2026-07-17 に改番）: PR #4636 の CI で判明した runtime 表示/例外 message の不整合。`kk_println_any(kk_box_char(0xDF1F))` と `println("hello🌟".lastOrNull())` が `?` ではなく U+FFFD を出力し、`(42 as String)` の `ClassCastException.message` が空になる — 再現: `Tests/RuntimeTests/RuntimePrintlnTests.swift`、`Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+StringHOFEdgeCases.swift`、`Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+InlineFunctionExceptionPropagation.swift`。修正 PR: #4636（`508d4fbb95` として 2026-07-14 マージ済みのため `[x]` 化）
...
- [x] BUG-043: `--no-stdlib` フラグが `CompilerOptions.includeStdlib` に格納されるだけで bundled stdlib 注入を止められなかった（最小再現: `.build/debug/kswiftc --no-stdlib hello.kt -o out`）。RF0–RF9 完了項目監査（2026-07-15）で発見し、採番競合を経て BUG-043 に確定。修正 PR: #4842（`LoadSourcesPhase` の `includeStdlib` 分岐と `FrontendPhasesTests.testLoadSourcesRespectsIncludeStdlibOption` を追加、2026-07-19 focused test green）
