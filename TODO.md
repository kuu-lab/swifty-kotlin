...
- [x] RF-LOWER-CALL-004: `buildList` の旧runtime rewriteを削除する（前提: CALL-003、KSP-697の必要な移行・fallback整理完了）
  ...
  - 順序について: 着手時点では前提の CALL-003 が未着手だったため先行した（CALL-003 の対象 `isSourceBackedStdlibBuilderDSLCall` は `isStdlibBuilderDSLCall` 内の恒偽分岐で、buildList 経路と直交する）。その後 CALL-003（#6758）と CALL-005（#6759）が並行してマージされたため、本PRは master を取り込み union 解決した。三者は同じ `builderDSLNames` / `switch callee` / 転送アクセサから**別の要素**を削除するので git は片側選択の衝突として提示するが、正解は両方の削除を適用すること。マージ後の rewrite 分岐に残るのは `buildMap` のみ（CALL-006 の担当）。
  - 削除内容:
