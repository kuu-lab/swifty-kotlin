...
- [x] RF-LOWER-CALL-005: `buildSet` の旧runtime rewriteを削除する（前提: CALL-004）
  ...
  - 前提 CALL-004 を待たずに着手した理由: 名義上の前提は未完了だが、依存はファイル局所性による直列化であって意味的依存ではないと判断した。根拠は (1) CALL-001 が `buildSet` の製品到達経路ゼロを実測済み、(2) KSP-697（#6675）がマージ済みで CALL-004 を止めていた `--no-stdlib` の `buildList` stub も消えているため `buildSet` 側の削除が `buildList` の状態に依存しない、(3) CALL-002〜004 のオープンPRが無く「直列化」すべき同時編集が存在しない。CALL-004（buildList）/ CALL-006（buildMap）の分岐と、`isStdlibBuilderDSLCall` / `BuilderDSLLookupNames` の登録口・未消費の `builderLambdaKinds` 配線は手つかずで残している。
  - 削除した製品コード:
    - `+CallRewriteFactories.swift`: `switch callee` の `case lookup.buildSetName:` と `setExprIDs` 登録ブロック。
    - `+LookupTables+BuilderDSL.swift`: `buildSetName` / `kkBuildSetName` / `kkBuildSetWithCapacityName` の宣言・intern と、`builderDSLNames` 集合からの `buildSetName`。`isStdlibBuilderDSLCall` は先頭で `builderDSLNames.contains(callee)` をゲートにしているため、集合からの除去が実質的な無効化。`case` だけ消すと `default: callee` に落ちて自分自身への `.call` + `.copy` を無駄に生むので、集合とcaseは一組で消す必要がある。
    - `+LookupTables.swift`: 対応する転送アクセサ3件。
    - `RuntimeABISpec+Collection.swift`: `__kk_build_set` / `__kk_build_set_with_capacity` の2エントリ。
    - `Sources/Runtime/RuntimeBuilderDSL.swift`: 同2件の `@_cdecl` と private `__kkBuildSet`。
  - ABI export 削除の利用者ゼロ根拠: Kotlin実装（`CollectionBuilders.kt` の `buildSetInternal`）は `__kk_builder_set_new` / `__kk_builder_set_freeze` を使い、`__kk_build_set*` とは別シンボル。CompilerCore 側の emit 元は削除した lookup だけで、`--no-stdlib` では `buildSet` が `KSWIFTK-SEMA-0023` で未解決（CALL-001 実測、KSP-697 後も不変）なので fallback 利用者も無い。`RuntimeSetBox(capacity:)` は `__kk_builder_set_new` が使い続けるため孤立しない。`ABIMismatchTests+RuntimeExportParity` は export→spec / spec→export の双方向検査なので、spec と `@_cdecl` を同時に消せば許可リストへの追記は不要（片方だけ消すと必ず落ちる）。`RuntimeABISpec.specVersion` は `allFunctions` 由来のSHA-256で値は変わるが、消費先は生成Cヘッダのコメントと `RuntimeABISpecVersionTests` の形式・決定性検査のみで、artifact 互換判定には使われていない。
  - テスト置換: `CollectionLiteralLoweringTests` の `testBuildSetRewrittenToKkBuildSet` / `testBuildSetCapacityRewrittenToKkBuildSetWithCapacity` を逆向きの契約（`symbol: nil` の手組み `buildSet` が pass を素通りし `__kk_build_set*` が現れない）へ置換した。`symbol: nil` は旧 `isStdlibBuilderDSLCall` が無条件 true を返していた分岐なので、最も書き換わりやすい形を負の契約で固定している。source / artifact 経路と capacity・負のcapacity・freeze の契約は CALL-001 で追加した `BuilderDSLLoweringRoutingTests` / `CodegenBackendIntegrationTests+CollectionBuilderDSL` が既に担保しており、そちらの `legacyBuilderRuntimeCallees` / `legacyBuilderIRCallees` は `__kk_build_set*` を含めたまま残して再導入の検出器として使う。
  - `builderLambdaKinds` への影響なし: `builderDSLNames` から `buildSet` が抜けることで `scanBuilderLambdaEntries` が集める項目は減るが、この辞書は `+PreScan.swift` で構築 → `CollectionLiteralLoweringRegistry` → `+CallRewrite.swift` → `+CallRewriteFactories.swift` と引数で渡るだけで一度も読まれない（CALL-002 の対象。本PR着手時に再確認済み）。
  - ゲート（変更範囲に絞った実行。全 Swift テスト・全 Golden・全 diff の総ざらいは本PRでは回していない）: `swift build` クリーン。`CollectionLiteralLoweringTests` 68件、`BuilderDSLLoweringRoutingTests` 5件、`CodegenBackendCollectionBuilderDSLTests` 5件（source / artifact の IR と capacity・負のcapacity・freeze の実行）、`RuntimeABIExternalLinkValidationTests` 4件（= `Scripts/validate_runtime_abi_links.sh`）、`ABIMismatchTests` 90件/2 suites（`RuntimeExportParity` の export→spec / spec→export 双方向検査を含む）がいずれもPASS。diff は `collection_builders` / `build_empty_collections` / `ksp950_build_family` / `builder_dsl_shadowing` の4ケースがPASS。
  - `loc_report.sh` 差分: `loc_by_directory Sources` -74行、`Tests` +13行、`.`（TODO.md）+10行、`__kk_cdecl_count` 883→881（削除した `@_cdecl` 2件ぶんの意図的な減少）。`header_helpers_synthetic_total_lines` / `kir_lowering_todo_fixme_count` / `kk_literal_count` / `kk_cdecl_count` / `interner_resolve_literal_comparison_count` / `call_lowerer_legacy_total_lines` は不変で悪化なし。
  - 環境ノート: load average ~180 の飽和状態では `diff_kotlinc.sh` の stdlib artifact ビルドが `COMPILE_TIMEOUT=120s` を超えて `Failed to build stdlib artifact`（stderr 空・`manifest.json` 欠落の部分 `.kklib`）になる。`kswiftc --stdlib-only --emit library` の単体実行は exit 0 / 215秒で成功したので回帰ではない。再現時は artifact を先に作って `--stdlib-library <path>` で渡すとタイムアウトを回避できる。
- [x] RF-LOWER-CALL-006: `buildMap` の旧rewriteと最後のBuilder専用配線を除去する（前提: CALL-005）
  ...
  - 2026-09-13 完了（buildMap スライス）: 前提の再確認で **CALL-001 の「CALL-004 は前提未達」メモが陳腐化している**ことが判明した。CALL-001（#6643、09-07）が rewrite の唯一の実利用者と記録した `--no-stdlib` の `buildList` synthetic stub は、KSP-697（#6675、09-09）が `HeaderHelpers+SyntheticBuilderDSLStubs.swift` ごと削除済みで、`BuilderDSLLoweringRoutingTests.noStdlibHasNoBuildListEntryPoint` は現在「`buildList` が未解決になる」ことを固定している。したがって `__kk_build_*` rewrite は3種すべて製品到達経路ゼロであり、`buildMap` の削除は CALL-005 の完了を待つ必要がなかった。
  - 削除範囲: `+CallRewriteFactories.swift` の `case lookup.buildMapName` 分岐と `state.mapExprIDs` 追跡、`+LookupTables+BuilderDSL.swift` の `buildMapName` / `kkBuildMapName` / `kkBuildMapWithCapacityName` と `builderDSLNames` の登録、`+LookupTables.swift` の転送3件、`RuntimeABISpec+Collection.swift` の `__kk_build_map` / `__kk_build_map_with_capacity`（隣接していないので個別に除去）、`Sources/Runtime/RuntimeBuilderDSL.swift` の同名 `@_cdecl` 2件と共有 `private func __kkBuildMap`。`CollectionBuilders.kt` が実際に呼ぶ `__kk_builder_map_new` / `__kk_builder_map_freeze` は1文字違いなので残置した。`buildMapName` を `builderDSLNames` から外したので、`isStdlibBuilderDSLCall` 冒頭の guard が先に false を返し、switch の `default: callee` に `buildMap` が落ちる経路も消えている。
  - 残置（本項の対象外・条件未成立）: `isStdlibBuilderDSLCall` / `BuilderDSLLookupNames` の登録口自体は、`buildList`（CALL-004・未着手）と `buildSet`（CALL-005・別worktreeで進行中）が利用者として残るため「全利用者がなくなった」条件を満たさない。この撤去は、004・005が着地して利用者ゼロになった時点で **CALL-015 の「使われなくなったlookup」**として扱う（CALL-004・005 の本文はいずれも「分岐・対応lookup・テストのみ」で、この共有登録口を対象に含めていない。005 の進行中差分も自分のarmだけを削って登録口は残していた）。CALL-015 は前提に CALL-006 を含むので依存関係も整合する。`kkMutableSetAddName` / `kkMutableSetRemoveName` は `+CallRewriteCollectionMember.swift` の汎用 mutable Set 操作が使う非Builder名で、`BuilderDSLLookupNames` に同居しているだけなので移設も残置した（005と同一ファイルを整形して衝突させないため）。
  - テスト: `CollectionLiteralLoweringTests` の STDLIB-071 buildMap 2ケース（`symbol: nil` 手組みKIRで旧 rewrite を期待していたもの）を削除し、同じ位置に反転契約 `testBuildMapIsNotRewrittenToRuntimeBuilder(argumentCount: 1/2)` を追加した。`symbol: nil` は旧 `guard let symbol else { return true }` を通る最強の入力なので、`buildMap` が `builderDSLNames` へ差し戻されたら即座に落ちる。`BuilderDSLLoweringRoutingTests.legacyBuilderRuntimeCallees` は「絶対に出現しない」ガードなので6名すべて温存した（名前を削ると保証が弱くなる）。source-backed 契約は同 suite の `sourceBackedBuilderCallsSurviveCollectionLiteralLowering` が6オーバーロード分を既に固定しているため重複追加しない。
  - ゲート: `swift build` green。`--filter BuilderDSLLoweringRoutingTests --filter CollectionLiteralLoweringTests` は2 suite 72テストPASS（新規2ケース含む）。`--filter CodegenBackendCollectionBuilderDSLTests --filter RuntimeABIExternalLinkValidationTests` は2 suite 9テストPASS（source / artifact 双方の LLVM IR 呼び出し先、capacity・負のcapacity・freeze の実行結果、emit名↔spec照合）。`SWIFT_TEST_PARALLEL=0 --filter ABIMismatchTests` は2 suite 90テストPASSで、spec 側と実 export 側を対称に削除したことを parity テストが確認している。`__kk_cdecl_count` は 883→881、`RuntimeABISpec+Collection` のエントリは 198→196 の純減のみ。全 Swift テスト・全 Golden・全 `diff_kotlinc.sh` の総ざらいは実施していない（2026-09-13 のオーナー指示に従い変更関連スコープに限定）。
  - マージ順の注意: `RuntimeABISpec+Collection.swift` の該当ブロックは `list, list_cap, [map], set, set_cap, [map_cap]` の順で、本PR（map 2件）と CALL-005（set 2件）の削除行が**交互に挟まる**。削除行自体は重ならないが hunk が重なるため、後に着地する側は実衝突を解決する必要がある（解決は両方の削除を採るだけ）。同様の隣接削除衝突は rewrite switch・`BuilderDSLLookupNames`・`RuntimeBuilderDSL.swift`（本PRの旧114-152行が005の76-113行に隣接）・`CollectionLiteralLoweringTests.swift`（本PRの挿入位置が005の削除ブロック直後）でも起きる。spec ブロックを解決したら `SWIFT_TEST_PARALLEL=0 --filter ABIMismatchTests` を再実行して parity を取り直す。
  - 検証時の注意（環境起因、本変更とは無関係）: `Scripts/diff_kotlinc.sh` の既定 `--compile-timeout 120` は、共有マシンが高負荷（load 200〜350、他worktreeの並行セッション）だと stdlib artifact ビルド自体を殺し、`stdlib_build.stdout` / `stdlib_build.stderr` が両方0バイトのまま「Failed to build stdlib artifact」で全ケースが落ちる。手動 `kswiftc --stdlib-only --emit library` は成功するが 2分24秒・CPU 29% だった。`--stdlib-library <既存.kklib> --compile-timeout 900 --jobs 0` で回避できる。`--filter` はファイル名ではなく型名で効くため、buildMap のバックエンド検証は `CodegenBackendIntegrationTests+CollectionBuilderDSL.swift` ではなく型名 `CodegenBackendCollectionBuilderDSLTests` を指定する（`CodegenBackendIntegrationTests` を指定すると無関係な巨大 suite を引き込む）。
  - CALL-006 の diff ケース結果: `ksp950_build_family` / `build_empty_collections` / `collection_builders` / `builder_dsl_shadowing`（同一key更新・順序・capacity・負のcapacity・freeze・`map.size` / `map.get` の下流lowering・ユーザー同名 `buildMap`）= 4ケースすべて `Summary: total=1 failed=0 passed=1 skipped=0`（PASS）。`collection_builders` の `map.size` / `map.get("x")` が通ることで、rewrite 内の `state.mapExprIDs` 追跡を削除しても source-backed `buildMap` の戻り値は `seedCollectionExprIDsFromStaticTypes`（LOWERING-001）が Map として分類し、下流の member lowering が維持されることを実測できている。
...
- [x] RF-LOWER-CALL-008: Listの変換・filter系だけをsymbol基準の保持判断へ寄せる（前提: CALL-007）
  ...
  - **前提 CALL-007 は未着手のまま実施した**（PR も無い）。理由: 本項の List 変換・filter 群は、rewrite 側が到達不能になっているため「symbol 基準の保持判断」を新しい分類ヘルパー無しで達成できる — 保持する rewrite が無いので、解決済み Kotlin 宣言は誰も横取りしないという理由だけで残る。CALL-007 は移動のみのPR（KIR/LLVM IR 不変性を根拠にする）として設計されているので、削除を混ぜると検証が壊れ、着手者と衝突する。もし production 経路のどれかが rewrite に到達していたら、その分岐は resolved symbol でゲートする必要があり、それは CALL-007 の分類そのものなので CALL-001 が CALL-004 に対して行ったのと同じ「前提未達」報告にする予定だった。実測では到達しなかった。
  - 到達可能性の実測（本PRの根拠。`collectionHOFRuntimeName(ownerKind: .list, ...)` が nil を返すことが全ての土台）:
    - `StdlibSurfaceSpec.listHOFMembers`（`Sources/RuntimeABI/StdlibSurfaceSpec+ListHOF.swift`）は KSP-421 以降 `forEach` / `firstNotNullOf` / `firstNotNullOfOrNull` / `maxOfOrNull` の4件のみ。`.list` キーはこの配列だけが供給するため、`map*` / `flatMap*` / `filter*` / `flatten` の runtime link は arity 問わず存在しない。
    - `kk_list_map*` / `kk_list_flatMap*` / `kk_list_filter*` / `kk_list_flatten` は Sources 全体で参照0件（`RuntimeABISpec` エントリも `@_cdecl` も既に無い）。したがって ABI export 削除・`validate_runtime_abi_links.sh` / `kk_cdecl_count` への影響は無い。唯一の残存は `CoroutineLoweringPass+CallRewriting.swift:775` のドキュメントコメント中の `kk_list_map` という例示。
    - synthetic stub も無い。`HeaderHelpers+SyntheticListTransformMembers.swift` は KSP-421 / KSP-427 の source 化で当該名の登録を全て落としており、現在登録するのは `sorted` / `shuffled` / `sortedDescending` の3件。`filter` 系は `sourceBackedFilterNames` の skip 経路に残っているが、そもそも登録呼び出しが無い。よって `--no-stdlib` でも「synthetic stub に解決して rewrite される」CALL-004 型のブロッカーは存在しない。
  - 削除したもの:
    - `+CallRewriteHOFTransforms.swift`: `*To` destination 分岐（`mapTo` / `mapNotNullTo` / `mapIndexedTo` / `mapIndexedNotNullTo` / `flatMapTo` / `flatMapIndexedTo` / `associateTo`、`.list` arity 2 lookup）と indexed 1-lambda 分岐（`mapIndexed` / `mapIndexedNotNull` / `onEachIndexed`、`.list` arity 1 lookup）の2ブロック。`associateTo`（CALL-009）と `onEachIndexed` は同じ死んだ lookup を共有するため同時に落ちたが、両者の allowlist エントリは触っていない。
    - `+CallRewriteHandlers.swift`: list 分岐の冗長な `callee != filterName` / `!= filterNotName` 除外（`.list` spec が無いので lookup が既に nil）、`isCollectionHOFMemberName` の `mapNotNull` / `filterNot`（list 分岐も map 分岐も nil で必ず nil 返し）、`listHOFReturnsList` の transform 4名、`mapHOFReturnsList` の `mapNotNull`、`mapHOFReturnsMap` の `filterNot`。`count` の除外は CALL-010 の担当として残置。
    - `+CallRewrite.swift` の `shouldPreserveSourceBackedAggregateCall`: 12名を削除（`mapIndexed` / `mapNotNull` / `mapIndexedNotNull` / `mapTo` / `mapIndexedTo` / `mapNotNullTo` / `mapIndexedNotNullTo` / `flatMapTo` / `flatMapIndexedTo` / `filterNot` / `filterNotNull` / `filterIndexed`）。
  - 残した5名と理由（同名APIの罠。実測で load-bearing を確認済み）: `map` / `filter` / `flatMap` は同じ intern 名が Map レシーバの rewrite（`+CallRewriteHandlers.swift` の `mapHOFRuntimeName` → `kk_map_map` / `kk_map_filter` / `kk_map_flatMap`）を選ぶため、`flatMapIndexed` は Sequence pipeline（`+CallRewriteSequencePipeline.swift:176,224`）、`flatten` は Sequence terminal（`+CallRewriteSequenceTerminals.swift:111`）が同名で rewrite するため。実証: `mapName` だけを allowlist から外してビルドすると `mapOf(1 to "a", 2 to "b").map { (k, v) -> "$k$v" }` の KIR に `kk_map_map` が現れる（戻すと消える）。Map は CALL-012、Sequence は CALL-014 の担当。
  - direct / virtual の非対称: `+VirtualCallRewrite.swift:66-160` にほぼ同一の保持 allowlist があり、そこは今回触っていない（CALL-008 の対象ファイルに含まれず、CALL-007 の対象が「`+VirtualCallRewrite.swift` の対応する保持判断」と明記しているため）。virtual 側の `.list` lookup（同 797 / 1013 行）も同様に nil で死んでいるので、CALL-007 での統合時にまとめて畳める。`+CallRewriteHOFCore.swift:35-77` の list インライン分岐も同じ理由で完全に死んでいる（gate の callee のうち `.list` spec を持つのは `forEach` だけで、それは先に `rewriteCollectionHOFCall` が処理する）が、対象ファイル外かつ CALL-009/010 の名前を含むため残置した。
  - 追加テスト: `Tests/CompilerCoreTests/Lowering/ListTransformFilterPreservationTests.swift`（4 test / 21 ケース）。`listHOFSurfaceHasNoTransformOrFilterEntry` が削除の前提（`.list` surface が空）を固定し、`bundledStdlibLeavesNoSyntheticListTransformOverload` が synthetic 不在を固定。`listTransformFilterCallsSurviveCollectionLiteralLowering` は通常 / 捕捉lambda / indexed / indexed+nullable / nullable要素 / `filterNotNull` / `flatMap` / `flatMapIndexed` / `flatten` / `filter` / `filterNot` / `filterIndexed` / `mapTo` / `mapIndexedTo` / `mapNotNullTo` / `flatMapTo` の16形について、call が resolved symbol を保持し source-backed・非synthetic・externalLinkName なし・`__bundled_kotlin/` 由来であることと、`kk_list_<transform>` 系 callee が出ないことを確認する。`sharedTransformNamesStayPreservedForMapReceivers` が上記5名の残置を守る。
  - 検証（変更が挙動に影響しないことの実測）: 代表49関数のバッテリー（List 通常/捕捉/indexed/nullable/`*To`、Map / Sequence / Array の同名レシーバ、パラメータ型レシーバ）と、allowlist 短絡が外れて steps 5-8 の `classifyTrackedExprByStaticType` 副作用が新たに走る形を狙った9関数のバッテリー（transform 後に runtime bridge 付き API を呼ぶ連鎖・同一レシーバ反復・destination 経由・Map/Sequence/Array 混在・`windowed` / `zipWithNext` / `groupBy` / `associateBy` 後続）で、source 経路の `--emit kir` ダンプが変更前後で完全一致。artifact 経路（`--stdlib-only` で作った `.kklib` を `--stdlib-library` で明示指定して実行ファイルを生成）でも `nm` の `kk_list_*` / `kk_map_*` シンボル集合と実行 stdout が完全一致。
  - ゲート: `swift build` green。`CompilerCoreTests.ListTransformFilterPreservationTests`（4 tests / 21 ケース）、`CompilerCoreTests.Collection*`（84 tests / 8 suites）、`CompilerCoreTests.BuilderDSLLoweringRoutingTests`（5 tests）、`CompilerBackendTests.BuildKIRCodegenRegressionTests`（37 tests）が PASS。`diff_kotlinc.sh` は map/filter/flatten/windowed 関連15ケースを抜き出して total=14 passed=14 failed=0 skipped=1（skip は既存の `ksp687_map_not_null.kt` の SKIP-DIFF）。**未実行**: 全 Swift テスト・Golden 4系統・`diff_cases` 全1200+件・`loc_report.sh`（CI に委ねる）。
  - 発見したバグ（本PRでは未修正、理由付きで TODO に記録）: BUG-252（generic destination へ `mutableListOf()` 直渡しが TYPE-0001）、BUG-257（`windowed(size) { transform }` の戻り型が期待型検査・メンバー解決に伝播せず KIR verifier / TYPE-0001）、BUG-258（`Array<out Iterable<T>>.flatten()` 未実装）。いずれも Sema 型推論層 / stdlib surface の問題で、本PRの「KIR・IR 不変性を根拠に削除する」という検証構造と混ぜられないため分離した。
...
- [x] RF-LOWER-CALL-011: Listのソート・極値系の保持判断を整理する（前提: CALL-010）
  ...
  - 2026-09-13 完了: **前提の CALL-007〜010 は未実施**。専用policy型は存在せず、保持判断は `+CallRewrite.swift` の `shouldPreserveSourceBackedAggregateCall` と `+VirtualCallRewrite.swift` の `shouldPreserveSourceBackedVirtualCall` にインラインのまま。CALL-007 の抽出・判断順序の変更は一切行わず、インラインpolicyに対して本項の範囲だけを実施した。
    - `+CallRewriteHOFExtrema.swift` は KSP-426（#5769）以降 `return false` だけの空スタブだったので、ファイルごと削除し `+CallRewriteHOF.swift` の呼び出しも除去。唯一の利用者が消えた `rewriteHigherOrderCollectionCall` の `function: KIRFunction` 引数も落とした（core / transform / accumulation の3分岐は受け取っていない）。
    - policy の KSP-426 ブロック（direct / virtual 各25名）は **両経路とも到達不能**だった。到達しうる rewrite はすべて外側のメンバー名ゲートの内側にあり、そのゲートがこれらの名前を含まない: `rewriteCoreHigherOrderCollectionCall` 先頭の `if`（`+CallRewriteHOFCore.swift`、`map` / `filter` / `count` 等15名のみ）と `isCollectionHOFMemberName`（`+CallRewriteHandlers.swift`、14名のみ）。この2つのゲートにより `(ownerKind: .list)` / `(ownerKind: .map)` の `collectionHOFRuntimeNames` 参照（`kk_list_maxOfOrNull` / `kk_map_{max,min}ByOrNull`）にも届かない。名前比較を持つ `+CallRewriteSequenceTerminals.swift` も、入口ガードが「source-backed かつ receiver が追跡対象の runtime Sequence handle でない」場合に `return false` するため、bundled stdlib では `sequenceOf` / `generateSequence` / `asSequence` の結果が `sequenceExprIDs` に入らず到達しない。
    - 到達不能の確認方法: 25名を両policyから削除した前後で lowering 後 KIR ダンプを比較し、`list_sort_max_min` / `list_sorted_variants` / `list_max_min_with` / `list_maxby_minby` / `bug161_user_comparable_sorted` / `map_minbyornull` / `map_flatmap_maxby_minby` の7 diff ケースと、range（`1..5` / `1L..5L` / `'a'..'e'` / `1u..5u` の `sorted()`）・Sequence（`sequenceOf` / `generateSequence` / `asSequence().map{}` の `max` / `min` / `maxOrNull` / `minOrNull`）・CALL-014 が名指しする `Sequence` 引数渡し形（`fun pick(s: Sequence<Int>) = s.maxOrNull()`）用の自作ケース4件、計11ケースすべてが **byte-identical**で、`kk_sequence_max*` / `kk_sequence_min*` はどの形でも出現しない。`maxOfOrNull` を単独で外した中間ビルドでも `kk_list_maxOfOrNull` は出現しない。
    - 温存したエントリ: virtual の `sorted`（KSP-453/454 のRange群にある1件。`+VirtualCallRewrite+Range.swift:537` が実消費者）と両policyの `maxByOrNull` / `minByOrNull`（Map群＝CALL-012 の所掌）。後者も同様に到達不能だが、List専用の判断ではないため本項では触っていない。重複記載（direct の `minByOrNull` 2回、virtual の `sorted` 2回）だけ解消した。
    - 未使用になった lookup フィールド19件（`sortedBy` / `sortedByDescending` / `sortedDescending` / `sortedWith` / `maxBy` / `maxOf` / `maxOfOrNull` / `maxOfWith` / `maxOfWithOrNull` / `maxWith` / `maxWithOrNull` / `min` / `minBy` / `minOf` / `minOfOrNull` / `minOfWith` / `minOfWithOrNull` / `minWith` / `minWithOrNull`）を、宣言・`interner.intern` 代入・`commonLookup` 転送アクセサの3箇所ずつ削除。`sorted` / `max` / `maxOrNull` / `minOrNull` / `maxByOrNull` / `minByOrNull` は PreScan・Sequence terminals・Range rewrite・Map分岐に消費者が残るため保持。`CallLowerer+UnresolvedMemberCalls.swift` / `CallLowerer+LegacyMemberLikeCalls.swift` / `HeaderHelpers+SyntheticListAggregateMembers.swift` の同名ローカル変数は別物で無関係。
    - CALL-012 への申し送り: (1) `+CallRewriteHOFCore.swift:82` と `:85` が同一 `if` 条件内で `flatMap` / `maxByOrNull` / `minByOrNull` を二重に列挙している。(2) `kk_map_maxByOrNull` / `kk_map_minByOrNull` / `kk_list_maxOfOrNull` は `RuntimeABISpec` にあるが `Sources/Runtime` に `@_cdecl` を持たない spec-only エントリで、redirect が復活するとリンクエラーになる（`kk_list_sortedBy` だけは実装が残っている）。
    - CALL-015 への材料: このAPI群では巨大allowlistの保護価値が実質ゼロだった。他の群（KSP-421 / 423 / 430 / 658）も同じ外側ゲートで無効化されている可能性が高く、同じKIR不変性チェックで群単位に検証できる。
    - 並行スライスとの関係: 着手時点で CALL-007 は `SourceBackedCallPreservationPolicy.swift` への抽出をコミット済み、CALL-008 / 012 は未コミットで走行中。本項は master 起点のインラインpolicyに対して CALL-009（fold/reduce/scan群）・CALL-010（contains/indexOf/count群）と同一形状の編集（policyエントリ削除＋死んだlookupフィールド削除）に揃えてある。削除対象名は CALL-009 / 010 / 012 と完全に非重複なので隣接行削除同士のunionになる。CALL-007 が先にmergeされる場合は、同じ削除を抽出後のpolicyファイルへ移すだけでよい。
    - 追加テスト: `Tests/CompilerCoreTests/Lowering/ListSortExtremaLoweringRoutingTests.swift`（6テスト: 25名がsource callとして残り旧 `kk_list_*` 19名が0件 / 25名すべて非synthetic・`externalLinkName` 無し・`__bundled_kotlin/` 解決 / 同名ユーザー拡張関数の非書き換え / Map・Sequence・Range receiver が redirect されないこと）、`Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+ListSortExtrema.swift`（5テスト: source / artifact 両経路のLLVM IRに旧 `kk_list_*` が0件、comparator / selector、同順位要素の安定順と極値の先頭優先、null key の降順末尾、空入力の `*OrNull`→null と非null形の `NoSuchElementException`）。実行期待値は同一ソースを kotlinc 2.x でコンパイル・実行して取得した。従来これらの挙動は `continue-on-error` の `diff_kotlinc.sh` ステップだけが見ていた。
    - ゲート（変更に関係する範囲のみ実行）: `swift build` green。`SWIFT_TEST_PARALLEL=0 bash Scripts/swift_test.sh --filter "ListSortExtremaLoweringRoutingTests|CodegenBackendListSortExtremaTests"` で2 suite 11テストPASS。`bash Scripts/diff_kotlinc.sh` を上記10ケース（`list_sort_max_min` / `list_sorted_variants` / `list_max_min_with` / `list_maxby_minby` / `list_sumof_maxby_minby` / `bug161_user_comparable_sorted` / `map_minbyornull` / `map_flatmap_maxby_minby` / `array_sorted_variants` / `maxof_comparable`）に対して実行し 10 PASS / 0 FAIL / 0 SKIP。`Scripts/loc_report.sh` は Sources -111行（+31 / -142）、`HeaderHelpers+Synthetic*` 行数・KIR/Lowering TODO/FIXME 数・`"kk_` リテラル数・`interner.resolve == "..."` 数・`kk_cdecl_count` / `__kk_cdecl_count` はいずれも不変（Runtime・ABI spec は未変更）。`loc_by_directory Tests` は追加テスト636行分の増加のみ。
    - **未実行**: 全Swiftテスト・Golden 4系統・`Scripts/diff_cases` 全件のsweep。2026-09-13 の「動作確認は変更に関係する最小スコープで良い」という指示に従い、変更箇所のフィルタ実行と対象diffケースに限定した。Lowering のみの変更なので Lexer / Parser / Sema / Diagnostics の Golden への影響は構造上ない。
...
- [x] RF-LOWER-STATE-003: PreScanの大量inout引数をstateに集約する（前提: STATE-002、CALL-002・003）
  ...
  - 完了根拠（配線のみ。製品 -106 行 / テスト +31 行、Kotlin 側は無変更）:
    - `collectInitialCollectionExprIDs` の11個の `inout Set<Int32>` を `state: inout CollectionRewriteState` 1個へ畳み、内部helper（`handleCallInstruction` / `classifyFactoryCall` / `propagateCollectionOperation` / `handleVirtualCallInstruction` / `seedCollectionExprIDsFromStaticTypes` / `classifyExprByTypeID`）も同じ形にした。`+PreScan.swift` の `inout Set<Int32>` は 61→1 で、残る1件は `seedULongValuedExprIDsFromStaticTypes` が使う ULong 値の作業集合（分類の種類別集合ではないので state には入れていない）。`charValuedExprIDs` / `ulongValuedExprIDs` も同じ理由で値渡しのローカルに残した。
    - copy伝播の一元化: PreScan の `handleCopyInstruction`（40行）を削除し、`CollectionRewriteState` に17種を1箇所で列挙する `forEachClassification` を置いて、その上に `propagateCopy`（rewrite用・置換）と `seedCopy`（PreScan用・加算）を実装した。種類を追加したとき片方の列挙だけ更新して取りこぼす経路がなくなる（削除前の `handleCopyInstruction` は iterator 系6種を実際に落としていた）。
    - 加算と置換を1つのセマンティクスに統一しなかった理由: PreScan は「静的型シード → 本体の前方走査」の順で走るため、`copy(from: 未分類, to: 静的型で既にシード済みの局所変数)` を置換にすると to のシードが消え、そのcopyより前に位置する命令の分類が変わる。完了条件の「初期分類が不変」に反し、かつ「分類精度の変更はこの配線PRへ混ぜない」に該当するので、実装を共有したまま入口を2つに分けた。契約は `CollectionRewriteStateTests.seedingCopiesAddSourceFactsWithoutDroppingDestinationFacts`（state単体で17種すべて）と `CollectionClassificationTests.seedingKeepsAStaticTypeWhenTheCopiedValueIsUnclassified`（PreScan経由）で固定したので、STATE-002 以降がこれを置換へ倒すなら意図的な変更として現れる。`seedCopy` が iterator 系6種にも作用するのは、PreScan が空の `State()` から始まる（Registry と当該テストの両呼び出し口でそう）ため常に no-op で、この前提は `seedCopy` の doc コメントに書いた。
    - `classifyExprByTypeID` の kind→集合 switch を state の `tag(_:as:)` へ移し、引数を `rawID: Int32` から `expr: KIRExprID` にした（STATE-002 の `KIRExprID` キー化に合わせた向き）。同じ switch は `+StaticTypeClassification.swift:34` と `+VirtualCallRewrite.swift:1146` にも残るが、それぞれ STATE-009 / STATE-004 の対象ファイルなので本PRでは触っていない。
    - 挙動不変の裏取り: base と本PRの `+PreScan.swift` から「種類別の引数宣言・引数渡し・`state.` 接頭辞」を機械的に除去した正規化diffを取り、差分が (1) `handleCopyInstruction` → `state.seedCopy`、(2) `classifyExprByTypeID` の `tag` 化 の2点だけであることを確認した（分類ロジックの本体は1行も変わっていない）。
    - 生成結果: copy / factory / range / CharRange / Sequence / Array / File / Map HOF / iterator を含む collection 系10ケース（`collection_copies`・`collection_constructor_copy`・`list_associate_group_hofs`・`arch_012_int_range_boundary`・`stdlib_kotlin_ranges_CharRange_Companion_Companion_n`・`bug_194_sequence_identity`・`array_conversions`・`file_basic`・`map_hof`・`iterator_builder`）を base `a6d031b066` の `kswiftc` と本PRの `kswiftc` で `--emit kir` 比較し、10件すべてバイト一致。
    - 前提の扱い: STATE-002 未完了のまま着手した。002 の対象は `+RewriteState.swift` と局所テストのみで「互換アクセサを用意し全ファイルの一括置換は行わない」と規定されているため、003 が配線を1引数へ畳んだ分だけ 002 が触る呼び出し口が減る（逆方向の技術的依存はない）。CALL-002・003 は同一ファイルの衝突回避のための順序指定で、CALL-003（#6758）は `isStdlibBuilderDSLCall` 末尾、CALL-002 は `collectBuilderLambdaKinds` / `scanBuilderLambdaEntries` と Registry の `builderLambdaKinds` 構築が対象。いずれも本PRの編集範囲（`collectInitialCollectionExprIDs` 以降、Registry の `collectInitialCollectionExprIDs` 引数列）と行が重ならないが、Registry では隣接hunkになりうる。衝突時の正しい解決は union（CALL 側の削除 + 本項の `state: &state`）で、どちらかを捨てない。
    - 観察（後続向け、本PRでは未修正）: `+PreScan.swift` の `classifyExprByTypeID` と `+StaticTypeClassification.swift` の `classifyTrackedExprByStaticType` は役割が重なるが、後者だけが `makeNonNullable` + `resolveClassTypeSymbol` を通し、前者だけが `.stringStruct` を扱う。統合は分類精度の変更になるので STATE-009/010 の判断に残した。`seedCollectionExprIDsFromStaticTypes` の「既分類ならskip」ガードは静的シードが最初に走る現状では常に false で、除去は走査回数削減（010）の範囲。
    - 検証（最小スコープ）: `swift build`（エラー0件）と `SWIFT_TEST_PARALLEL=0 bash Scripts/swift_test.sh --filter CollectionRewriteStateTests --filter CollectionClassificationTests --filter CollectionLiteralLoweringTests`（3 suites / 81 tests PASS）。**未実行**: 全Swiftテスト / `--filter Golden` / `bash Scripts/diff_kotlinc.sh Scripts/diff_cases` — 正規化diffとKIRバイト一致で挙動不変を押さえたのでCIに委ねた。`Scripts/loc_report.sh` の変動は `loc_by_directory Sources` -106 行と `Tests` +31 行のみで、`HeaderHelpers+Synthetic*` 行数・`kir_lowering_todo_fixme_count`・`kk_literal_count`・`interner_resolve_literal_comparison_count`・`kk_cdecl_count` / `__kk_cdecl_count` はいずれも不変。
...
- [~] KSP-CAP-018: object 式によるクラス継承を通す（= BUG-215）。ブロック対象: KSP-441（object 式でパイプラインを表現する方針）。2026-09-13 に本項目の2症状（空ボディ含む）まで実装・検証済み、マージ後に [x] 化。下記「未解消」に残る2件は本項目の完了条件外: (1) 2026-09-13 に発見した object 式プロパティ delegation の未実装 (2) 名前付き `object : Base(x)` 宣言（当初からスコープ外）
  ...
  - **追加修正4（2026-09-13）**: object 式の custom accessor（getter/setter）が正しく動かない問題を解消。当初は「Sema が accessor 本体を型検査していない」1点と記録していたが、実測で**5層**に分かれていた:
    ...
    **当初想定の訂正**: 「KIR で継承プロパティを読めるようにする」作業は不要だった。継承プロパティのオフセットは object 式の `NominalLayout` に継承済みで、(a) で識別子が束縛されれば既存の汎用経路がそのまま解決する（`object : Ticker(4) { val doubled: Int get() = step * 2 }` を明示レシーバで読むケースで実証）。
    回帰: `Scripts/diff_cases/object_literal_class_inheritance.kt` に sibling/継承プロパティ読み・別ストレージ setter・`field` 経由 setter・accessor 本体からの capture（直接／ラムダ内）を追加（`diff_kotlinc.sh` PASS）、`CodegenBackendIntegrationTests+ObjectLiteralClassInheritance.swift` に対応する3テストを追加（16/16 PASS）。`testBuildKIRObjectLiteralCustomGetterUsesAccessorCall` は callee 名しか見ておらずこのバグを長年見逃していたため、発行された accessor 本体がインスタンスを読むこと・`.unit` 定数を含まないことを検証するよう強化した。
  - **未解消（別途対応が必要、本 PR のスコープ外）**:
    1. **（2026-09-13 発見・未修正）** object 式のプロパティ **delegation**（`object { val x by lazy { 42 } }`）が両方向とも未実装。`lowerObjectLiteralPropertyAccessors` は delegate プロパティの accessor を発行せず、delegate 式自体もどこにも格納されない（`lowerStoredObjectLiteralExpr` は `propertyDecl.initializer` のみを走らせる）。明示レシーバ読みは `_get` 未定義の `KSWIFTK-LINK-0001`、暗黙レシーバ読みも（上記 (c) で述語を共有化した結果）同じくリンクエラーになる。master 時点から壊れており（明示側はリンクエラー、暗黙側は無言の 0）、bundled stdlib には該当箇所ゼロ。明示側は失敗のまま維持、暗黙側は本 PR で**無言の 0 → リンクエラーに変わった（意図的）**: 静かな誤値より大声の失敗が良いと判断し、(c) の述語共有化で accessor 発行条件と読み経路の判定を一致させた副作用。object 式内の `by` を踏むテスト・diff ケースは存在しないため回帰は出ていない。
...
- [x] KSP-1543: real `ProducerScope` を用いた channelFlow/callbackFlow の (b) 実装を設計・実装する（KSP-686 完了メモの残課題として記録されたまま未起票だった — 2026-08-19 起票。当初 KSP-1542 として起票したが、master 側の別コミットが同番号を別内容〔`HeaderHelpers+SyntheticCollectionTypeFallbacks.swift` 整理〕に使っていたためマージ時に 1543 へ採番し直した）。2026-09-04 実装: source-backed `ProducerScope`/`SendChannel`/`ChannelResult`、channelFlow/callbackFlow の cold channel-backed runtime、receiver-first launcher/capture ABI、`trySend`/`close` と Runtime ABI 登録、Sema/Runtime/diff 回帰を追加し、`flow_builders.kt` の `SKIP-DIFF` を解除した。focused Sema/Runtime/ABI/`flow_builders.kt` parity は PASS。
  - 完了根拠（2026-09-13）: 実装は #6597（`3ed834624d`, 2026-09-09 merge）として master 済み。共通ゲート G は CI 実績で green を確認 — #6597 の全 18 チェックが SUCCESS（`Verification 1/5 (CompilerCore and Smoke, shard 1〜6/6)` = `swift_test.sh` の CompilerCore/Smoke 群と Golden 4 スイート（`KSWIFTK_GOLDEN_SHARD_*` 経由）を含む / `Verification 2-3/5 (Backend, Runtime, CLI, and LSP, shard 1〜4/4)` / `Verification 5/5 (kotlinc Diff, shard 1〜4/4)` = `diff_kotlinc.sh Scripts/diff_cases` 相当 / `Verification 4/5 (Repository Checks)` / `Verification 0/5 (TODO ID duplicate check)`）。kotlinc diff ステップ自体は `continue-on-error: true` だが、後続の "Fail when kotlinc diff regression mismatches exist" が `outcome == 'failure'` で job を落とすため、job SUCCESS は diff green を意味する。回帰有無の追加確認として、直近 merge 済み PR #6749 の head（`b6c449a7f7`。base は `a6d031b066` = KSP-1543 merge から 112 コミット後の master、差分はドキュメントのみ）でも同じ 18 チェックが失敗ゼロで green だった。なお master ブランチ自体を対象にした CI run は nightly fuzzer のみで（これも直近4回 success）、`c6826127ff` を直接対象にした full gate run は存在しない。
  - 検証: 上記 CI ゲート green の確認と、`Scripts/diff_cases/flow_builders.kt` の `SKIP-DIFF` 解除・source-backed 宣言（`Stdlib/kotlin/coroutines/channels/ProducerScope.kt`, `Stdlib/kotlinx/coroutines/flow/Builders.kt`）・Runtime ABI 登録（`RuntimeABISpec+Coroutine.swift`）・回帰テストの現存確認（`CompilerCoreTests.FlowSemaTests` と `RuntimeTests.RuntimeChannelTests` は channelFlow/callbackFlow/ProducerScope/trySend を直接参照する回帰として grep で現存を確認。`CompilerCoreTests.RuntimeABIExternalLinkValidationTests` への #6597 の変更は suspend continuation ABI のアリティ検証拡張で、channel 系シンボル名の直接参照はない — 差分から確認）。**ローカルでの全 Swift suite / Golden / `diff_kotlinc.sh` 再実行は未実施**（共有マシンが load average 174 の高負荷だったため。同一ツリーに対する CI 全ゲート green を証拠として採用）。姉妹台帳 `docs/stdlib-pipeline.md` §9 の KSP-686 (a) 分類注記も本コミットで更新。
...
- [ ] BUG-252: generic な destination パラメータ（`C : MutableCollection<in R>`）へ引数リテラルとして `mutableListOf()` を直接渡すと、入れ子呼び出しの型引数が外側の制約集合に参加せず `Nothing` に先に確定して失敗する。最小再現: `fun main() { val dest: MutableList<Int> = listOf(1, 2).mapTo(mutableListOf()) { it * 2 }; println(dest) }` —— kotlinc（2.4.10）は `[2, 4]` を出力するが、本コンパイラは `error KSWIFTK-TYPE-0001: Type constraint could not be satisfied.`。stdlib 非依存の最小形でも再現し、そちらは原因が直接読める: `fun <T, C : MutableCollection<in T>> collectInto(dest: C, value: T): C { dest.add(value); return dest }` に対し `collectInto(mutableListOf(), 1)` が `Conflicting bounds for type variable #1: inferred Class#185<Nothing> is not a subtype of Class#185<Int>. lower=[Class#185<Nothing>], upper=[Class#185<Int>]`。`mapTo` / `filterTo` / `mapNotNullTo` など destination を取る API 全般に及ぶ。`mutableListOf<Int>()` と明示するか、いったん `val dest = mutableListOf<Int>()` に束縛すれば通る。`-Xnew-inference` / `-Xproper-type-inference-constraints-processing` のいずれを付けても解消しない。発見元: RF-LOWER-CALL-008 の到達可能性バッテリーで `mapTo(mutableListOf())` を書いてコンパイルが落ちた。今回修正しない理由: 入れ子呼び出しの型引数を外側の制約解決まで型変数として保留する変更は ConstraintSolver の設計レベルの修正で、BUG-236 / BUG-237 と同じ型推論層に属する。lowering の保持判断整理（KIR/LLVM IR の不変性を根拠にする PR）のスコープを超えるため、§13-9に従って追跡する。

- [ ] BUG-257: `Iterable<T>.windowed(size) { transform }`（`step` / `partialWindows` をデフォルトに任せる形）の戻り型が、期待型検査とメンバー解決に伝播しない。実行値は正しい（`listOf(1,2,3).windowed(2) { it.sum() }` は kotlinc と同じ `[3, 5]`）のに、静的な型が付く文脈で2通りに壊れる。最小再現(a): `fun f(xs: List<Int>): Int = xs.windowed(2) { it.sum() }.size` → `error KSWIFTK-KIR-0003: KIR verifier: f: call to 'size' does not resolve to a module function, an external link name, or a runtime ABI function`。最小再現(b): `fun f(xs: List<Int>): Int { val ys: List<Int> = xs.windowed(2) { it.sum() }; return ys.size }` → `error KSWIFTK-TYPE-0001`。`xs.windowed(2, 1, false) { it.sum() }` と値引数を全部明示すれば両方通り、`val ys = xs.windowed(2) { it.sum() }`（期待型なし）も通る。つまりデフォルト引数を省略した trailing lambda 形だけで `List<R>` の R 置換が落ちている。宣言は `Sources/CompilerCore/Stdlib/kotlin/collections/ListWindowChunk.kt:60,71` の2 overload。`Scripts/diff_cases/list_windowed_transform.kt` は PASS しており、型付き文脈の形を含まないため既存 diff では露出しない。発見元: RF-LOWER-CALL-008 の副作用検証バッテリーに `indexed.windowed(2) { it.sum() }.size` を入れて KIR verifier が落ちた。今回修正しない理由: デフォルト引数を伴う overload 解決の戻り型置換は Sema の型検査層の修正であり、当該 PR（lowering の保持判断整理）のスコープ外。なお BUG-250（関数型プロパティ呼び出し）も `KIR verifier: call to '...' does not resolve` で失敗するため、「解決できなかった呼び出しが Sema を素通りして KIR verify まで届く」という共通の機構を持つ可能性がある（トリガーは別）。

- [ ] BUG-258: `Array<out Iterable<T>>.flatten()` が bundled stdlib に無く、`arrayOf(listOf(1, 2), listOf(3)).flatten()` が `error KSWIFTK-SEMA-0024: Unresolved member function 'flatten'.` になる。現在の宣言は `Iterable<Iterable<T>>.flatten()`（`Sources/CompilerCore/Stdlib/kotlin/collections/Iterables.kt:222`）、`List<List<T>>.flatten()`（`ListHOF.kt:109`）、`Sequence<Sequence<T>>.flatten()`（`sequences/SequenceTransformHOF.kt:369`）の3つで、`Array` は `Iterable` を継承しないため専用 overload が必要。kotlin.collections には `Array<out Iterable<T>>.flatten(): List<T>` と `Array<out Array<out T>>.flatten(): List<T>` がある。発見元: RF-LOWER-CALL-008 のバッテリーで Array レシーバの同名 API を並べたときに解決失敗。今回修正しない理由: stdlib surface の追加は `docs/stdlib-pipeline.md` §13 のブリッジ入場審査・粒度ルールに従う KSP 系タスクの担当であり、lowering リファクタ PR で surface を足すのは移行ガバナンス上不適切。番号注記: 当初 BUG-250〜252 として起票したが、`master` に並行マージされた BUG-250（関数型プロパティ呼び出し）と BUG-251（UByte/UShort の Number 誤適合）との二度の番号衝突で 252〜254 へ繰り上げ、さらに BUG-253（KSP-CAP-004 の非ローカル break/continue）と BUG-254（KSP-699 の set factory `is` 判定）との衝突で本エントリは 258 へ振り直し（同バッテリーの windowed 戻り型バグは BUG-257）。
...
- [ ] RF-FIXTURE-015: `file_use_lines.kt` を useLines のラムダ / 戻り型推論に絞る（17行 / 8回）
  ...
  - 実行先は `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+FileUseLinesForEachLine.swift`（旧 `Scripts/diff_cases/file_uselines.kt` は CLEANUP-STUB-107 でセットアップが `File.writeText()` に依存していたため削除済み）。count / 行走査は既存だが、List を返して useLines の外で使うケースは今回照合した実行先にないため補完する。
