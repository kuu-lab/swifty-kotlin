...
- [ ] BUG-046: `launch { }` の子コルーチン本体が同期的な `cancelJob.cancel()` より先に実行される起動-キャンセル競合。`RuntimePendingLaunchQueue`/`RuntimeCoroutineBurstDepth` による deferred dispatch の根本修正を PR #4837 に実装済み（`runSuspendEntryLoopWithContinuation` の次の yield/completion まで遅延し、直後の `cancel()` を先着させる）。検証: 対象テスト12回連続パス、関連コルーチン/runtimeテスト157件0失敗。発見元: PR #4578 のCI診断。修正PR: #4837（マージ後に `[x]` 化）
