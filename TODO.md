...
- [x] RF-LOWER-CALL-008: Listの変換・filter系だけをsymbol基準の保持判断へ寄せる（前提: CALL-007）
  ...
  - 発見したバグ（本PRでは未修正、理由付きで TODO に記録）: BUG-252（generic destination へ `mutableListOf()` 直渡しが TYPE-0001）、BUG-253（`windowed(size) { transform }` の戻り型が期待型検査・メンバー解決に伝播せず KIR verifier / TYPE-0001）、BUG-254（`Array<out Iterable<T>>.flatten()` 未実装）。いずれも Sema 型推論層 / stdlib surface の問題で、本PRの「KIR・IR 不変性を根拠に削除する」という検証構造と混ぜられないため分離した。
