...
- [ ] BUG-141: `swift build` が XCBuild バックエンド（macOS + Xcode-beta 系ツールチェインで発生。中間ディレクトリ名が `<Target>-t.build/Objects-normal/<arch>/` 形式になる構成）でランタイムを再ビルドすると、`CodegenRuntimeSupport+RuntimeObjects.swift` の `collectObjectPaths` が `*.swift.o` サフィックスのみを走査するため、実際に生成される `RuntimeXxx.o`（`.swift.o` ではなく `.o`）を発見できず `KSWIFTK-LINK-0001: Unable to locate packaged runtime object files` でリンク失敗する（`swift build --target Runtime` 自体は exit 0 で成功し `.o` は生成されている。Linux 上のネイティブ SwiftPM ビルドシステムでは `.swift.o` 命名になるため CI では再現しない）。最小再現: 該当構成のローカル macOS で `bash Scripts/diff_kotlinc.sh --clean-runtime-cache Scripts/diff_cases/string_indent.kt`（内容非依存、`.runtime-build/` を伴うどの diff_cases ケースでも再現する想定）— PR #4598 の CI 失敗（StringIndentFunctionTests）調査中、ローカル kotlinc diff 回帰検証で発見。BUG-036 の「Xcode ツールチェイン不一致」所見と同根の疑い
- [ ] BUG-142: CI ジョブ `Full Swift Tests (RuntimeTests)` が `swift test --filter '^RuntimeTests\.'` 実行時に `error: posix_spawn error: Argument list too long (7)` で確定的に失敗する（`RuntimeTests` は `Tests/RuntimeTests/*.swift` 全体で `func test` が3171件あるのに `CompilerCoreTests`/`CompilerBackendTests` と異なり `Scripts/shard_swift_tests.sh` によるシャーディングが一切されていない）。**PR 固有ではなくリポジトリ全体の問題**: `master` の直近4回の CI（2026-07-15 07:27〜12:40 UTC）でも同一ジョブが同一メカニズムで確定的に失敗している。commit `804b5ba151`（「Serialize RuntimeTests CI execution」、`SWIFT_TEST_PARALLEL=0`/`workers=1`化）はこれを解決しない — むしろ悪化させる: `--no-parallel` はワーカー分割なしで全マッチテストを **1回の posix_spawn に丸ごと**渡すため、実測で 2975件超のテスト ID を含む単一引数（231KB 超、Linux `MAX_ARG_STRLEN` の 128KB を大幅超過）が生成され、並列モード（4ワーカー分割、1回あたり約793件）より条件が悪くなる。修正は sharding と serialization の両立が必要: `Scripts/shard_swift_tests.sh --mode static --tests-dir Tests/RuntimeTests --target-prefix RuntimeTests`（`RuntimeTests` は XCTestCase と `@Suite` が混在するため `--mode dynamic` は使用不可、CompilerCoreTests と同じ static モードが必要）で `full-swift-tests` から `RuntimeTests` を切り出してシャード化しつつ、各シャード内では `SWIFT_TEST_PARALLEL=0` を維持して共有ランタイム状態のテスト分離を保つ — PR #4598 の CI 監視中に発見。fix 実装は同 PR で対応
