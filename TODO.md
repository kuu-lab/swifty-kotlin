...
- [ ] BUG-136: `sequence { for (i in 1..5) { yield(i * i) } }` が CPS sequence-builder 経路で最初の要素しか返さない（再現: `Scripts/diff_cases/sequence_lazy_eval.kt` / `CodegenBackendIntegrationTests.testSequenceBuilderRangeLoopYieldUsesCPSProducer`）。`CoroutineLoweringPass+CallRewriting.swift` が内部 continuation 生成と direct-suspend relay を含む adapter を `kk_sequence_builder_build_coro` へ通すと発生。発見元: PR #4733 の CI failure。修正: PR #4733。
