...
- [x] BUG-145: `Tests/RuntimeTests/ABIMismatchTests+RuntimeExportParity.swift` の3テスト（`testRuntimeExportsHaveMatchingRuntimeABISpecEntries`/`testRuntimeExportSignaturesMatchRuntimeABISpec`/`testSpecOnlyRuntimeABINamesAreExplicitlyAllowed`）が、`Sources/Runtime/**`（`@_cdecl` export）と `Sources/RuntimeABI/RuntimeABISpec+*.swift`（spec 宣言）の不一致で恒常的に failed。`SWIFT_TEST_PARALLEL=0`（フル直列実行）でも再現するため並行性由来ではなく、正真正銘の spec 未更新。missing 側は Base64（`kk_base64_*` 全般）・charset/bytearray decode（`kk_bytearray_decodeToString*`, `kk_charset_*`）・list 変形（`kk_list_chunked`, `kk_list_windowed`, `kk_list_zip*`）・例外コンストラクタ群（`kk_illegal_argument_exception_new` 等 15 型）・array joinToString（`kk_byteArray_joinToString` 等 8 型）・coroutine scope（`kk_coroutine_scope_launch`, `kk_supervisor_job_new` 等）・`kk_itable_lookup_dynamic` など約80件。spec-only側（export 側に実体が無いのに spec にだけ残っている、または allowlist 未整備）も同数規模で存在（`kk_base64_*` の instance/mime/urlsafe バリアント等）。TODO.md 上の KSP-619/643/660/679/682（例外・array joinToString・coroutineScope wrapper 等の Kotlin 化タスク）や Base64/charset 関連の最近のマージ群が候補だが、単一コミットに絞り込めておらず要調査（複数 PR の累積の疑い）。**重要**: この drift は `Full Swift Tests (RuntimeTests)` 単体ジョブの下に隠れており、同ジョブは別途 Swift Testing 移行由来の並行性クラッシュ（BUG-021〜035/039 系、2026-07-15 時点で master 自身の直近 CI 3 連続 failure の原因）で頻繁に赤くなるため、「RuntimeTests が落ちている＝既知のフレーク」という前提でレビュー・マージされ見逃されてきた可能性が高い。発見元: PR #4828 の CI 修正作業中、ローカルで `shard_swift_tests.sh --mode static --tests-dir Tests/RuntimeTests --target-prefix RuntimeTests` の shard 0/6 を `SWIFT_TEST_PARALLEL=0` で実行して発見（`swift build` 完全リビルド後も再現、staleビルドではないことを確認済み）。現行masterへ取り込まれた各修正により、#4847ブランチの3 parity testは2026-07-19時点で全てpassしているため追跡を完了する。
