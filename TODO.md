...
- [ ] BUG-046: `launch { }` の子コルーチン本体が、直後（介在する suspension point なし）の同期的な `cancelJob.cancel()` より先に実行を開始してしまうことがある、`kk_kxmini_launch`（`Sources/Runtime/RuntimeCoroutine.swift`）の起動-キャンセル競合。原因: `KxMiniRuntime.launch(workItem:)` が `DispatchQueue.global()`（真の並行スレッドプール）へ即座に dispatch するため、子スレッド側 `job.markStarted()` と親スレッド側 `job.cancel()` の `NSLock` 獲得順序に確定的な happens-before 保証がない。本家 kotlinx.coroutines は `runBlocking` 直下の `launch` を単一スレッドの協調的 event loop で動かすため、suspension point を跨がない `cancel()` は子の実行開始を確実に防げるが、本実装は実スレッド並行に依存しており、子スレッドが `markStarted()`→`cancellationSnapshot()` チェックを親の `cancel()` より先に通過すると本体が走り出し、`delay()` 到達後に協調的キャンセルで中断されて期待外の出力（例: catch節の "cancelled"）が漏れる。再現: `Scripts/diff_cases/coroutine_cancellation_edge_cases.kt` / `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+CoroutineCancellationEdgeCases.swift` の `testCodegenCompilesCoroutineCancellationEdgeCases`（PR #4830 で追加）。CI高負荷時に発現（本件は PR #4578 の `CompilerBackendTests shard 3/6`、`Actions run 29439186351/job 87460299566` で観測）。頻度は未確定 — ローカルでの繰り返し再現を試みたが、本開発機で他セッションの並行ビルドが多数走っていたことによるリソース競合とみられる無関係な `LinkPhase.swift:109` の `"outputUnavailable"` エラーに阻まれ、対象のレース自体をクリーンに再現できなかった（1回だけ `cancelJob.join()` 待ちとみられる無限待機/0%CPU sleepingも観測したが、上記のリソース競合下での観測のため本バグと確定できない）。低負荷環境での再現率は改めて要検証。同種バグは別セッションでも独立発見・BUG-039 として記録されていたが（`Log coroutine cancel-before-start timing race as BUG-039`, commit `1f925b36d4`）、その後 master 側で BUG-039/040 が全く別内容（RuntimeTests CI の cross-suite GC race / exec引数長制限）で確定したため番号が競合し、本項目として BUG-041 に採番し直した。発見元: PR #4578 の CI 診断中（#4830 自体の変更ではなく `kk_kxmini_launch` の既存 dispatch モデルに起因する既存バグ）。修正: `Sources/Runtime/RuntimeCoroutine.swift` に `RuntimePendingLaunchQueue`/`RuntimeCoroutineBurstDepth` を追加。`kk_kxmini_launch`/`kk_kxmini_async_with_cont`/`kk_kxmini_launch_with_exception_handler` の実 dispatch を、呼び出しスレッドが `runSuspendEntryLoopWithContinuation` バースト内（コンパイル済み Kotlin コルーチン本体の実行中）にいる場合のみそのバーストの次の yield（suspend/completion、`RuntimeJobHandle.join()`/`RuntimeAsyncTask.awaitResult()` のブロッキング待機直前を含む）まで遅延し、cancel() が dispatch 前に先着することを保証。バースト外からの直接呼び出し（ホワイトボックステスト等、次の yield が存在しない文脈）は従来通り即時 dispatch にフォールバックし、`testKxMiniLaunchRunsSuspendEntryAsynchronously` 等 3件の回帰を回避。検証: 対象テストをローカルで計12回連続パス確認（クリーンな環境下）、`RuntimeCoroutineStateTests`/`RuntimeCoroutineAdvancedTests`/`JobComprehensiveTests` 等コルーチン関連テスト157件は0失敗。修正 PR: #4837（マージ後に `[x]` 化）
- [ ] BUG-132: `===`/`!==`（identity equality 演算子）が Lexer レベルで丸ごと未実装 — Sema の型検査以前の問題で、interface 型に限らず全ての型で発生。`Symbol` enum に3文字トークンが無いため `===` は貪欲マッチで `==`＋`=` に誤分割され、パーサが `=` 以降を静かに読み捨てる。型注釈なし文脈（`val x = a === b`）では黙って誤ったコンパイル結果（`Boolean` 期待箇所に生ポインタ値が入る）になり、明示的型注釈がある文脈でのみ `KSWIFTK-TYPE-0001`（Type constraint could not be satisfied）として顕在化する。最小再現:
