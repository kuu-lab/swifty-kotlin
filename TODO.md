...
- [ ] BUG-040: `swift build`/`swift test` が SwiftPM の新「Swift Build」ビルドシステムを選択する環境（`.build/.buildSystem_debug` の中身が `swiftbuild`。新しめの Xcode ツールチェインでデフォルト化しうる）で、リンク・実行を伴うテスト/コマンドが軒並み `KSWIFTK-LINK-0001: Failed to build packaged runtime objects` ないし `Unable to locate packaged runtime object files under .../<triple>/debug/Runtime.build` で失敗する。原因は `Sources/CompilerBackend/CodegenRuntimeSupport+RuntimeObjects.swift` の `discoverRuntimeObjectPaths`/`runtimeBuildDirectory` が、旧来ビルドシステムの `<triple>/debug/Runtime.build/*.o` ディレクトリ命名のみを探索しており、新ビルドシステムが生成するフラットな `out/Products/Debug/Runtime.o` 配置(かつ `<triple>` サブディレクトリ自体が存在しない)に対応していないため。最小再現: `.build/.buildSystem_debug` が `swiftbuild` の環境で `bash Scripts/diff_kotlinc.sh Scripts/diff_cases/hello.kt`、または `DEVELOPER_DIR=/Applications/Xcode-beta.app/Contents/Developer bash Scripts/swift_test.sh --filter CompilerBackendTests.AnyTypedLocalVarBoxingTests/testAnyAndNumberTypedLocalsAreBoxedCorrectly` を実行。DEADCODE-CORE-004 の作業中に発見。フロントエンド(`CompilerCore`)のみを触るコード変更とは無関係であることを、削除前後で同一テストが同一エラーで失敗することを確認しベースライン比較済み。CompilerCoreTests/RuntimeTests/Golden は本問題の影響を受けない(リンク・実行を伴わないため)
