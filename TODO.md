...
- [x] MIGRATION-TEXT-009: `commonPrefixWith` / `commonSuffixWith` を Kotlin source に移行する。2026-07-06: `StringComparison.kt` は implicit stdlib として読み込まれ、KIR direct lowering / TypeCheck fallback / `RuntimeABISpec` / `RuntimeStringHOF.swift` / `RuntimeStringFlat.swift` の `kk_string_common*` direct runtime 経路を削除済み。
...
- [x] RF-STDLIB-004: E2E 縦切り第1弾: `StringComparison.kt` の `commonPrefixWith`/`commonSuffixWith` をパイプライン実配線し、対応する TypeCheck フォールバック + KIR direct lowering + runtime `@_cdecl` を同一 PR で削除する（以後の移行のテンプレート）
...
- [x] KSP-201: StringComparison 縦切り（`commonPrefixWith`/`commonSuffixWith`）[RF-STDLIB-004]
  ...
  - 削除: `CallTypeChecker+MemberCallInferenceRegularNoCandidateFallbacks.swift` の `commonPrefixWith`/`commonSuffixWith` 特例 3 箇所（`rg -n 'commonPrefixWith' Sources/CompilerCore`）/ `CallLowerer+StringStdlibMemberCalls.swift` と legacy lowering の同 case / `NativeEmitter+FunctionEmission.swift` の flat-return special call / `RuntimeStringHOF.swift` と `RuntimeStringFlat.swift` の `kk_string_common*` entrypoint / `RuntimeABISpec.swift` の同 flat ABI エントリ
