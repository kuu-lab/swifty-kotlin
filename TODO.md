...
- [x] DEBT-CORO-005: `launch { ... }` 直後の `cancel()` で、開始前キャンセルされるべき child body が先に実行される scheduling race を調査・修正する。最小再現: `runBlocking { val job = launch { try { delay(Long.MAX_VALUE) } finally { println("finally") } }; job.cancel(); job.join() }` は JVM kotlinc では `finally` を出力しないが、kswiftc では出力する場合がある。PR #4632 の `kotlinc Diff Regression Shard (1/11)` で発見。該当 diff case は child を明示的に開始してから cancel する決定論的な形へ修正済み。**本タスクは後続で BUG-041 として詳細採番され、その修正 commit `3ee8504c9`（"Fix coroutine launch cancel-before-start race"、`RuntimePendingLaunchQueue` 導入）で解決済み・master にマージ済み。** 検証（2026-07-23、`.build/debug/kswiftc` を実ビルドして最小再現を実行）: 上記 `finally` 版を 20/20 回実行して常に `done` のみ出力（`finally` は一度も出力されず JVM kotlinc と一致）。回帰テスト: `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+CoroutineCancellationEdgeCases.swift` の `testCodegenLaunchCancelBeforeStartSkipsFinally`（`finally` 版）と既存 `testCodegenCompilesCoroutineCancellationEdgeCases`（`catch` 版）、diff case `Scripts/diff_cases/coroutine_launch_cancel_before_start_finally.kt` / `coroutine_cancellation_edge_cases.kt`。詳細は BUG-041 を参照
...
- [x] KSP-404: prefix/suffix 系を Kotlin 化（`startsWith`, `endsWith`, `removePrefix`, `removeSuffix`, `removeSurrounding`） (PR #5019)
...
- [x] KSP-405: take/drop 系を Kotlin 化（`take`, `takeLast`, `drop`, `dropLast`, `takeWhile`, `dropWhile`, `takeLastWhile`） (PR #5019)
