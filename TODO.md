...
> 2026-07-06 棚卸し: [`docs/diff-skip-inventory.md`](docs/diff-skip-inventory.md)。現時点の `DEBT-DIFF-*` タグ付き skip は 68 件（001:23 / 002:8 / 003:14 / 004:5 / 005:15 / 006:3。001 は #4575（2026-07-08、`uuid_basic.kt` 追加）で 22→23、005 は inventory 本体の詳細表に元々 15 件列挙済みだったため 14→15 へ補正——いずれも要約行のみ更新漏れだった）。各タスクの完了条件は、該当ケースを通常 `diff_kotlinc.sh` に戻すか、JVM kotlinc を oracle にできない理由と代替 runner / unit test owner を同文書へ移すこと。2026-07-09、DEBT-DIFF-002 の一部（top-level functions / custom declarations 4件）を実測して timeout 無関係と確定し、新たに DEBT-DIFF-007 / 008 の2バグを発見・分離した（計 70 件）。
