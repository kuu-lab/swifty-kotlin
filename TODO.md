...
- [~] RF-STDLIB-002: `LoadSourcesPhase` に bundled Stdlib ソース読み込みを実装する（`Bundle.module` 列挙 → `sourceManager` 登録は基本配線済み。2026-07-15 監査で残リストを補正: ユーザー入力との診断パス区別は `__bundled_` 接頭辞機構で実装済みのため残から除外。残: `--no-stdlib` の実配線（BUG-043 — フラグはパースされるが未参照の死にフラグで、`injectBundledStdlib` は無条件実行。実装タスクは技術負債バックログ側にも独立起票あり）、形式的な source origin 型の導入）
...
- [ ] BUG-043: `--no-stdlib` フラグが黙って無視される。`Sources/KSwiftKCLI/CLIParser.swift:126` でパースされ help テキスト（同 `:36`「Disable automatic stdlib inclusion」）にも記載されるが、格納先の `CompilerOptions.includeStdlib` は Sources 全域で一度も参照されず、`Sources/CompilerCore/Driver/FrontendPhases.swift:94` の `injectBundledStdlib` が無条件実行されるため bundled stdlib は常に注入される（`docs/stdlib-pipeline.md` §4-3 の「opt-out できる」記述とも不整合）。最小再現: `.build/debug/kswiftc --no-stdlib hello.kt -o out`（任意の .kt で bundled stdlib が読み込まれる）— RF0–RF9 完了項目監査（2026-07-15）で発見（当初 BUG-038、次いで BUG-041 として採番したが、いずれも master 側の採番と競合したため BUG-043 へ改番）。修正は Phase RF2 の `LoadSourcesPhase` タスク（`[~]`）の残作業「`--no-stdlib` の実配線」と対（当該タスク行からも本バグを参照）
