...
- [ ] BUG-046: `kotlin.reflect.full.createInstance` が宣言のみで呼ぶとリンクエラー確定 → KSP-682 で判断
- [ ] BUG-047: `ByteArray.joinToString`/`contentEquals` が未スタブ（`Scripts/diff_cases/string_tobytearray.kt` の既知ギャップ）→ KSP-660 で吸収
- [ ] BUG-048: PR #4799（XCTest→Swift Testing移行）で `#if canImport(Testing)` ガードの閉じ位置が誤り、`@Suite struct` 本体（全 `@Test` 関数）がガード外に取り残される構造的バグ。`canImport(Testing)` が false の環境でビルド不能の恐れ — 対象 `Tests/RuntimeTests/RuntimeRangeHOFTests.swift`（2026-07-13 オープンPR一括レビューで発見）
- [ ] BUG-049: PR #4801（XCTest→Swift Testing移行）で `tearDown()` が呼んでいた `kk_runtime_force_reset()` が `defer` 等に引き継がれず消失、テスト間のランタイム状態隔離が失われる — 対象 `Tests/RuntimeTests/RuntimeListPropertyTests.swift`（同型バグ: BUG-022/024/025/026/027。2026-07-13 オープンPR一括レビューで発見）
...
- [ ] BUG-034: PR #4814（XCTest→Swift Testing移行）で `.serialized` は正しく付与されているが、`tearDown`（`kk_runtime_force_reset()`）が約20テスト全てで `defer` 化されず消失（一次レビューでは見落とし、深掘り再検証で発見）— 対象 `Tests/RuntimeTests/RuntimeStringLastIndexOfAnyTests.swift`。**2026-07-15 更新: この指摘は前提が逆だった（BUG-135 参照）**。Swift Testing は全スイートが同一プロセスで並列実行されるため、`kk_runtime_force_reset()` は呼ばないのが正しい（呼ぶと並列中の他スイートの生きたハンドルを全解放して破壊する）。PR #4814 で `init()` の force_reset ごと除去して解消
...
- [ ] BUG-051: itableDynamic dispatch（`kk_itable_lookup_dynamic`、interface型パラメータ経由の呼び出し先slot解決に使用）が、コンパイラ生成クラス構築を経由しない hand-crafted Runtime オブジェクト（Comparator 系ファクトリ全般・`AutoCloseable { }`・`Uuid.LEXICAL_ORDER`）で `kk_object_register_itable_iface` 登録漏れにより失敗（`Virtual dispatch failed: method not found in vtable/itable`）。再現: `sortedWith(CASE_INSENSITIVE_ORDER)`、`sortedWith(Uuid.LEXICAL_ORDER)`、`fun f(c: AutoCloseable) { c.close() }` — PR #4836 open（commit `c8917b5068` で `RuntimeComparator.swift`/`RuntimeCollectionHOF.swift`/`RuntimeUuid.swift` に登録追加）
- [ ] BUG-052: master CI（`Full Swift Tests (RuntimeTests)` ジョブ）が2026-07-15〜16に直近4回中3回失敗。原因は BUG-021/023/024/028 系と同根で、`kk_runtime_force_reset()` 等が書き換える `Sources/Runtime/RuntimeGC.swift` のプロセス全体共有ハンドルテーブル（`runtimeStorage.objectPointers` 等）に対し、XCTest 時代の `RuntimeTestIsolationSupport.swift`（`gcSemaphore` 等のプロセス全体セマフォ、`IsolatedRuntimeXCTestCase.setUp/tearDown` 経由）が提供していたクロスファイル排他が、Swift Testing の `@Suite` では構造的に再現できない（`XCTestCase` サブクラス専用の仕組みで Swift Testing の struct suite からは使えない）。`@Suite(.serialized)` はスイート内のみを直列化し、他ファイルの並行スイートとは排他されないため、あるスイートの reset が別スイートの生存中ハンドルを消し飛ばし、`invalid range/array/string handle` パニックや `Set`/`Array` 内容破損（解放先アドレスの再利用によるものと推定）を引き起こす。対象確認: `Tests/RuntimeTests/RuntimeSynchronizedTests.swift`（PR #4823 で移行済み・`init()` と各 `@Test` の `defer` で `kk_runtime_force_reset()` を正しく呼び `.serialized` も付与済みだが、cross-suite 排他が無いため依然として攻撃源になり得ることを示す新規ケース。BUG-020〜035 では未記載）。既存の BUG-023/024/028 も現行 master 上で確認済みの同根の実攻撃源（詳細は各項目参照）。最小再現: `DEVELOPER_DIR=/Applications/Xcode-beta.app SWIFT_TEST_WORKERS=4 bash Scripts/swift_test.sh --filter '^RuntimeTests\.' -Xswiftc -swift-version -Xswiftc 6` をローカルで数回実行（`SWIFT_TEST_PARALLEL` 未設定＝デフォルト `--parallel --num-workers 4`）。発見元: 本 master CI 失敗調査タスク（2026-07-16）。暫定対応: `.github/workflows/ci.yml` の `full-swift-tests` ジョブで `RuntimeTests` マトリクスエントリのみ `SWIFT_TEST_PARALLEL: "0"` を設定し直列実行に固定（実測 196 tests / 0.235 秒、タイムアウト無関係）。根本修正（Swift Testing 向けクロススイート排他機構の再構築、または全対象ファイルの個別是正）は未着手。暫定緩和 PR: #4846 → **この緩和が BUG-040 を誘発した（`--no-parallel` 化で exec() 引数長制限に抵触）**。攻撃源スイート側の根本修正は BUG-135（PR #4814）
- [ ] BUG-135: Swift Testing 化された Runtime テストスイートがプロセスグローバル破壊 API（`kk_runtime_force_reset()` / `kk_system_gc()` 等）を呼び、同一プロセスで並列実行中の他 Swift Testing スイートの生きたハンドルを解放して破壊する（BUG-039 と同一根本原因の PR #4814 側調査）。PR #4814 の CI（Full Swift Tests (RuntimeTests)）が `KSWIFTK-RUNTIME-0001` の invalid handle パニックで確率的にクラッシュする実害を 2 回確認（1回目 `kk_native_byteArray_getCharAt`、2回目は master マージで加害スイートが増えた後の `runtimeStringScalars`）。XCTest はテストクラス単位のサブプロセス分離並列のため setUp/tearDown での force_reset が安全だったが、Swift Testing では同じ作法が有害。判明した加害スイートは 4 つ: `RuntimeSetCollectionHOFTests`（#4824）・`RuntimeSynchronizedTests`（#4823）・`RuntimeReadWriteLockTests`（#4819）・`RuntimeMemoryTests`（#4769、`kk_system_gc` の mark&sweep がルート未登録ハンドルを回収）。PR #4814 で (1) 前 3 者から force_reset を除去、(2) `RuntimeMemoryTests` を XCTest に差し戻し、(3) Swift Testing ファイルでの禁止 API 呼び出しを決定論的に検出する `RuntimeSwiftTestingIsolationLintTests` を追加（以後の移行 PR の同型地雷は当該 PR の CI で確実に fail する）。この修正により Swift Testing の攻撃源は排除済みで、CI の RuntimeTests 直列化（BUG-039/040 の暫定対応、#4854 のチャンク化直列）は原理的には不要になる — 並列復帰（`SWIFT_TEST_PARALLEL=0` の撤去）は本 PR マージ後に別 PR で検証つきで実施すること。注意: BUG-021〜035 の「`.serialized`/force_reset 欠落」系レビュー指摘はこの逆前提で書かれており要棚卸し（プロセスグローバル状態を実際に変異するテストに保護が必要という点は引き続き正しいが、対処は「force_reset を足す」ではなく XCTest に留めること）— PR #4814 で発見・修正
