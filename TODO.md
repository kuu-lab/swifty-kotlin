...
- [ ] BUG-046: `launch { }` の子コルーチン本体が、直後（介在する suspension point なし）の同期的な `cancelJob.cancel()` より先に実行を開始してしまうことがある、`kk_kxmini_launch`（`Sources/Runtime/RuntimeCoroutine.swift`）の起動-キャンセル競合。原因: `KxMiniRuntime.launch(workItem:)` が `DispatchQueue.global()`（真の並行スレッドプール）へ即座に dispatch するため、子スレッド側 `job.markStarted()` と親スレッド側 `job.cancel()` の `NSLock` 獲得順序に確定的な happens-before 保証がない。本家 kotlinx.coroutines は `runBlocking` 直下の `launch` を単一スレッドの協調的 event loop で動かすため、suspension point を跨がない `cancel()` は子の実行開始を確実に防げるが、本実装は実スレッド並行に依存しており、子スレッドが `markStarted()`→`cancellationSnapshot()` チェックを親の `cancel()` より先に通過すると本体が走り出し、`delay()` 到達後に協調的キャンセルで中断されて期待外の出力（例: catch節の "cancelled"）が漏れる。再現: `Scripts/diff_cases/coroutine_cancellation_edge_cases.kt` / `Tests/CompilerBackendTests/Codegen/CodegenBackendIntegrationTests+CoroutineCancellationEdgeCases.swift` の `testCodegenCompilesCoroutineCancellationEdgeCases`（PR #4830 で追加）。CI高負荷時に発現（本件は PR #4578 の `CompilerBackendTests shard 3/6`、`Actions run 29439186351/job 87460299566` で観測）。頻度は未確定 — ローカルでの繰り返し再現を試みたが、本開発機で他セッションの並行ビルドが多数走っていたことによるリソース競合とみられる無関係な `LinkPhase.swift:109` の `"outputUnavailable"` エラーに阻まれ、対象のレース自体をクリーンに再現できなかった（1回だけ `cancelJob.join()` 待ちとみられる無限待機/0%CPU sleepingも観測したが、上記のリソース競合下での観測のため本バグと確定できない）。低負荷環境での再現率は改めて要検証。同種バグは別セッションでも独立発見・BUG-039 として記録されていたが（`Log coroutine cancel-before-start timing race as BUG-039`, commit `1f925b36d4`）、その後 master 側で BUG-039/040 が全く別内容（RuntimeTests CI の cross-suite GC race / exec引数長制限）で確定したため番号が競合し、本項目として BUG-041 に採番し直した。発見元: PR #4578 の CI 診断中（#4830 自体の変更ではなく `kk_kxmini_launch` の既存 dispatch モデルに起因する既存バグ）。**関連**: PR #4849 の `kotlinc Diff Regression`（`coroutine_cancellation_advanced.kt`、Actions run 29553012820/job 87829404757）でも同根とみられる別のキャンセル絡みの flaky 出力差分を観測（`NonCancellable` クリーンアップ経路、BUG-044参照）— ローカル5回連続再現せず、CI高負荷時のみ発現する点が本バグと一致
- [ ] BUG-047: ローカル環境で `swift build` のデフォルトビルドシステムが新エンジン（`--build-system swiftbuild`、XCBuild ベース）になっているツールチェイン（Swift 6.4, `swift-driver version: 1.168.4` で確認）だと、`CodegenRuntimeSupport+RuntimeObjects.swift` の `buildRuntimeObjects`/`discoverRuntimeObjectPaths` がランタイムオブジェクトの自動ビルド探索に旧レイアウト（`<scratch>/<triple>/debug/Runtime.build/*.swift.o`）しか見ておらず、新レイアウト（`.../Runtime-t.build/Objects-normal/<arch>/*.o`）を探索できないため、ビルド自体は成功しているにもかかわらず `KSWIFTK-LINK-0001: Unable to locate packaged runtime object files` でネイティブリンクが常に失敗する。`CompilerBackendTests` の `assertKotlinOutput` 系（`CodegenBackendIntegrationTests` 等）が PR 内容に関わらず全件 `outputUnavailable` で失敗する形で顕在化。CI（`ubuntu-latest` + Swift 6.2）では `swift build` のデフォルトビルドシステムが旧エンジンのままのため未発生と推測（要継続確認）。再現: 該当ツールチェインで `bash Scripts/swift_test.sh --filter CodegenBackendIntegrationTests/testKotlinTextSubstringEdgeCases`（PR 変更内容と無関係な既存テストでも再現） — PR #4567 コンフリクト解消作業中に発見（BUG-039→041→043 と2回番号衝突のため再採番）。修正 PR: #4849 open（マージ後に `[x]` 化）
- [ ] BUG-048: `Scripts/diff_cases/coroutine_cancellation_advanced.kt` の kotlinc diff regression が Linux CI 上で断続的に失敗する（`kotlinc Diff Regression Shard (1/11)`）。標準出力に kotlinc 版には無い `cleanup done` が1行余分に出力される（`stdout.diff`: `job2 cancelled: true` の直後に候補側だけ `cleanup done` が挿入され、その後 `done`）。ケース4の `job4.cancel(); job4.join()` 後、`finally { withContext(NonCancellable) { delay(1); println("cleanup done") } }` のクリーンアップが `job4.join()` の完了判定と競合しているタイミング依存の疑い。ローカル（macOS, `bash Scripts/diff_kotlinc.sh Scripts/diff_cases/coroutine_cancellation_advanced.kt` を5回連続実行）では毎回 PASS し再現せず、CI 実行環境（Linux, 高負荷時）特有の可能性が高い。**BUG-041（`kk_kxmini_launch` 起動-キャンセル競合）と同根の疑いが強い** — 両方とも `DispatchQueue.global()` の並行実行と cancel 状態確認の happens-before 未保証が原因とみられ、CI高負荷時のみ発現する点も一致。発見元: PR #4849 の CI（Actions run 29553012820, job 87829404757）。未着手（当初 BUG-042 として記載したが、master 上の別件 BUG-041 との番号衝突連鎖で BUG-044 へ採番し直し）
