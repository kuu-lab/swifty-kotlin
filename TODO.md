...
- [x] RF-LOWER-CALL-007: source-backed呼び出しの保持判断を専用policyへ抽出する（前提: CALL-002）
  ...
  - 前提の扱い: CALL-002 は未着手のまま本項を先行実施した。両者の依存は意味上ではなく `+CallRewrite.swift` の同時編集回避（セクション冒頭の直列化ルール）であり、CALL-002 のPRは存在しない（`gh pr list --search "RF-LOWER-CALL"` で確認）。CALL-002 が触る `builderLambdaKinds` の事前走査・引数配線には本PRで一切手を付けていないため、CALL-002 は `lowerCallInstruction` のシグネチャ変更として素直にrebaseできる。
