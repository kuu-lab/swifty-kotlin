...
- [ ] BUG-034: PR #4814（XCTest→Swift Testing移行）で `.serialized` は正しく付与されているが、`tearDown`（`kk_runtime_force_reset()`）が約20テスト全てで `defer` 化されず消失（一次レビューでは見落とし、深掘り再検証で発見）— 対象 `Tests/RuntimeTests/RuntimeStringLastIndexOfAnyTests.swift`。**2026-07-15 更新: この指摘は前提が逆だった（BUG-038 参照）**。Swift Testing は全スイートが同一プロセスで並列実行されるため、`kk_runtime_force_reset()` は呼ばないのが正しい（呼ぶと並列中の他スイートの生きたハンドルを全解放して破壊する）。PR #4814 で `init()` の force_reset ごと除去して解消
...
- [ ] BUG-038: PR #4824（マージ済み）で Swift Testing 化された `RuntimeSetCollectionHOFTests` が `init()` で `kk_runtime_force_reset()` を呼び、同一プロセスで並列実行中の他 Swift Testing スイートの生きたハンドルを全解放して破壊する。PR #4814 の CI（Full Swift Tests (RuntimeTests)）が `KSWIFTK-RUNTIME-0001: invalid array handle in kk_native_byteArray_getCharAt` で確率的にクラッシュする実害を確認（被害側は `RuntimeNativePrimitiveByteArray*Tests` 等の並列スイート）。XCTest はテストクラス単位のサブプロセス分離並列のため setUp/tearDown での force_reset が安全だったが、Swift Testing では同じ作法が有害。注意: BUG-028〜035 の「`.serialized`/force_reset 欠落」系レビュー指摘はこの逆前提で書かれており要棚卸し（プロセスグローバル状態を実際に変異するテスト（GC 起動・locale 書き換え等）に保護が必要という点は引き続き正しいが、対処は「force_reset を足す」ではない）— PR #4814 で発見、同 PR で `init()` から force_reset を除去して修正
