...
最終更新: 2026-07-16（master CI 失敗調査で BUG-023/024/028 が実際に master 上で発火していることを確認し、BUG-039 を追補。CI 側に暫定緩和を適用 — 詳細は BUG-039。それ以前: 2026-07-13 dead-code 再監査と、オープンPR一括レビューで判明した Swift Testing 移行の変換不備を追補。BUG-020〜035 は canImport ガード不備、tearDown 消失系、`.serialized` 欠落系などを扱う。PR #4837 の identity equality 関連を BUG-132/133 として追加）
