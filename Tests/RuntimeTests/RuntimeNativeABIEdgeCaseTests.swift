...
struct RuntimeNativeABIEdgeCaseTests {
    ...
    @Test func testFreezeParentAlsoFreezesDirectChild() {
        ...
        #expect(kk_is_frozen(parentRaw) == 0, "parent should not be frozen before freeze()")
        #expect(kk_is_frozen(childRaw) == 0, "child should not be frozen before freeze()")

        _ = kk_freeze_object(parentRaw)

        #expect(kk_is_frozen(parentRaw) == 1, "parent must be frozen after freeze()")
        #expect(kk_is_frozen(childRaw) == 1, "child reachable from parent must be frozen (ABI-004)")
    }

    /// Freezing the root of a two-level chain must freeze all levels.
    @Test func testFreezeRecursiveTwoLevels() {
        ...
        #expect(kk_is_frozen(parentRaw) == 1, "root must be frozen")
        #expect(kk_is_frozen(childRaw) == 1, "mid-level child must be frozen")
        #expect(kk_is_frozen(grandchildRaw) == 1, "leaf grandchild must be frozen")
    }

    /// Freeze on a cyclic graph (A → B → A) must terminate and freeze all nodes.
    @Test func testFreezeWithCycleTerminates() {
        ...
        #expect(kk_is_frozen(aRaw) == 1, "node A must be frozen in cyclic graph")
        #expect(kk_is_frozen(bRaw) == 1, "node B must be frozen in cyclic graph")
    }

    /// Freezing a node with a zero child ref must not crash (zero refs are skipped).
    @Test func testFreezeObjectWithZeroChildRef() {
        ...
        #expect(kk_is_frozen(raw) == 1, "object with zero child ref must still be frozen")
    }

    /// Freezing an already-frozen object must be idempotent and not crash.
    @Test func testFreezingAlreadyFrozenObjectIsIdempotent() {
        ...
        #expect(kk_is_frozen(raw) == 1)
    }

    /// Freeze using a RuntimeListBox — list elements must be frozen too.
    @Test func testFreezeListBoxChildrenAreFrozen() {
        ...
        #expect(kk_is_frozen(listRaw) == 1, "list must be frozen")
        #expect(kk_is_frozen(itemRaw) == 1, "list element must be frozen")
    }

    // MARK: - ABI-005: Idempotent kk_unpin_object

    /// Calling kk_unpin_object twice on the same handle must not crash.
    @Test func testUnpinObjectTwiceDoesNotCrash() {
        let obj = RuntimeObjectBox(length: 0, classID: 100)
        let objRaw = registerRuntimeObject(obj)
        let pinHandle = kk_pin_object(objRaw)

        #expect(pinHandle != 0, "pin handle must be non-zero")

        let result1 = kk_unpin_object(pinHandle)
        #expect(result1 == objRaw, "first unpin must return the original object raw")
    ...
    @Test func testUnpinObjectThreeTimesDoesNotCrash() {
    ...
    @Test func testUnpinObjectReturnsCorrectRaw() {
    ...
    @Test func testUnpinObjectZeroHandleIsNoop() {
