...
struct ABIMismatchTests {
    ...
    @Test
    func kkStringReplaceRangeFlatSignature() throws {
        let spec = try requireSpec("kk_string_replaceRange_flat")
        #expect(spec.returnType == .nullableUInt8Pointer)
        #expect(spec.parameters.count == 13)
        #expect(spec.parameters.map(\.type) == [
            .nullableConstUInt8Pointer,
            .intptr,
            .intptr,
    ...
    @Test
    func kkStringRemoveRangeFlatSignatures() throws {
        let indexed = try requireSpec("kk_string_removeRange_flat")
        #expect(indexed.returnType == .nullableUInt8Pointer)
        #expect(indexed.parameters.count == 10)
        #expect(indexed.parameters.map(\.type) == [
            .nullableConstUInt8Pointer,
            .intptr,
            .intptr,
            .intptr,
        ...
        #expect(ranged.returnType == .nullableUInt8Pointer)
        #expect(ranged.parameters.count == 9)
        #expect(ranged.parameters.map(\.type) == [
            .nullableConstUInt8Pointer,
            .intptr,
            .intptr,
            .intptr,
