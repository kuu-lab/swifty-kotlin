...
struct RuntimeStreamTests {
    @Test func testInputStreamReadAvailableSkipAndClose() throws {
        ...
        var thrown = 0
        let streamRaw = __kk_file_inputStream(fileRaw, &thrown)

        #expect(thrown == 0)
        #expect(__kk_input_stream_available(streamRaw) == 4)
        #expect(__kk_input_stream_read(streamRaw, &thrown) == 97)
        #expect(__kk_input_stream_skip(streamRaw, 1, &thrown) == 1)
        #expect(__kk_input_stream_read(streamRaw, &thrown) == 99)
        #expect(__kk_input_stream_close(streamRaw) == 0)
        #expect(__kk_input_stream_available(streamRaw) == 0)
    }
    ...
    @Test func testInputStreamReadIntoByteArrayLikeBuffer() throws {
        ...
        #expect(__kk_input_stream_read_bytes(streamRaw, bufferRaw, &thrown) == 3)
    ...
    @Test func testInputStreamCopyToTransfersBytesAndReturnsCount() throws {
        ...
        var thrown = 0
        let inputStreamRaw = __kk_file_inputStream(srcFileRaw, &thrown)
        #expect(thrown == 0)
        let outputStreamRaw = __kk_file_outputStream(dstFileRaw, &thrown)
        #expect(thrown == 0)

        let bufferSizeRaw = kk_box_int(1024)
        let resultRaw = __kk_input_stream_copyTo(inputStreamRaw, outputStreamRaw, bufferSizeRaw, &thrown)
        #expect(thrown == 0)
        let bytesCopied = kk_unbox_long(resultRaw)
        #expect(bytesCopied == 5)

        _ = __kk_output_stream_flush(outputStreamRaw, &thrown)
        _ = __kk_output_stream_close(outputStreamRaw)
