...
struct CodegenBackendSequenceJoinToStringTests {
    @Test func testCodegenSequenceJoinToStringDoesNotUseRuntimeHelper() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            let ctx = makeCompilationContext(inputs: [path], moduleName: "SequenceJoinToStringKIR", emit: .kirDump, allowDefaultStdlibLibrary: false)
            try runToLowering(ctx)

            let module = try #require(ctx.kir)
            let body = try findKIRFunctionBody(named: "render", in: module, interner: ctx.interner)
            let callees = extractCallees(from: body, interner: ctx.interner)
            #expect(callees.contains("joinToString"))
