...
struct CodegenBackendMathOverloadEdgeCasesTests {
    ...
    func testCodegenFQNMathCallsLowerToRuntimeHelpers() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            #expect(callees.contains("kk_math_abs_int"), "FQN abs(Int) must lower to kk_math_abs_int, got \(callees)")
            #expect(callees.contains("kk_math_abs"), "FQN abs(Double) must lower to kk_math_abs, got \(callees)")
