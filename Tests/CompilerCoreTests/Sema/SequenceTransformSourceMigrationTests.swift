...
struct SequenceTransformSourceMigrationTests {
    ...
    /// `flatMap` / `flatMapIndexed` used to be bound to the synthetic
    /// `kk_sequence_flatMap*` members even though bundled source declarations
    /// existed, because Sema resolved them before the source binding ran.
    @Test
    func flatMapCallsResolveToBundledSource() throws {
        ...
        for calleeName in ["flatMap", "flatMapIndexed"] {
            let callExprIDs = allExprIDs(in: ast) { _, expr in
