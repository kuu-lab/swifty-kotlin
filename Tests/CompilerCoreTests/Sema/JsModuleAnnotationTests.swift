...
final class JsModuleAnnotationTests: XCTestCase {
    private func makeSema(
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ...
                "Expected JsModule annotation surface to resolve cleanly, got: \(diagnostics)"
            )
    ...
    func testJsModuleAnnotationIsRegistered() throws {
        let (sema, interner) = try makeSema()
        let fqName = ["kotlin", "js", "JsModule"].map { interner.intern($0) }
        let symbol = try XCTUnwrap(
            sema.symbols.lookup(fqName: fqName),
            "kotlin.js.JsModule must be registered"
        )
    ...
    func testJsModuleImportPropertyAndConstructorAreRegistered() throws {
        let (sema, interner) = try makeSema()
        let baseFQName = ["kotlin", "js", "JsModule"].map { interner.intern($0) }
        let annotationSymbol = try XCTUnwrap(sema.symbols.lookup(fqName: baseFQName))
        let propertySymbol = try XCTUnwrap(
            sema.symbols.lookup(fqName: baseFQName + [interner.intern("import")]),
            "JsModule.import property must be registered"
        )
        let constructorSymbol = try XCTUnwrap(
            ...
            "JsModule String constructor must be registered"
        )
    ...
    func testJsModuleIsAcceptedOnExternalClass() {
        let source = """
        ...
        XCTAssertTrue(
            errors.isEmpty,
            "Expected JsModule on an external class to type-check, got \(errors)"
        )
    }
}
