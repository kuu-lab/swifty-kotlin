...
final class NativeCInteropStringCstrPropertyTests: XCTestCase {
    func testStringCstrPropertySurfaceMatchesNativeShape() throws {
        let ctx = makeContextFromSource("fun noop() {}")
        try runSema(ctx)
        XCTAssertFalse(
            ctx.diagnostics.hasError,
            "Expected String.cstr surface to compile cleanly, got: \(ctx.diagnostics.diagnostics)"
        )
        ...
        let byteVarType = try XCTUnwrap(sema.symbols.typeAliasUnderlyingType(for: byteVarAlias))
    ...
    func testStringCstrPropertyResolvesInSource() throws {
        ...
        XCTAssertFalse(
            ctx.diagnostics.hasError,
            "Expected String.cstr to resolve, got: \(ctx.diagnostics.diagnostics)"
        )
    }
}
