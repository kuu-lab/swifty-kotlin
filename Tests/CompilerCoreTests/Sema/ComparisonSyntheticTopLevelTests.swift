...
final class ComparisonSyntheticTopLevelTests: XCTestCase {
    ...
    func testComparableThreeArgMinOfResolvesToSyntheticComparisonFunction() throws {
        let source = """
        fun sample(): String {
            return minOf("c", "a", "b")
        }
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            let callExpr = try XCTUnwrap(
                ...
                "Expected 3-arg comparable minOf call"
            )
            XCTAssertNil(sema.bindings.stdlibSpecialCallKind(for: callExpr))
            ...
            XCTAssertEqual(signature.parameterTypes.count, 3)
