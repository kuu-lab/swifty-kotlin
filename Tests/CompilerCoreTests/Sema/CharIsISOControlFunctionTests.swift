...
@Suite
struct CharIsISOControlFunctionTests {
    @Test func testIsISOControlResolvesOnCharLiteralReceiver() throws {
    ...
    @Test func testIsISOControlResolvesOnCharParameterReceiver() throws {
        let ctx = makeContextFromSource("""
    ...
    @Test func testIsISOControlResolvesInBranch() throws {
        let ctx = makeContextFromSource("""
    ...
    @Test func testIsISOControlLinksToRuntimeStub() throws {
        ...
        let symbol = try #require(charReceiverSymbol, "Char.isISOControl synthetic stub should be registered")
        #expect(sema.symbols.externalLinkName(for: symbol) == "kk_char_isISOControl")
    }

    @Test func testIsISOControlResolvesAtCallSite() throws {
        let ctx = makeContextFromSource("""
        fun probe(ch: Char) {
            ch.isISOControl()
        }
        ...
        #expect(chosen.flatMap { sema.symbols.externalLinkName(for: $0) } == "kk_char_isISOControl")
    }
}
#endif
