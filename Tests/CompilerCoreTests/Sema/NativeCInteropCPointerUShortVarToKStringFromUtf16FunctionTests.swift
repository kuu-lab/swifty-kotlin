...
@Suite
struct NativeCInteropCPointerUShortVarToKStringFromUtf16FunctionTests {
    @Test func testCPointerUShortVarToKStringFromUtf16FunctionSurfaceMatchesNativeShape() throws {
    ...
    @Test func testCPointerUShortVarToKStringFromUtf16FunctionLinksToRuntimeSymbol() throws {
        let ctx = makeContextFromSource("fun noop() {}")
        try runSema(ctx)
        let sema = try #require(ctx.sema)
        let interner = ctx.interner
        ...
        #expect(sema.symbols.externalLinkName(for: fn) == "kk_cpointer_toKStringFromUtf16", "CPointer<UShortVar>.toKStringFromUtf16() must link to kk_cpointer_toKStringFromUtf16")
    }

    @Test func testCPointerUShortVarToKStringFromUtf16FunctionResolvesInSource() throws {
