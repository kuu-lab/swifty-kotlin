...
final class JsCollectionsReadonlySetToSetTests: XCTestCase {
    private func makeSema(
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ...
                "Expected JsReadonlySet.toSet surface to resolve cleanly, got: \(diagnostics)"
            )
    ...
    func testJsReadonlySetToSetIsRegistered() throws {
        ...
        let setSymbol = try XCTUnwrap(sema.symbols.lookup(
            fqName: collectionsPkg + [interner.intern("Set")]
        ))
        ...
        let function = try XCTUnwrap(
            sema.symbols.lookupAll(
                fqName: pkg + [interner.intern("JsReadonlySet"), interner.intern("toSet")]
            ).first { symbol in
            ...
            "JsReadonlySet<E>.toSet() must be registered"
        )
    ...
    func testJsReadonlySetToSetResolvesFromSource() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertTrue(
                ...
                "Expected JsReadonlySet.toSet usage to type-check cleanly, got: \(ctx.diagnostics.diagnostics)"
            )
            ...
            let setSymbol = try XCTUnwrap(sema.symbols.lookup(
                fqName: ["kotlin", "collections", "Set"].map { ctx.interner.intern($0) }
            ))

            guard case let .classType(returnClassType) = sema.types.kind(of: signature.returnType) else {
                return XCTFail("Expected strings() to return Set<String>")
            }
            ...
            XCTAssertEqual(sema.symbols.externalLinkName(for: chosenCallee), "kk_js_set_toSet")
        }
    }
}
