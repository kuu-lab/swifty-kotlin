...
@Suite
struct NativeCInteropLongArrayToCValuesFunctionTests {
    @Test func testLongArrayToCValuesFunctionSurfaceMatchesNativeShape() throws {
        let ctx = makeContextFromSource("fun noop() {}")
        try runSema(ctx)
        #expect(!(ctx.diagnostics.hasError), "Expected LongArray.toCValues() surface to compile cleanly, got: \(ctx.diagnostics.diagnostics)")
    ...
    @Test func testLongArrayToCValuesFunctionResolvesInSource() throws {
        ...
        #expect(!(ctx.diagnostics.hasError), "Expected LongArray.toCValues() to resolve, got: \(ctx.diagnostics.diagnostics)")
    }
}
#endif
