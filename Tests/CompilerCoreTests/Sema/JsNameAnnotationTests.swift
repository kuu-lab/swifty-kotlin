...
final class JsNameAnnotationTests: XCTestCase {
    private func makeSema(
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ...
                "Expected JsName annotation surface to resolve cleanly, got: \(diagnostics)"
            )
    ...
    func testJsNameAnnotationIsRegistered() throws {
        let (sema, interner) = try makeSema()
        let fqName = ["kotlin", "js", "JsName"].map { interner.intern($0) }
        let symbol = try XCTUnwrap(
            sema.symbols.lookup(fqName: fqName),
            "kotlin.js.JsName must be registered"
        )
    ...
    func testJsNameNamePropertyAndConstructorAreRegistered() throws {
        let (sema, interner) = try makeSema()
        let baseFQName = ["kotlin", "js", "JsName"].map { interner.intern($0) }
        ...
        let propertySymbol = try XCTUnwrap(
            ...
            "JsName.name property must be registered"
        )
        let constructorSymbol = try XCTUnwrap(
            ...
            "JsName String constructor must be registered"
        )
    ...
    func testJsNameIsAcceptedOnClassAndFunction() {
        let source = """
        ...
        XCTAssertTrue(
            errors.isEmpty,
            "Expected JsName on class and function to type-check, got \(errors)"
        )
    }
}
