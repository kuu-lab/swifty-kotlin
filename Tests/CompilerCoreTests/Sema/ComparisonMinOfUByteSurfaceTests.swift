...
final class ComparisonMinOfUByteSurfaceTests: XCTestCase {
    func testUByteMinOfSurfaceRegistersUnsignedOverloads() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertTrue(signatures.contains { signature in
                signature.parameterTypes == [sema.types.ubyteType, sema.types.ubyteType]
                    && signature.returnType == sema.types.ubyteType
                    && !signature.valueParameterIsVararg.contains(true)
            })
            XCTAssertTrue(signatures.contains { signature in
                signature.parameterTypes == [sema.types.ubyteType, sema.types.ubyteType, sema.types.ubyteType]
                    && signature.returnType == sema.types.ubyteType
                    && !signature.valueParameterIsVararg.contains(true)
            })
            XCTAssertTrue(signatures.contains { signature in
