...
final class ComparisonMinOfUShortSurfaceTests: XCTestCase {
    func testUShortMinOfSurfaceRegistersUnsignedOverloads() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertTrue(signatures.contains { signature in
                signature.parameterTypes == [sema.types.ushortType, sema.types.ushortType]
                    && signature.returnType == sema.types.ushortType
                    && !signature.valueParameterIsVararg.contains(true)
            })
            XCTAssertTrue(signatures.contains { signature in
                signature.parameterTypes == [sema.types.ushortType, sema.types.ushortType, sema.types.ushortType]
                    && signature.returnType == sema.types.ushortType
                    && !signature.valueParameterIsVararg.contains(true)
            })
            XCTAssertTrue(signatures.contains { signature in
