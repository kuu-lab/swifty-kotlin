...
final class JsIntNumberInteropTests: XCTestCase {
    private func makeSema(
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ...
                "Expected Int JsNumber interop surface to resolve cleanly, got: \(diagnostics)"
            )
    ...
    func testIntToJsNumberIsRegistered() throws {
        ...
        let function = try XCTUnwrap(
            sema.symbols.lookupAll(fqName: jsPkg + [interner.intern("toJsNumber")]).first { symbol in
                ...
                return signature.receiverType == sema.types.intType
                    && signature.parameterTypes.isEmpty
                    && signature.returnType == jsNumberType
            },
            "Int.toJsNumber() must be registered"
        )
        ...
        XCTAssertEqual(sema.symbols.externalLinkName(for: function), "kk_int_toJsNumber")
    }

    func testIntToJsNumberResolvesFromSource() throws {
        let source = """
        import kotlin.js.JsNumber
        import kotlin.js.toJsNumber

        fun convert(value: Int): JsNumber = value.toJsNumber()
        """
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertTrue(
                ...
                "Expected Int.toJsNumber usage to type-check cleanly, got: \(ctx.diagnostics.diagnostics)"
            )
            ...
            XCTAssertEqual(sema.symbols.externalLinkName(for: chosenCallee), "kk_int_toJsNumber")
        }
    }
}
