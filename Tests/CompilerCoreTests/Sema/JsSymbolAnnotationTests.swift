...
final class JsSymbolAnnotationTests: XCTestCase {
    private func makeSema(
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ...
                "Expected JsSymbol annotation surface to resolve cleanly, got: \(diagnostics)"
            )
    ...
    func testJsSymbolAnnotationIsRegistered() throws {
        let (sema, interner) = try makeSema()
        let fqName = ["kotlin", "js", "JsSymbol"].map { interner.intern($0) }
        let symbol = try XCTUnwrap(
            sema.symbols.lookup(fqName: fqName),
            "kotlin.js.JsSymbol must be registered"
        )
    ...
    func testJsSymbolCarriesExpectedMetadata() throws {
        let (sema, interner) = try makeSema()
        let fqName = ["kotlin", "js", "JsSymbol"].map { interner.intern($0) }
        let symbol = try XCTUnwrap(sema.symbols.lookup(fqName: fqName))
        let target = try XCTUnwrap(
            sema.symbols.annotations(for: symbol).first { $0.annotationFQName == "kotlin.annotation.Target" },
            "JsSymbol must carry @Target metadata"
        )

        XCTAssertEqual(Set(target.arguments), Set(["AnnotationTarget.FUNCTION"]))
        XCTAssertTrue(
            sema.symbols.annotations(for: symbol).contains {
                $0.annotationFQName == "kotlin.ExperimentalStdlibApi"
            },
            "JsSymbol must carry ExperimentalStdlibApi metadata"
        )
    }

    func testJsSymbolNamePropertyAndConstructorAreRegistered() throws {
        let (sema, interner) = try makeSema()
        let baseFQName = ["kotlin", "js", "JsSymbol"].map { interner.intern($0) }
        ...
        let propertySymbol = try XCTUnwrap(
            ...
            "JsSymbol.name property must be registered"
        )
        let constructorSymbol = try XCTUnwrap(
            ...
            "JsSymbol String constructor must be registered"
        )
