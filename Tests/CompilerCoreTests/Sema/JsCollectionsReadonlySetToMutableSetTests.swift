...
final class JsCollectionsReadonlySetToMutableSetTests: XCTestCase {
    private func makeSema(
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertFalse(
                ...
                "Expected JsReadonlySet.toMutableSet surface to resolve cleanly, got: \(diagnostics)"
            )
    ...
    func testJsReadonlySetToMutableSetIsRegistered() throws {
        ...
        let mutableSetSymbol = try XCTUnwrap(sema.symbols.lookup(
            fqName: collectionsPkg + [interner.intern("MutableSet")]
        ))
        ...
        let function = try XCTUnwrap(
            sema.symbols.lookupAll(
                fqName: pkg + [interner.intern("JsReadonlySet"), interner.intern("toMutableSet")]
            ).first { symbol in
            ...
            "JsReadonlySet<E>.toMutableSet() must be registered"
        )
    ...
    func testJsReadonlySetToMutableSetResolvesFromSource() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            XCTAssertTrue(
                ...
                "Expected JsReadonlySet.toMutableSet usage to type-check cleanly, got: \(ctx.diagnostics.diagnostics)"
            )
            ...
            let mutableSetSymbol = try XCTUnwrap(sema.symbols.lookup(
                fqName: ["kotlin", "collections", "MutableSet"].map { ctx.interner.intern($0) }
            ))

            guard case let .classType(returnClassType) = sema.types.kind(of: signature.returnType) else {
                return XCTFail("Expected strings() to return MutableSet<String>")
            }
            XCTAssertEqual(returnClassType.classSymbol, mutableSetSymbol)
            ...
            XCTAssertEqual(sema.symbols.externalLinkName(for: chosenCallee), "kk_js_set_toMutableSet")
        }
    }
}
