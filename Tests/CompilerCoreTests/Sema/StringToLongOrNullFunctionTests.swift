    ...
    func testToLongOrNullStubLinksToRuntimeSymbol() throws {
        try withTemporaryFile(contents: "fun noop() {}") { path in
            ...
            XCTAssertEqual(
                ...
                "kk_string_toLongOrNull_flat",
                "String.toLongOrNull should link to kk_string_toLongOrNull_flat"
            )

            let links = externalLinks(for: "toLongOrNull", sema: sema, interner: ctx.interner)
            XCTAssertTrue(
                links.contains("kk_string_toLongOrNull_flat"),
                "lookupAll for toLongOrNull must include kk_string_toLongOrNull_flat; got: \(links)"
            )
        }
    }
