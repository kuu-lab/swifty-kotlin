...
/// KSP-1093: the legacy AtomicLongArray constructor overloads are source-backed
/// while the allocation and residual member operations retain their runtime ABI.
@Suite(.serialized)
struct AtomicLongArraySourceMigrationTests {
    @Test
    func testConstructorsAreSourceBackedAndUseTheExpectedVisibility() throws {
        ...
        fun fromSize(size: Int): AtomicLongArray = AtomicLongArray(size)
        fun fromArray(values: LongArray): AtomicLongArray = AtomicLongArray(values)
        """)
        try runSema(ctx)

        #expect(
            !ctx.diagnostics.hasError,
            "Expected AtomicLongArray constructor source to type-check, got: \(ctx.diagnostics.diagnostics)"
        )

        let sema = try #require(ctx.sema)
        let interner = ctx.interner
        let constructorFQName = ["kotlin", "concurrent", "AtomicLongArray"].map(interner.intern)
        ...
        let longArraySymbol = try #require(
            sema.symbols.lookup(fqName: ["kotlin", "LongArray"].map(interner.intern))
        )
        ...
        let storageConstructor = try #require(sema.symbols.lookupAll(fqName: constructorFQName).first { candidate in
            ...
            return signature.parameterTypes == [longArrayType]
                && signature.returnType == arrayType
        })
        ...
        #expect(ctx.sourceManager.path(of: sourceFileID) == "__bundled_kotlin/concurrent/AtomicLongArray/Stdlib.kt")
        ...
        let calls = ast.arena.exprs.indices.compactMap { index -> ExprID? in
            ...
            guard case let .call(callee, _, _, range) = ast.arena.expr(exprID),
                  ...
                  interner.resolve(name) == "AtomicLongArray"
            else {
                return nil
            }
            return exprID
