...
final class FrontendPhasesTests: XCTestCase {
    ...
    func testLoadSourcesSkipsDuplicatePaths() throws {
        try withTemporaryFile(contents: "fun main() {}") { path in
            ...
            // 1 user file (deduped) + 3 bundled stdlib files (collections, text + sequences)
    ...
    func testIncrementalFrontendLexesParsesAndBuildsOnlyRecompiledFiles() throws {
        try withTemporaryFiles(contents: [
            ...
            // FileID 0 = bundled collections, FileID 1 = bundled text, FileID 2 = bundled sequences, FileID 3 = kept, FileID 4 = changed
            XCTAssertEqual(incrementalCtx.tokensByFile.map(\.0), [FileID(rawValue: 4)])

            try ParsePhase().run(incrementalCtx)
            XCTAssertEqual(incrementalCtx.syntaxTrees.map(\.0), [FileID(rawValue: 4)])

            try BuildASTPhase().run(incrementalCtx)
            let ast = try XCTUnwrap(incrementalCtx.ast)
            XCTAssertEqual(ast.files.map(\.fileID), [FileID(rawValue: 0), FileID(rawValue: 1), FileID(rawValue: 2), FileID(rawValue: 3), FileID(rawValue: 4)])
            XCTAssertEqual(Set(ast.activeDeclsByFileRawID.keys), Set([0, 1, 2, 3, 4]))
