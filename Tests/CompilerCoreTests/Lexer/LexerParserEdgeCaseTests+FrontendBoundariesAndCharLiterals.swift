...
extension LexerParserEdgeCaseTests {
    func testFrontendPhasesBuildASTForMixedDeclarations() throws {
        ...
        try withTemporaryFile(contents: source) { path in
            ...
            // 1 user file + 5 bundled stdlib files (collections + text + atomic + sequences + time)
    ...
    func testMultiFileParseBoundaryProducesPerFileASTFiles() throws {
        ...
        try withTemporaryFiles(contents: [fileA, fileB]) { paths in
            ...
            XCTAssertEqual(ast.files.count, 7)

            XCTAssertEqual(ctx.tokensByFile.count, 7)
            XCTAssertEqual(ctx.syntaxTrees.count, 7)

            for (_, fileTokens) in ctx.tokensByFile {
                XCTAssertTrue(fileTokens.last.map { $0.kind == .eof } ?? false)
            }

            // Skip bundled stdlib files (indices 0, 1, 2, 3, 4), user files at indices 5 and 6
    ...
    func testMultiFileCrossFileBoundaryDoesNotConcatenateStatements() throws {
        ...
        try withTemporaryFiles(contents: [fileA, fileB]) { paths in
            ...
            // 2 user functions + 92 bundled stdlib functions (77 from master + 15 atomic)
            XCTAssertEqual(allFunNames.count, 94)
    ...
    func testMultiFilePerFileScriptAndKotlinFileDetermination() throws {
        ...
        try withTemporaryFiles(contents: [fileA, fileB]) { paths in
            ...
            // Find user's .kt file (not bundled stdlib; bundled files occupy FileIDs 0-4)
