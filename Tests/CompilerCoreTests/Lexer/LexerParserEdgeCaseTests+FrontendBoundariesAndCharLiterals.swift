...
extension LexerParserEdgeCaseTests {
    ...
    func testMultiFileCrossFileBoundaryDoesNotConcatenateStatements() throws {
        ...
        try withTemporaryFiles(contents: [fileA, fileB]) { paths in
            ...
            // 2 user functions + 28 bundled stdlib functions (17 collections + 11 text)
            XCTAssertEqual(allFunNames.count, 30)
    ...
    func testMultiFilePerFileScriptAndKotlinFileDetermination() throws {
        ...
        try withTemporaryFiles(contents: [fileA, fileB]) { paths in
            ...
            // Find user's .kt file (not bundled stdlib; bundled files have FileIDs 0 and 1)
            let kotlinFile = ast.files.first(where: { $0.scriptBody.isEmpty && $0.fileID.rawValue > 1 })
