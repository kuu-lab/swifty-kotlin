...
struct SourceBackedCallPreservationPolicyTests {
    ...
    @Test
    func virtualOnlyNamesAreExactlyTheExpectedSet() {
        ...
        #expect(
            resolved == [
                ...
                // RF-LOWER-CALL-008 moved the derived List transform/filter
                // names here: no direct-path rewrite claims them, but the
                // virtual predicate was deliberately left covering them.
                "filterNot", "filterNotNull", "filterIndexed",
                "mapIndexed", "mapNotNull", "mapIndexedNotNull",
                "mapTo", "mapIndexedTo", "mapNotNullTo", "mapIndexedNotNullTo",
                "flatMapTo", "flatMapIndexedTo",
                "isEmpty", "iterator",
    ...
    /// RF-LOWER-CALL-008 moved the twelve derived List transform/filter names
    /// (`mapIndexed`, `mapTo`, `filterNot`, `flatMapTo`, ...) to the
    /// virtual-only set.
    @Test
