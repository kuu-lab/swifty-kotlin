...
extension LoweringPassRegressionTests {
    ...
    func testInlineLoweringMapsReifiedTypeTokenSymbolRefToHiddenArgument() throws {
        ...
        let ctx = try runLowering(module: module, interner: interner, moduleName: "InlineReifiedToken", sema: makeSemaModule(symbols: symbols, types: types, bindings: bindings, diagnostics: diagnostics).ctx, diagnostics: diagnostics)

        let loweredMain = try requireTestValue(module.arena.decl(mainDeclID)?.function, "Expected lowered main function.")
