...
@Suite struct FrontendParallelBenchmarkTests {
    ...
    @Test func testPerFileFrontendResultsPopulatedForAllFiles() throws {
        ...
        #expect(ctx.tokensByFile.count == 9, "Expected tokens for 7 files (5 user + 2 bundled stdlib)")
        #expect(ctx.syntaxTrees.count == 9, "Expected syntax trees for 7 files")
        let ast = try #require(ctx.ast)
        #expect(ast.sortedFiles.count == 9, "Expected AST files for 7 files")
    ...
    @Test func testPerFileFrontendResultsPopulatedInParallelMode() throws {
        ...
        // 5 user files + 2 bundled stdlib files (collections + text)
        #expect(ctx.tokensByFile.count == 9, "Expected tokens for 7 files in parallel mode (5 user + 2 bundled stdlib)")
        #expect(ctx.syntaxTrees.count == 9, "Expected syntax trees for 7 files in parallel mode")
        let ast = try #require(ctx.ast)
        #expect(ast.sortedFiles.count == 9, "Expected AST files for 7 files in parallel mode")
