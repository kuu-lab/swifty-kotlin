...
| File/Path/IO fallback family: `java.io.File`, `kotlin.io.path.Path`, `useLines`, `read`, `readValue` | `HeaderHelpers+SyntheticJavaIOStreamStubs`（`registerSyntheticJavaIOStreamStubs`; CLEANUP-STUB-107 が File 自身の facade を削除した後は bare shell/ctor/`path` と Reader/Writer/Stream 共有ファミリのみ）, `HeaderHelpers+SyntheticPathStubs`, delegate stubs | Yes after IO stubs expose complete signatures and owner receiver metadata. |
