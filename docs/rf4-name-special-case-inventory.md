...
| `measureTimeMillis {}` (`kotlin.system`) | Bundled Kotlin source `Stdlib/kotlin/system/Timing.kt` | Yes for name guard: migrated in RF-SEMA-002 slice 1. Remaining block exists because KIR lowering consumes `stdlibSpecialCallKind` and discards lambda result. |
| `measureTimeMicros {}` (`kotlin.system`) | Bundled Kotlin source `Stdlib/kotlin/system/Timing.kt` | Yes for name guard: migrated in RF-SEMA-002 slice 1. Same remaining lowering constraint as `measureTimeMillis`. |
| `measureNanoTime {}` (`kotlin.system`) | Bundled Kotlin source `Stdlib/kotlin/system/Timing.kt` | Yes for name guard: migrated with the system timing common path. Same remaining lowering constraint as millis/micros. |
| `measureTime {}` / `measureTimedValue {}` (`kotlin.time`) | Bundled Kotlin source `Stdlib/kotlin/time/MeasureTime.kt` | Yes after kind metadata is attached and return-type construction is driven by resolved stub signature. |
...
| File/IO fallback family: `java.io.File`, `useLines`, `read`, `readValue` | `HeaderHelpers+SyntheticJavaIOStreamStubs`（`registerSyntheticJavaIOStreamStubs`; CLEANUP-STUB-107 が File 自身の facade を削除した後は bare shell/ctor/`path` と Reader/Writer/Stream 共有ファミリのみ）, delegate stubs | Yes after IO stubs expose complete signatures and owner receiver metadata. `kotlin.io.path.Path` and its `tryPathCharsetReadExtensionFallback` special case were removed entirely (CLEANUP-STUB-115). |
