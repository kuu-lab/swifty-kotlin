...
| DEBT-DIFF-005 | 2（2026-08-01 時点） | 残り2件は source Sequence/`sequence {}` builder の Iterator itable dispatch 未整備（KSP-441 では不十分、KSP-447 待ち）。他は全解消（CASE_INSENSITIVE_ORDER 誤登録＝BUG-154 は `origin/master` 側、property delegate lowering の実バグ＝BUG-151/BUG-200 は本 PR で修正） | KSP-447 の itable ブリッジ整備後に `--force-run-skipped` で再判定 |
