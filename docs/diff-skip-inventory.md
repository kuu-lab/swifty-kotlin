...
5. **[未修正]** delegate のトレーリングラムダは受信者を取らない top-level thunk へ lowering されるため、本文からクラスメンバ（暗黙 `this`）へ書き込めない。`lazy { initCount += 1; "ready" }` の `initCount += 1` は KIR から消え、`initCount` が 0 のままになる（4 の一般バグ自体は解消済みで、`class C { var n = 0; fun f() { n += 1 } }` は正しく 1 を返す）。本ケースが SKIP-DIFF のまま残る唯一の理由（BUG-168）。
