...
パス一覧と実行順序は `Sources/CompilerCore/Lowering/LoweringPhase.swift` の `passes` 配列を単一の正とする。
現行の一覧・順序依存の注記は [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) の「Lowering パス実行順序」を参照（本書では二重管理しない）。
