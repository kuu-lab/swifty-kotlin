...
| `HeaderHelpers+SyntheticBuilderDSLStubs.swift` | 0 (deleted) | (b) migrated | KSP-697: both `buildList` overloads use `CollectionBuilders.kt`; the legacy `__kk_build_*` lowering/ABI surface is fully retired (RF-LOWER-CALL-004 list, -005 set, -006 map), with the shared `isStdlibBuilderDSLCall` / `BuilderDSLLookupNames` entry point pending RF-LOWER-CALL-015. |
...
| `HeaderHelpers+SyntheticComparableHelpers.swift` | 81 | (c) | 上記の呼び出し先。`setupPrimitiveComparableImplementations`（Int/Long/Double/Float/Char/Boolean/UInt/ULong/UByte/UShort を `Comparable<Self>` へ適合、Companion anchor 込み）。言語コアの組込宣言に相当し KSP-700 後も (c) 恒久残留。 |
| `HeaderHelpers+SyntheticCollectionResiduals.swift` | 260 | (c)/(b) 混在 | 旧ファイルの3分割先の一つ。`registerSyntheticCollectionStubs` はorchestrator——Iterable/Collection/MutableCollection/MutableIterable/List/AbstractList/MutableList/Set/MutableSet/Map/MutableMap の各 register 関数（別ファイル所在、KSP-703/704/705/1542 等の担当）を正しい順序で呼び出すため、KSP-700 後も残置（呼び出し構造自体は変更していない）。`registerSyntheticRandomAccessStub` は `RandomAccess.kt`（KSP-669）を lookup-or-fallback するのみで実質 (c)。`makeComparableTypeParam` ヘルパーは `HeaderHelpers+SyntheticMutableListStubs.swift`/`+SyntheticMapStubs.swift` から使用中。 |
| `HeaderHelpers+SyntheticListResiduals.swift` | 904 | (c)/(b) 混在 | 旧ファイルの3分割先の一つ。**KSP-700（2026-09-15）**: `List<E>.get(index)` を `Stdlib/kotlin/collections/List.kt` へ source 化（既存 `__kk_list_get` ブリッジを再利用）、`registerListGetOperator` は `bundledIndex` ガード付きフォールバックへ縮小。呼び出し元のない dead 関数3件（`registerListSetOperationMembers`/`registerListToMapMember`/`registerListAsSequenceMember`）と kotlin-stdlib に実在しない `List.contentEquals` member 登録を削除。残る `isEmpty`（`kk_list_is_empty` が `CallLowerer+UnresolvedMemberCalls.swift`/`CollectionLiteralLoweringPass+LookupTables+List.swift` の name-string 定数として Sema メンバーシンボルと独立に参照されるため要監査）と `listIterator()`/`listIterator(index)`（`MutableList.listIterator()` との共変オーバーライド対で、MutableList 未移行の間は片側 source 化のみだと BUG-200 相当の再型付けギャップを再現しうる）は (c) 残置。`ensureSyntheticListIteratorStub`/`ensureSyntheticMutableListIteratorStub` の型シェル定義とメンバー登録は `ListIterator.kt`/`MutableListIterator.kt`（KSP-1064/945）に対する bundledIndex ガード付きフォールバックとして既に妥当な状態。`registerListTransformMembers`（別ファイル `+SyntheticListTransformMembers.swift` 定義、(b) 残・M3）への呼び出しは対象外のまま。 |
...
| `HeaderHelpers+SyntheticSetStubs.swift` | 843 | (b) | M3 set shell and HOF source migration. **KSP-704 で部分完了**（2026-09-14）: `Set.contains`/`isEmpty`/`iterator` は `Stdlib/kotlin/collections/Set.kt` へ `@KsSymbolName` 付きで source 化済み。`size` は `.property` への `@KsSymbolName` 未対応のため Swift 側 `registerSetSizeMember` を残置。`MutableSet` の変異系メンバ（add/remove/clear/addAll/plusAssign/removeAll/minusAssign/retainAll）は、このコンパイラが interface のボディなしメンバを `external`/`@KsSymbolName` の有無に関わらず一律 abstract 化するため、`LinkedHashSet`/`AbstractMutableMapKeys`（オーバーライドせずインターフェースのランタイムブリッジをデフォルト実装として利用）が壊れると判明し断念。詳細は TODO.md KSP-704 実装メモ参照。 |
...
| `collections/LinkedHashMap.kt`（KSP-703） | 解消済み（KUU-556: `LinkedHashMap<K, V>` を `HashMap<K, V>` の concrete subclass へ昇格し、diff オラクル（`kotlinc-jvm`: `LinkedHashMap extends HashMap`）と一致する一方向継承を再現。`linkedHashMapRuntimeTypeID` 新設、`isMapLikeSymbol`/`isMutableMapSymbol` の LinkedHashMap 対応漏れ修正、`__kk_linked_hash_map_of` 経由の construction 経路拡張、および HashMap が初めて subtype を持つことで顕在化した `keys`/`values`/`entries`/`size` の vtable dispatch 誤爆（KSWIFTK-RUNTIME-0001）を修正） | diff オラクルの `kotlinc-jvm` では `HashMap`/`LinkedHashMap` は java.util の別クラスで `LinkedHashMap extends HashMap`（実測: `HashMap() is LinkedHashMap<*, *>` は false、`LinkedHashMap() is HashMap<*, *>` は true） | KUU-556 で解消済み |
