...
| `HeaderHelpers+SyntheticCoercionStubs.swift` | 369 | (c) | **KSP-1544 (KUU-588)**: (b) は全て source-backed 化済み。`coerceIn`/`coerceAtLeast`/`coerceAtMost` は `ranges/RangeCoercion.kt`、Float/Double の `toByte()`/`toShort()` は `Numbers.kt`（`toInt().toX()` 合成 + `warningSince=1.3`/`errorSince=1.5`）へ移行。残置は Int×9 / Long×9 / `Double.toFloat()` の compiler/runtime residual (c) と、`--no-stdlib` fallback 用の `kotlin.math` package bootstrap。着手時の実測は 449 行で、654 行は stale。 |
...
#### KSP-1544: `HeaderHelpers+SyntheticCoercionStubs.swift` の (b) 残余（2026-09-18）

KSP-1532〜1535 は unsigned receiver の `toChar()` 誤分類を修正して close 済みであり、Coercion stub の未追跡 `(b)` 分ではない。master の 449 行を登録単位で再確認した結果、未移行だった `(b)` は `Float.toByte()`/`toShort()` と `Double.toByte()`/`toShort()` の 4 件だけだった。旧 stub は `__kk_*_to_int` を直接指して `Int` を返す誤シグネチャでもあったため、`Numbers.kt` の実装へ移し、`toInt().toByte()`/`toInt().toShort()` と本家準拠の error-level deprecation metadata を保持する。

`Int`/`Long` の固定幅変換と `Double.toFloat()` は KSP-1531 の `C-WIDTH`/`C-FP` に属する compiler/runtime residual `(c)` であり、今回も synthetic registration を残す。`kotlin.math` package bootstrap も `--no-stdlib` fallback のため残置する。したがってファイルは 369 行の `(c)` と確定し、`kk_*`/`__kk_*` ABI の削除・改名は発生しない。
