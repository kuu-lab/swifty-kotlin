...
| `HeaderHelpers+SyntheticBuilderDSLStubs.swift` | 0 (deleted) | (b) migrated | KSP-697: both `buildList` overloads use `CollectionBuilders.kt`; the `buildList` lowering/ABI cleanup landed in RF-LOWER-CALL-004, `buildSet` / `buildMap` remain tracked by RF-LOWER-CALL-005–006. |
