...
| `collections/LinkedHashMap.kt`（KSP-703） | `LinkedHashMap<K, V>` が `MutableMap<K, V>` への typealias（`HashMap.kt` が先に concrete class 化されたため alias 方向が逆転）。`HashMap()`/`LinkedHashMap()` は `CollectionLiteralLoweringPass` が名前で認識し、runtime map box を直接構築するため動作上の差異はない | kotlin-native は `LinkedHashMap` が concrete class、`HashMap` がその typealias（`actual typealias HashMap<K, V> = LinkedHashMap<K, V>`） | `LinkedHashMap` を concrete class（`@KsSymbolName` bridge 付き）へ昇格し、alias 方向を反転する構造変更。`CollectionLiteralLoweringPass` の construction 経路再設計を伴うため独立タスク化が必要 |
