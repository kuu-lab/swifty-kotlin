...
| `collections/LinkedHashMap.kt`（KSP-703） | `LinkedHashMap<K, V>` が `MutableMap<K, V>` への typealias（`HashMap.kt` が KSP-1055 で先に concrete class 化されたため別名方向が逆転）。`HashMap()`/`LinkedHashMap()` はどちらも `CollectionLiteralLoweringPass`（`+LookupTables+Map.swift` の `mutableMapConstructorNames`）が名前で認識し runtime map box を直接構築するため、機能上の差異はない | kotlin-native は `LinkedHashMap` が concrete class（`HashSet`/`LinkedHashSet` と同型）で `HashMap` がその typealias（`actual typealias HashMap<K, V> = LinkedHashMap<K, V>`） | `LinkedHashSet` と同様に `LinkedHashMap` を concrete class（`@KsSymbolName` ブリッジ付き）へ昇格し、別名方向を反転する構造変更。`CollectionLiteralLoweringPass` の construction 経路の再設計を伴うため独立タスク化が必要 |
