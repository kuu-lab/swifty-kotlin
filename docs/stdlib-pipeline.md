...
9. **タスク運用**: 未完了タスクの削除禁止（`[x]` のみ削除可）。本文から参照される ID は必ず解決可能に保つ
   （M1–M17 / CLEANUP-STUB 個別リスト消失の教訓）。作業中に発見したバグは、症状を再現する最小ケースと回帰テストを含めて同じPR内で修正する（AGENTS.md「バグ修正ルール」）。
   同じPRのスコープや安全な修正方針を超える場合は、理由付きで PR description に記録する。TODO.md には追加しない（並行セッションとの同一箇所挿入コンフリクトの原因になるため）。
