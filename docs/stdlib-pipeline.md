...
> 補足（§12 も参照）: 並行して書かれた設計メモでは閾値の提案が異なる（Smoke 相当の入力で
> wall-clock 15%未満 or 200ms未満）。実測後にどちらの基準で正式化するかをすり合わせること。
...
- **コンパイル時間の許容閾値が §7 と異なる**: §7 は「hello.kt 相当の小入力で +100ms」を計測トリガーとするが、
  並行メモは「Smoke 相当の小さい入力で wall-clock 15%未満 or 200ms未満、golden suite 10%未満、
  `diff_kotlinc.sh` shards 10%未満」を提案していた。**要すり合わせ**（§7 にも注記済み）。
