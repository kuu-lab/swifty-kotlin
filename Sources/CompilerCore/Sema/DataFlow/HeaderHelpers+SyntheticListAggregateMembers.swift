...
extension DataFlowSemaPhase {
    func registerListAggregateMembers(
        ...
        // but member stub takes precedence. Keep until RF-LOWER dispatch is wired for extension fallback.
        ...
        // firstOrNull predicate HOF (STDLIB-COL-FN-074)
        // NOTE: Kotlin source exists in BundledKotlinStdlib.kotlinCollectionsSearchSource (MIGRATION-COL-005)
        // Keep stub until RF-LOWER dispatch is wired for extension fallback.
        let firstOrNullHOFName = interner.intern("firstOrNull")
        let firstOrNullHOFFQName = listFQName + [firstOrNullHOFName]
        registerMemberOverload(
            memberName: firstOrNullHOFName,
            memberFQName: firstOrNullHOFFQName,
            parameterTypes: [predicateType],
            externalLinkName: "kk_list_firstOrNull_predicate",
            returnTypeOverride: nullableElementType,
            flags: [.synthetic, .inlineFunction]
        )

        // indexOfFirst / indexOfLast (HOF, predicate lambda)
        // NOTE: Kotlin source exists in BundledKotlinStdlib.kotlinCollectionsSearchSource (MIGRATION-COL-005)
        // but member stub takes precedence. Keep until RF-LOWER dispatch is wired.
