...
/// Synthetic stdlib stubs for Kotlin scope functions (STDLIB-061, STDLIB-401).
/// - with<T, R>(receiver: T, block: T.() -> R): R
/// - run<R>(block: () -> R): R
/// - T.run<R>(block: T.() -> R): R
/// Inline-expanded by CallLowerer; no runtime call.
///
/// KSP-603: `context` (arity 1-6) and `contextOf` now live in bundled Kotlin
/// source (`Stdlib/kotlin/ContextParameters.kt`).
