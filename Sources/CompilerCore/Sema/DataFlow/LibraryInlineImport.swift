...
extension DataFlowSemaPhase {
    ...
    private func parseImportedInlineInstruction(
        ...
        case "virtualCall":
            guard let calleeEncoded = pairs["calleeB64"],
                  ...
                  let dispatch = parseInlineDispatchKind(dispatchRaw)
            else {
                return nil
            }
            let args = parseInlineIntList(pairs["args"] ?? "[]").map { value in
    ...
    /// Mirrors the dispatch encoding written by `CodegenPhase`'s inline KIR
    /// serializer (`vtable:<slot>`, `itable:<iface>:<method>`,
    /// `itableDynamic:<typeID>:<method>`).
    private func parseInlineDispatchKind(_ raw: String) -> KIRDispatchKind? {
