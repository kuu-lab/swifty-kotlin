...
extension CollectionLiteralConstructionLoweringPass {
    ...
    // sumBy / sumByDouble still route to runtime helpers; sumOf is now bundled Kotlin source.
    if callee == lookup.sumByName || callee == lookup.sumByDoubleName {
        if arguments.count == 2 || arguments.count == 3 {
            ...
            if state.listExprIDs.contains(receiverID.rawValue) {
                ...
                if callee == lookup.sumByDoubleName {
    ...
    // only sumBy / sumByDouble still route to runtime helpers above.
