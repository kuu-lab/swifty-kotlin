...
final class InlineLoweringPass: LoweringPass {
    ...
    private func collectInlineFunctions(module: KIRModule, ctx: KIRContext) -> [SymbolID: KIRFunction] {
        var inlineFunctionsBySymbol = Dictionary(uniqueKeysWithValues: module.arena.declarations.compactMap { decl -> (SymbolID, KIRFunction)? in
            guard case let .function(function) = decl, function.isInline,
                  !isGeneratedLambdaFunction(function, ctx: ctx)
            else {
                return nil
            }
        ...
        if let imported = ctx.sema?.importedInlineFunctions {
            for (symbol, function) in imported where inlineFunctionsBySymbol[symbol] == nil {
                if !isGeneratedLambdaFunction(function, ctx: ctx) {
    ...
    /// Lambda bodies generated by `LambdaClosureConversionPass` are named
    /// `kk_lambda_*` (or `kk_hof_adapter_*` wrappers). They are expanded at their
    /// invocation sites via `expandLambdaBody`, not as ordinary inline functions,
    /// so they should not be treated as inline-expansion targets. Inlining them
    /// as regular functions splices a pre-ABI body into the caller; later passes
    /// then miss the type information needed to insert the correct boxing/unboxing
    /// and narrowing operations.
    private func isGeneratedLambdaFunction(_ function: KIRFunction, ctx: KIRContext) -> Bool {
        let name = ctx.interner.resolve(function.name)
        return name.hasPrefix("kk_lambda_") || name.hasPrefix("kk_hof_adapter_")
    }
