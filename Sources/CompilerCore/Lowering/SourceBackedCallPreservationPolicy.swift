...
struct SourceBackedCallPreservationPolicy {
    ...
    init(lookup: CollectionLiteralLookupTables, interner: StringInterner) {
        sharedAggregateNames = [
            ...
            // RF-LOWER-CALL-008: the List transform/filter HOFs are
            // source-backed (KSP-421) and `StdlibSurfaceSpec.listHOFMembers`
            // carries no runtime link for them, so no direct-path rewrite can
            // claim those callees — the derived transform names moved to
            // `virtualOnlyAggregateNames`. `map` / `flatMap` /
            // `flatMapIndexed` / `flatten` stay: the Map receiver rewrite
            // (`kk_map_*`) and the Sequence pipeline/terminal rewrites still
            // key off these names (Map is RF-LOWER-CALL-012, Sequence is -014).
            lookup.mapName,
            lookup.flatMapName,
        ...
        virtualOnlyAggregateNames = [
            ...
            // RF-LOWER-CALL-008 moved these twelve off the direct chain: the
            // List transform/filter rewrites they guarded are gone. The
            // virtual list kept them (the original change left the virtual
            // predicate untouched), so they live here until RF-LOWER-CALL-014
            // revisits the Sequence question.
