...
extension CallLowerer {
    ...
    func tryLowerStoredMemberPropertyRead(
        ...
        if ownerInfo.kind == .interface {
            ...
            // properties declared in Kotlin dispatch through the itable: user
            // code and bundled stdlib source (they carry a decl site, e.g.
            // `SharedFlow.replayCache`), plus the same declarations read back
            // from a precompiled library. The owner alone is not enough — an
            // imported nominal can still gain synthetic runtime members.
