...
extension KIRLoweringDriver {
    ...
    /// Stores `value` into the storage backing `targetSymbol` (the property
    /// itself, or its separately-materialized backing field symbol) on the
    /// instance currently under construction.
    ///
    /// Class instance properties live inside the heap-allocated object, not
    /// in a module-global slot, so the write must go through `kk_array_set`
    /// at the property's computed field offset — the same mechanism reads
    /// use in `tryLowerStoredMemberPropertyRead` — falling back to a direct
    /// `.copy` to a `.symbolRef` only when no constructor receiver/layout is
    /// available (top-level and object-singleton properties, which really
    /// are backed by a module-global slot). `NominalLayout.fieldOffsets` is
    /// only ever keyed by the property symbol itself (see LayoutSynthesis),
    /// so `targetSymbol`'s own offset is tried first for forward-compat but
    /// always falls back to `propSymbol`'s offset.
    private func emitMemberOrGlobalFieldStore(
        propSymbol: SymbolID,
