...
extension ExprLowerer {
    ...
    /// Runtime entry point reading the current value of a stdlib delegate handle
    /// (`lazy`, `Delegates.observable/vetoable/notNull`).
    static func stdlibDelegateGetValueRuntimeName(for kind: StdlibDelegateKind) -> String? {
        switch kind {
    ...
    /// Runtime entry point writing through a stdlib delegate handle. `lazy` is
    /// read-only, so it has none.
    static func stdlibDelegateSetValueRuntimeName(for kind: StdlibDelegateKind) -> String? {
        switch kind {
    ...
    /// Lowers a read of a local `by`-delegated declaration backed by a stdlib
    /// delegate (BUG-147) into a `kk_<kind>_get_value` call on its handle.
    ///
    /// Emitted per read rather than once at the declaration so that
    /// `val x by lazy { ... }` still initializes on first access, and so that a
    /// `var` read after a write observes what the delegate actually stored.
    /// Returns nil for locals that are not stdlib-delegated.
    func lowerLocalStdlibDelegateRead(
        symbol: SymbolID,
        sema: SemaModule,
        arena: KIRArena,
        interner: StringInterner,
        ...
        guard let kind = driver.ctx.localStdlibDelegateKind(for: symbol),
              let handleExprID = driver.ctx.localDelegateStorage(for: symbol),
              let getValueName = Self.stdlibDelegateGetValueRuntimeName(for: kind)
        else {
            return nil
        }
        let valueType = driver.ctx.localDeclaredType(for: symbol)
            ?? sema.symbols.propertyType(for: symbol)
            ?? sema.types.anyType
        let result = arena.appendTemporary(type: valueType)
        instructions.append(.call(
            symbol: nil,
            callee: interner.intern(getValueName),
            arguments: [handleExprID],
            result: result,
            canThrow: false,
            thrownResult: nil
        ))
