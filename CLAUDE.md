...
KSwiftK は Swift で書かれた Kotlin コンパイラ。Kotlin 2.3.10 stable をターゲットとし、LLVM 経由で macOS ネイティブ実行ファイルを生成する。実行バイナリは `kswiftc` とエディタ連携用の LSP サーバー `kswift-lsp`。
...
モジュール構成: `CompilerCore`（コンパイラ本体）/ `KSwiftKCLI`（CLIエントリ）/ `LSPServer`（Language Server）/ `KSwiftLSPCLI`（LSPエントリ）/ `Runtime`（GC・coroutine）
