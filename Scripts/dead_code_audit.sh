...
#   fixture 2: kk_atomic_int_array_fetchAndUpdateAt — 2 段階 prefix 生成で到達。B に入れば誤り。
if [[ $SELFTEST -eq 1 ]]; then
  ...
  if grep -qx "kk_atomic_int_array_fetchAndUpdateAt" "$WORK/dead_B.txt"; then
    echo "FAIL: kk_atomic_int_array_fetchAndUpdateAt misclassified as B (two-stage prefix emit not detected)" >&2
    selftest_failed=1
  else
    echo "PASS: kk_atomic_int_array_fetchAndUpdateAt not in B (two-stage prefix emit detected)"
  fi
